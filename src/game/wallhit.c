...
struct wallhittex g_WallhitTexes[] = {
	...
	/*0x12*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOOD1
	/*0x13*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOOD2
	/*0x14*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOOD3
	/*0x15*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOOD4
	/*0x16*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOOD5
	/*0x17*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOODSIDE1
	/*0x18*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOODSIDE2
	/*0x19*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOODSIDE3
	/*0x1a*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOODSIDE4
	/*0x1b*/ { 18,  18,  WALLHITTYPE_BLOOD  }, // WALLHITTEX_CHRBLOODSIDE5
};
...
void wallhitsTick(void)
{
	...
	for (i = 0; i < g_WallhitsMax; i++, wallhit++) {
		...
		if (wallhit->texturenum >= WALLHITTEX_CHRBLOOD1 && wallhit->texturenum <= WALLHITTEX_CHRBLOOD5) {
			s32 frameindex = (g_Vars.lvframenum - wallhit->createdframe) / 2;
			...
			wallhit->texturenum = WALLHITTEX_CHRBLOOD1 + frameindex;
			wallhit->expanding = false;
			wallhit->timermax = 0;
			wallhit->timercur = 0;
		...
		s32 rotdeg, u32 timermax, u32 timerspeed, bool xlu)
{
	...
	if (g_FreeWallhits != NULL) {
		...
		if (objprop) {
			...
			if (objprop->type == PROPTYPE_CHR || objprop->type == PROPTYPE_PLAYER) {
				model = objprop->chr->model;
			} else {
...
Gfx *wallhitRenderPropHits(Gfx *gdl, struct prop *prop, bool xlu)
{
	...
	while (wallhit) {
		if (wallhit->inuse) {
			...
			if (wallhit->mtxindex != prevmtxindex) {
				Mtxf *mtx = &model->matrices[wallhit->mtxindex];
				if (wallhit->mtxindex);
	...
	if (!hasany && prop->type != PROPTYPE_CHR && prop->type != PROPTYPE_PLAYER) {
		prop->obj->hidden2 &= ~(1 << xlu);
	}
