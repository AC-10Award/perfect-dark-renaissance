...
void botmgrAllocateBot(s32 chrnum, s32 aibotnum)
{
...
#ifndef PLATFORM_N64
	sysLogPrintf(LOG_NOTE, "botmgrAllocateBot: begin chrnum=%d botslot=%d head=%d body=%d", chrnum, aibotnum, headnum, bodynum);
#endif
	...
	if (model != NULL) {
		...
		if (prop != NULL) {
			...
			if (g_BotCount < MAX_BOTS_RUNTIME) {
...
#ifndef PLATFORM_N64
					sysLogPrintf(LOG_NOTE, "botmgrAllocateBot: registered chrnum=%d botslot=%d g_BotCount=%d g_MpNumChrs=%d", chrnum, aibotnum, g_BotCount, g_MpNumChrs);
#endif
...
#ifndef PLATFORM_N64
					sysLogPrintf(LOG_ERROR, "botmgrAllocateBot: aibot alloc failed for botslot=%d", aibotnum);
#endif
					return;
				}
...
#ifndef PLATFORM_N64
	if (model == NULL) {
		sysLogPrintf(LOG_ERROR, "botmgrAllocateBot: model allocation failed for botslot=%d", aibotnum);
	}
#endif
}
