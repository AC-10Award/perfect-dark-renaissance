...
struct model *body0f02ce8c(s32 bodynum, s32 headnum, struct modeldef *bodymodeldef, struct modeldef *headmodeldef, bool sunglasses, struct model *model, bool isplayer, u8 varyheight)
{
	...
	if (bodymodeldef == NULL) {
		sysLogPrintf(LOG_ERROR,
				"body0f02ce8c: missing body modeldef bodynum=%d headnum=%d isplayer=%d",
				bodynum,
				headnum,
				isplayer ? 1 : 0);
	}
		...
		if (bodymodeldef->skel == &g_SkelChr) {
			...
			if (node != NULL) {
				if (headnum < 0) {
					...
					if (headmodeldef == NULL) {
						sysLogPrintf(LOG_ERROR,
								"body0f02ce8c: missing head modeldef bodynum=%d headnum=%d isplayer=%d",
								bodynum,
								headnum,
								isplayer ? 1 : 0);
					}
	...
	if (model == NULL) {
		sysLogPrintf(LOG_ERROR,
				"body0f02ce8c: instantiate failed bodynum=%d headnum=%d bodyrwlen=%u headrwlen=%u isplayer=%d",
				bodynum,
				headnum,
				bodymodeldef ? bodymodeldef->rwdatalen : 0,
				headmodeldef ? headmodeldef->rwdatalen : 0,
				isplayer ? 1 : 0);
	}
#endif
...
s32 bodyChooseVoicebox(s32 bodynum, s32 headnum)
{
	if (bodynum == BODY_ROBOT_SKELINGTON) {
		return VOICEBOX_NONE;
	}

	if (headnum >= 0 && headnum < ARRAYCOUNT(g_HeadsAndBodies)) {
		if (!g_HeadsAndBodies[headnum].ismale) {
			return VOICEBOX_FEMALE;
		}
	} else if (bodynum >= 0 && bodynum < ARRAYCOUNT(g_HeadsAndBodies)) {
