...
u32 pak0f118674(s8 device, u32 filetype, s32 *outfileid)
{
	...
	if (offset == 0 ||
		...
		if (pakWriteFileAtOffset(device, bestoffset, filetype, NULL, 0, outfileid, NULL, 0, 1) == 0) {
			...
			// Write new termination after file
 ...
 * write a termination file at the start address and return true. A blank file
...
bool pakRepairAsBlank(s8 device, u32 *offsetptr, struct pakfileheader *header)
{
	...
	while (offset < g_Paks[device].pdnumbytes) {
		...
		// If the end was reached, write a termination at the starting offset
 ...
 *
 * ntsc-1.0:
 * - Fixes the termination if missing
...
s32 pakRepairFilesystem(s8 device)
{
	...
	// Stop when the end is reached, or a blank file or termination is found.
...
#endif
			{
				// File overflows the device length -> replace with termination
	...
	// Return 0 if the first file was a termination (ie. pak is empty).
 ...
 * - A termination file (containing the serial) is then written at the start of the pak.
 * - Random bytes are then written into a block after the termination.
 *
