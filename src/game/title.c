...
#ifndef PLATFORM_N64
#define PRODUCT_IDENTIFICATION_TEX_NUM 0x0a9a
#define PRODUCT_IDENTIFICATION_TEX_WIDTH 64
#define PRODUCT_IDENTIFICATION_TEX_HEIGHT 24

static Gfx *titleRenderProductIdentificationCard(Gfx *gdl)
{
	const f32 imageaspect = 16.0f / 9.0f;
	const f32 screenaspect = videoGetAspect();
	s32 x1 = 0;
	s32 y1 = 0;
	s32 x2 = viGetWidth();
	s32 y2 = viGetHeight();
	...
	if (screenaspect > imageaspect) {
		width = (s32)(viGetWidth() * imageaspect / screenaspect);
		x1 = (viGetWidth() - width) / 2;
		x2 = x1 + width;
	} else if (screenaspect < imageaspect) {
		height = (s32)(viGetHeight() * screenaspect / imageaspect);
		y1 = (viGetHeight() - height) / 2;
		y2 = y1 + height;
	}

	width = x2 - x1;
	height = y2 - y1;
	dsdx = (PRODUCT_IDENTIFICATION_TEX_WIDTH << 10) / width;
	dtdy = -((PRODUCT_IDENTIFICATION_TEX_HEIGHT << 10) / height);
	...
	gSPTextureRectangle(gdl++,
			...
			G_TX_RENDERTILE,
			0,
			PRODUCT_IDENTIFICATION_TEX_HEIGHT << 5,
			dsdx,
			dtdy);
...
#ifndef PLATFORM_N64
	if (extTexExists(G_TEXTYPE_GENERAL, 0, PRODUCT_IDENTIFICATION_TEX_NUM)) {
		return titleRenderProductIdentificationCard(gdl);
	}
#endif
...
void titleInitSkip(void)
{
	...
	if (g_TitleNextStage == STAGE_CITRAINING) {
		g_TitleBootToMenu = true;
	}
