...
// Deliberately oversized for stress testing so pool exhaustion is easy to rule out.
#define CHR_BLOOD_MARKER_POOL_SIZE 256
#define CHR_BLOOD_HIT_POOL_SIZE 600
#define CHR_BLOOD_FRAME_TICKS TICKS(3)
#define CHR_BLOOD_NUM_FRAMES 5
#define CHR_BLOOD_ANIM_LIFETIME (CHR_BLOOD_FRAME_TICKS * CHR_BLOOD_NUM_FRAMES)
#define CHR_BLOOD_MARKER_LIFETIME CHR_BLOOD_ANIM_LIFETIME
#define CHR_BLOOD_HIT_LIFETIME TICKS(45)

static struct chrbloodmarker g_ChrBloodMarkers[CHR_BLOOD_MARKER_POOL_SIZE];
static struct chrbloodhit g_ChrBloodHits[CHR_BLOOD_HIT_POOL_SIZE];

static bool chrIsRobotSkelingtonBody(struct chrdata *chr)
{
	return chr && chr->bodynum == BODY_ROBOT_SKELINGTON;
}

static u32 g_ChrBloodDebugFleshHits = 0;
static u32 g_ChrBloodDebugEligibleHits = 0;
static u32 g_ChrBloodDebugRequests = 0;
static u32 g_ChrBloodDebugSpawned = 0;
static u32 g_ChrBloodDebugRejectedNull = 0;
static u32 g_ChrBloodDebugSlotReuses = 0;
static u32 g_ChrBloodDebugSkipNoBlood = 0;
static u32 g_ChrBloodDebugSkipRace = 0;
static u32 g_ChrBloodDebugSkipMelee = 0;
static u32 g_ChrBloodDebugSkipTranq = 0;
static u32 g_ChrBloodDebugSkipPaintball = 0;
static u32 g_ChrBloodDebugRenderCalls = 0;
static u32 g_ChrBloodDebugRenderedHits = 0;
static u32 g_ChrBloodDebugRenderScans = 0;
static u32 g_ChrBloodDebugPeakActiveHits = 0;
static u32 g_ChrBloodDebugPeakActiveMarkers = 0;
static s32 g_ChrBloodDebugMinFreeGfx = -1;
static s32 g_ChrBloodDebugMinFreeVtx = -1;
static s32 g_ChrBloodDebugLastReportFrame60 = -1;

static s32 chrBloodGetFrameIndex(s32 age)
{
	s32 frameindex = age / CHR_BLOOD_FRAME_TICKS;

	if (frameindex >= CHR_BLOOD_NUM_FRAMES) {
		frameindex = CHR_BLOOD_NUM_FRAMES - 1;
	}

	return frameindex;
}

static u8 chrBloodGetFadeAlpha(s32 age)
{
	if (age < 0) {
		return 255;
	}

	if (age >= CHR_BLOOD_ANIM_LIFETIME) {
		return 0;
	}

	return (CHR_BLOOD_ANIM_LIFETIME - age) * 255 / CHR_BLOOD_ANIM_LIFETIME;
}

static void chrBloodDebugSampleRenderBudget(Gfx *gdl)
{
	s32 freegfx = gfxGetFreeGfx(gdl);
	s32 freevtx = gfxGetFreeVtx();

	if (g_ChrBloodDebugMinFreeGfx < 0 || freegfx < g_ChrBloodDebugMinFreeGfx) {
		g_ChrBloodDebugMinFreeGfx = freegfx;
	}

	if (g_ChrBloodDebugMinFreeVtx < 0 || freevtx < g_ChrBloodDebugMinFreeVtx) {
		g_ChrBloodDebugMinFreeVtx = freevtx;
	}
}

static void chrBloodDebugSampleActivePools(void)
{
	u32 activehits = 0;
	u32 activemarkers = 0;
	s32 i;

	for (i = 0; i < ARRAYCOUNT(g_ChrBloodHits); i++) {
		if (g_ChrBloodHits[i].prop) {
			activehits++;
		}
	}

	for (i = 0; i < ARRAYCOUNT(g_ChrBloodMarkers); i++) {
		if (g_ChrBloodMarkers[i].prop) {
			activemarkers++;
		}
	}

	if (activehits > g_ChrBloodDebugPeakActiveHits) {
		g_ChrBloodDebugPeakActiveHits = activehits;
	}

	if (activemarkers > g_ChrBloodDebugPeakActiveMarkers) {
		g_ChrBloodDebugPeakActiveMarkers = activemarkers;
	}
}

static void chrBloodMarkersTick(void)
{
	s32 i;

	for (i = 0; i < ARRAYCOUNT(g_ChrBloodMarkers); i++) {
		if (g_ChrBloodMarkers[i].prop && g_ChrBloodMarkers[i].createdframe < g_Vars.lvframe60 - CHR_BLOOD_MARKER_LIFETIME) {
			g_ChrBloodMarkers[i].prop = NULL;
		}
	}
}

static void chrBloodHitsTick(void)
{
	s32 i;

	for (i = 0; i < ARRAYCOUNT(g_ChrBloodHits); i++) {
		struct chrbloodhit *bloodhit = &g_ChrBloodHits[i];

		if (bloodhit->prop) {
			s32 age = g_Vars.lvframe60 - bloodhit->createdframe;

			/*
			 * Keep the slot reserved for the longer churn test window, but drop
			 * render-side pointers once the visible animation has ended so stale
			 * model/node references can't survive into a respawn/model rebuild.
			 */
			if (age >= CHR_BLOOD_ANIM_LIFETIME) {
				bloodhit->model = NULL;
				bloodhit->node = NULL;
...
static s16 chrDamageClampS16(s32 value)
{
	if (value < -32768) {
		return -32768;
	}
...
static struct chrbloodhit *chrBloodHitAlloc(bool *reusedslot)
{
	struct chrbloodhit *bloodhit = NULL;
	s32 i;
	s32 oldestindex = 0;
	...
	for (i = 0; i < ARRAYCOUNT(g_ChrBloodHits); i++) {
		if (g_ChrBloodHits[i].prop == NULL) {
			bloodhit = &g_ChrBloodHits[i];
			break;
		}

		if (g_ChrBloodHits[i].createdframe < oldestframe) {
			oldestframe = g_ChrBloodHits[i].createdframe;
			oldestindex = i;
		}
	}

	if (bloodhit == NULL) {
		bloodhit = &g_ChrBloodHits[oldestindex];
...
static bool chrBloodSprayCreateVariant(struct prop *prop, struct model *model, struct modelnode *node, struct coord *localhitpos, u8 variant)
{
	struct chrbloodhit *bloodhit;
	s16 hitpos[3];
	bool reusedslot = false;

	if (prop == NULL || model == NULL || node == NULL || localhitpos == NULL) {
		g_ChrBloodDebugRejectedNull++;
		return false;
	}

	hitpos[0] = chrDamageClampS16(localhitpos->x);
	hitpos[1] = chrDamageClampS16(localhitpos->y);
	hitpos[2] = chrDamageClampS16(localhitpos->z);

	bloodhit = chrBloodHitAlloc(&reusedslot);
	...
	if (reusedslot) {
		g_ChrBloodDebugSlotReuses++;
	}

	bloodhit->prop = prop;
	bloodhit->node = node;
	bloodhit->model = model;
	bloodhit->createdframe = g_Vars.lvframe60;
	bloodhit->pos[0] = hitpos[0];
	bloodhit->pos[1] = hitpos[1];
	bloodhit->pos[2] = hitpos[2];
	bloodhit->variant = variant;

	return true;
}

static bool chrBloodSprayCreate(struct prop *prop, struct model *model, struct modelnode *node, struct coord *localhitpos)
{
	bool spawned = false;

	spawned |= chrBloodSprayCreateVariant(prop, model, node, localhitpos, 0);
	spawned |= chrBloodSprayCreateVariant(prop, model, node, localhitpos, 1);

	return spawned;
}

static void chrBloodDebugMaybeReport(void)
{
	s32 interval = TICKS(300);

	if (!g_Vars.normmplayerisrunning) {
		g_ChrBloodDebugLastReportFrame60 = g_Vars.lvframe60;
		return;
	}

	if (g_ChrBloodDebugLastReportFrame60 < 0) {
		g_ChrBloodDebugLastReportFrame60 = g_Vars.lvframe60;
		return;
	}

	if (g_Vars.lvframe60 - g_ChrBloodDebugLastReportFrame60 < interval) {
		return;
	}

	g_ChrBloodDebugLastReportFrame60 = g_Vars.lvframe60;
	chrBloodDebugSampleActivePools();

#ifdef PLATFORM_N64
	osSyncPrintf(
			"BloodDbg: flesh=%u eligible=%u requests=%u spawned=%u nullrej=%u reuse=%u rendercalls=%u rendered=%u scans=%u peak[hits=%u markers=%u] mingfx=%d minvtx=%d skip[noblood=%u race=%u melee=%u tranq=%u paint=%u]\n",
			g_ChrBloodDebugFleshHits,
			g_ChrBloodDebugEligibleHits,
			g_ChrBloodDebugRequests,
			g_ChrBloodDebugSpawned,
			g_ChrBloodDebugRejectedNull,
			g_ChrBloodDebugSlotReuses,
			g_ChrBloodDebugRenderCalls,
			g_ChrBloodDebugRenderedHits,
			g_ChrBloodDebugRenderScans,
			g_ChrBloodDebugPeakActiveHits,
			g_ChrBloodDebugPeakActiveMarkers,
			g_ChrBloodDebugMinFreeGfx,
			g_ChrBloodDebugMinFreeVtx,
			g_ChrBloodDebugSkipNoBlood,
			g_ChrBloodDebugSkipRace,
			g_ChrBloodDebugSkipMelee,
			g_ChrBloodDebugSkipTranq,
			g_ChrBloodDebugSkipPaintball);
#else
	sysLogPrintf(
			LOG_NOTE,
			"BloodDbg: flesh=%u eligible=%u requests=%u spawned=%u nullrej=%u reuse=%u rendercalls=%u rendered=%u scans=%u peak[hits=%u markers=%u] mingfx=%d minvtx=%d skip[noblood=%u race=%u melee=%u tranq=%u paint=%u]",
	...
	g_ChrBloodDebugRenderCalls = 0;
	g_ChrBloodDebugRenderedHits = 0;
	g_ChrBloodDebugRenderScans = 0;
	g_ChrBloodDebugPeakActiveHits = 0;
	g_ChrBloodDebugPeakActiveMarkers = 0;
	g_ChrBloodDebugMinFreeGfx = -1;
	g_ChrBloodDebugMinFreeVtx = -1;
}

static Gfx *chrBloodMarkersRender(Gfx *gdl, struct prop *prop)
{
	s32 i;
	bool used = false;
	Mtxf *mtx;
	...
	for (i = 0; i < ARRAYCOUNT(g_ChrBloodMarkers); i++) {
		struct chrbloodmarker *marker = &g_ChrBloodMarkers[i];

		if (marker->prop == prop) {
			s32 age = g_Vars.lvframe60 - marker->createdframe;

			if (age >= CHR_BLOOD_MARKER_LIFETIME) {
				marker->prop = NULL;
				continue;
			}
			...
			{
				s32 frameindex = chrBloodGetFrameIndex(age);
				struct textureconfig *bloodtex;
				Mtxf billmtx;
				Vtx *vertices;
				struct coord pos;
				f32 normalmag;
				...
				bloodtex = &g_TexWallhitConfigs[18 + frameindex];
				texSelect(&gdl, bloodtex, 4, 0, 2, true, NULL);

				pos = marker->pos;
				normalmag = sqrtf(marker->rot.x * marker->rot.x + marker->rot.y * marker->rot.y + marker->rot.z * marker->rot.z);

				if (normalmag > 0.0001f) {
					pos.x += marker->rot.x * (6.0f / normalmag);
					pos.y += marker->rot.y * (6.0f / normalmag);
					pos.z += marker->rot.z * (6.0f / normalmag);
				}

				mtx4LoadIdentity(&billmtx);
				mtx4SetTranslation(&pos, &billmtx);
				mtx00015be0(camGetWorldToScreenMtxf(), &billmtx);

				mtx = gfxAllocateMatrix();
				mtxF2L(&billmtx, mtx);
				gSPMatrix(gdl++, osVirtualToPhysical(mtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

				halfsize = marker->size;
				...
				vertices[0].s = 0;
				vertices[0].t = 0;
				vertices[1].s = bloodtex->width * 32;
				vertices[1].t = 0;
				vertices[2].s = bloodtex->width * 32;
				vertices[2].t = bloodtex->height * 32;
				vertices[3].s = 0;
				vertices[3].t = bloodtex->height * 32;
...
static Gfx *chrBloodHitsRender(Gfx *gdl, struct prop *prop)
{
	s32 i;
	struct model *model = NULL;

	g_ChrBloodDebugRenderCalls++;
	g_ChrBloodDebugRenderScans += ARRAYCOUNT(g_ChrBloodHits);
	chrBloodDebugSampleRenderBudget(gdl);
	...
	for (i = 0; i < ARRAYCOUNT(g_ChrBloodHits); i++) {
		if (g_ChrBloodHits[i].prop == prop && g_ChrBloodHits[i].model && g_ChrBloodHits[i].node) {
			s32 age = g_Vars.lvframe60 - g_ChrBloodHits[i].createdframe;
			struct shieldhit fakehit = {0};

			if (age < 0 || age >= CHR_BLOOD_ANIM_LIFETIME) {
				continue;
			}

			if (g_ChrBloodHits[i].model != model) {
				g_ChrBloodHits[i].model = NULL;
				g_ChrBloodHits[i].node = NULL;
				continue;
			}

			fakehit.lvframe60 = g_ChrBloodHits[i].createdframe;
			fakehit.unk011 = g_ChrBloodHits[i].variant;
			fakehit.unk012 = g_ChrBloodHits[i].pos[0];
			fakehit.unk014 = g_ChrBloodHits[i].pos[1];
			fakehit.unk016 = g_ChrBloodHits[i].pos[2];

			gdl = chrRenderShieldComponent(gdl, &fakehit, prop, g_ChrBloodHits[i].model, g_ChrBloodHits[i].node, -9, -1, -1, 255);
			g_ChrBloodDebugRenderedHits++;
		}
	}
...
static u8 chrDamageClampU8(s32 value)
{
	if (value < 0) {
		return 0;
	}
...
Gfx *chrRender(struct prop *prop, Gfx *gdl, bool xlupass)
{
	...
	if (shademode != SHADEMODE_XLU && alpha > 0) {
		...
		renderdata.gdl = chrBloodHitsRender(renderdata.gdl, prop);

		renderdata.gdl = chrBloodMarkersRender(renderdata.gdl, prop);
...
void chrEmitSparks(struct chrdata *chr, struct prop *prop, s32 hitpart, struct coord *coord, struct coord *coord2, struct chrdata *chr2)
{
	...
	if (chrIsRobotSkelingtonBody(chr)) {
		sparksCreate(chrprop->rooms[0], chrprop, coord, coord2, 0, SPARKTYPE_DEFAULT);
		mdMaterialImpactPlayTerminationMetalHit();
		return;
	}
				...
				s32 op = (s8)gdlptr->bytes[GFX_W0_BYTE(0)];
					...
					if (op == G_ENDDL) {
						...
						if (op == G_VTX) {
							...
							if (posnode) {
								for (i = 0; i < numverts; i++) {
									s32 worldx = vertices[i].x + (s32)spd4.f[0];
									s32 worldy = vertices[i].y + (s32)spd4.f[1];
									s32 worldz = vertices[i].z + (s32)spd4.f[2];
									s32 dx = worldx - bestcoords[0];
									s32 dy = worldy - bestcoords[1];
									s32 dz = worldz - bestcoords[2];
									s32 dist2 = dx * dx + dy * dy + dz * dz;
									s32 outerradius = 90;
									s32 innerradius = 36;
									s32 outerradius2 = outerradius * outerradius;
									s32 innerradius2 = innerradius * innerradius;
									...
									if (dist2 <= outerradius2) {
										...
										if ((uintptr_t)rwdata->colours != ALIGN8((uintptr_t)rodata->vertices + rodata->numvertices * sizeof(Vtx))) {
											s32 offset = rwdata->vertices[word / sizeof(Vtx) + i].colour >> 2;
											Col *colours = (Col *) ((uintptr_t)rwdata->colours + spac);
											s32 outerstrength = (outerradius2 - dist2) * 255 / outerradius2;
											s32 corestrength = dist2 < innerradius2 ? (innerradius2 - dist2) * 255 / innerradius2 : 0;
											s32 red = colours[offset].r;
											s32 green = colours[offset].g;
											s32 blue = colours[offset].b;

											red = red + 120 + (100 * outerstrength) / 255;
											green = (green * (255 - outerstrength / 2 - corestrength / 3)) / 255;
											blue = (blue * (255 - outerstrength - corestrength / 2)) / 255;

											colours[offset].r = chrDamageClampU8(red);
											colours[offset].g = chrDamageClampU8(green);
											colours[offset].b = chrDamageClampU8(blue);
											colours[offset].a = 255;
										}
									}
								}
							}
	...
	if ((chr->chrflags & CHRCFLAG_HIDDEN) == 0) {
		...
		if (shield <= 0) {
			if (hit->prop->type == PROPTYPE_WEAPON) {
				...
				if (chr->noblood) {
					g_ChrBloodDebugSkipNoBlood++;
				} else if (chrIsRobotSkelingtonBody(chr)) {
					g_ChrBloodDebugSkipRace++;
				} else if (race == RACE_DRCAROLL || race == RACE_ROBOT || race == RACE_EYESPY) {
					g_ChrBloodDebugSkipRace++;
				} else if (ismelee) {
					g_ChrBloodDebugSkipMelee++;
				} else if (shotdata->gset.weaponnum == WEAPON_TRANQUILIZER) {
					g_ChrBloodDebugSkipTranq++;
				} else {
					u8 darker;
					...
					if (!chrIsUsingPaintball(g_Vars.currentplayer->prop->chr)) {
						mtx4RotateVec(&spb0, &shotdata->gundir3d, &normal);
						normal.x = -normal.x;
						normal.y = -normal.y;
						normal.z = -normal.z;
						chrBruise(hit->model, hit->hitpart, hit->bboxnode, &sp5c);
						mdMaterialImpactPlayBodyHit(chr && chr->aibot);
						g_ChrBloodDebugRequests++;

						if (chrBloodSprayCreate(prop, hit->model, hit->bboxnode, &sp5c)) {
							g_ChrBloodDebugSpawned++;
						}
					} else {
						g_ChrBloodDebugSkipPaintball++;
		...
		struct modelnode *node, s32 side, s32 arg6, s32 arg7, s32 alpha)
{
	...
	if (side == -8) {
		bloodgap += 18.0f / model->scale;
	}

	mtxindex = modelFindNodeMtxIndex(node, 0);
	modelmtx = &model->matrices[mtxindex];

	xmin = bbox->xmin - bloodgap;
	xmax = bbox->xmax + bloodgap;
	ymin = bbox->ymin - bloodgap;
	ymax = bbox->ymax + bloodgap;
	zmin = bbox->zmin - bloodgap;
	zmax = bbox->zmax + bloodgap;
	...
	if (side == -9) {
		s32 age = g_Vars.lvframe60 - hit->lvframe60;
		s32 frameindex = chrBloodGetFrameIndex(age);
		struct textureconfig *bloodtex;
		s16 bloods;
		s16 bloodt;
		...
		s16 hity = hit->unk014;
		s16 hitz = hit->unk016;
		s16 halfsize = 72.0f / model->scale;
		s16 offset = 12.0f / model->scale;
		s16 vx[4];
		s16 vy[4];
		s16 vz[4];
		s32 axis = 0;
		...
		bloodtex = &g_TexWallhitConfigs[(useside ? 23 : 18) + frameindex];
		bloods = bloodtex->width * 32;
		bloodt = bloodtex->height * 32;

		texSelect(&gdl, bloodtex, 4, 1, 2, true, NULL);
		gDPSetRenderMode(gdl++, G_RM_AA_ZB_XLU_SURF, G_RM_AA_ZB_XLU_SURF2);
		gDPSetCombineMode(gdl++, G_CC_DECALRGBA, G_CC_DECALRGBA);
		...
		colours[0].g = 0xff;
		colours[0].b = 0xff;
		colours[0].a = chrBloodGetFadeAlpha(age);

		bestdist = ABS(hitx - bbox->xmin);

		if (ABS(hitx - bbox->xmax) < bestdist) {
			bestdist = ABS(hitx - bbox->xmax);
			axis = 0;
			usemax = true;
		}

		if (ABS(hity - bbox->ymin) < bestdist) {
			bestdist = ABS(hity - bbox->ymin);
			axis = 1;
			usemax = false;
		}

		if (ABS(hity - bbox->ymax) < bestdist) {
			bestdist = ABS(hity - bbox->ymax);
			axis = 1;
			usemax = true;
		}

		if (ABS(hitz - bbox->zmin) < bestdist) {
			bestdist = ABS(hitz - bbox->zmin);
			axis = 2;
			usemax = false;
		}

		if (ABS(hitz - bbox->zmax) < bestdist) {
			axis = 2;
			usemax = true;
		}

		if (!useside) {
			struct coord localpos;
			struct coord campos;
			Mtxf ident;
			Mtxf *billboardmtx;
			s16 billboardhalf = 4.0f / model->scale;
			s16 billboardoffset = 10.0f / model->scale;

			if (billboardhalf < 10) {
				billboardhalf = 10;
			}

			if (billboardoffset < 4) {
				billboardoffset = 4;
			}
			...
			mtx4TransformVec(modelmtx, &localpos, &campos);

			campos.z += billboardoffset;

			mtx4LoadIdentity(&ident);
			billboardmtx = gfxAllocateMatrix();
			mtxF2L(&ident, billboardmtx);
			gSPMatrix(gdl++, osVirtualToPhysical(billboardmtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

			vx[0] = chrDamageClampS16(campos.x - billboardhalf);
			vy[0] = chrDamageClampS16(campos.y + billboardhalf);
			vz[0] = chrDamageClampS16(campos.z);
			vx[1] = chrDamageClampS16(campos.x + billboardhalf);
			vy[1] = chrDamageClampS16(campos.y + billboardhalf);
			vz[1] = chrDamageClampS16(campos.z);
			vx[2] = chrDamageClampS16(campos.x + billboardhalf);
			vy[2] = chrDamageClampS16(campos.y - billboardhalf);
			vz[2] = chrDamageClampS16(campos.z);
			vx[3] = chrDamageClampS16(campos.x - billboardhalf);
			vy[3] = chrDamageClampS16(campos.y - billboardhalf);
			vz[3] = chrDamageClampS16(campos.z);
		} else if (useside) {
			struct coord localpos;
			struct coord campos;
			Mtxf ident;
			Mtxf *billboardmtx;
			s16 billboardhalf = 4.0f / model->scale;
			s16 billboarddepth = 10.0f / model->scale;
			s16 billboardwidth;
			...
			billboardwidth = billboardhalf * 2;
			...
			/*
			 * Side spray uses the same front-facing billboard sizing as the
			 * front burst, but is pushed behind the chr so it reads more like
			 * an exit spray.
			 */
			campos.z -= billboarddepth;
			...
			/*
			 * Side art is anchored at x=0%, y=50%, so pin the left edge to the
			 * hit point and let the burst extend rightward from there.
			 */
			vx[0] = chrDamageClampS16(campos.x);
			vy[0] = chrDamageClampS16(campos.y + billboardhalf);
			vz[0] = chrDamageClampS16(campos.z);
			vx[1] = chrDamageClampS16(campos.x + billboardwidth);
			vy[1] = chrDamageClampS16(campos.y + billboardhalf);
			vz[1] = chrDamageClampS16(campos.z);
			vx[2] = chrDamageClampS16(campos.x + billboardwidth);
			vy[2] = chrDamageClampS16(campos.y - billboardhalf);
			vz[2] = chrDamageClampS16(campos.z);
			vx[3] = chrDamageClampS16(campos.x);
			vy[3] = chrDamageClampS16(campos.y - billboardhalf);
			vz[3] = chrDamageClampS16(campos.z);
		} else if (axis == 0) {
			s16 face = usemax ? bbox->xmax + offset : bbox->xmin - offset;
			vx[0] = face;
			vx[1] = face;
			vx[2] = face;
			vx[3] = face;
			vy[0] = chrDamageClampS16(hity + halfsize);
			vy[1] = chrDamageClampS16(hity + halfsize);
			vy[2] = chrDamageClampS16(hity - halfsize);
			vy[3] = chrDamageClampS16(hity - halfsize);
			vz[0] = chrDamageClampS16(hitz - halfsize);
			vz[1] = chrDamageClampS16(hitz + halfsize);
			vz[2] = chrDamageClampS16(hitz + halfsize);
			vz[3] = chrDamageClampS16(hitz - halfsize);
		} else if (axis == 1) {
			s16 face = usemax ? bbox->ymax + offset : bbox->ymin - offset;
			vy[0] = face;
			vy[1] = face;
			vy[2] = face;
			vy[3] = face;
			vx[0] = chrDamageClampS16(hitx - halfsize);
			vx[1] = chrDamageClampS16(hitx + halfsize);
			vx[2] = chrDamageClampS16(hitx + halfsize);
			vx[3] = chrDamageClampS16(hitx - halfsize);
			vz[0] = chrDamageClampS16(hitz - halfsize);
			vz[1] = chrDamageClampS16(hitz - halfsize);
			vz[2] = chrDamageClampS16(hitz + halfsize);
			vz[3] = chrDamageClampS16(hitz + halfsize);
		} else {
			s16 face = usemax ? bbox->zmax + offset : bbox->zmin - offset;
			vy[0] = chrDamageClampS16(hity + halfsize);
			vy[1] = chrDamageClampS16(hity + halfsize);
			vy[2] = chrDamageClampS16(hity - halfsize);
			vy[3] = chrDamageClampS16(hity - halfsize);
		...
		colours[0].g = 0xff;
		colours[0].b = 0xff;
		colours[0].a = age < TICKS(10) ? (TICKS(10) - age) * 25 : 0;
...
void shieldhitsTick(void)
{
	...
	chrBloodMarkersTick();
	chrBloodHitsTick();
	chrBloodDebugMaybeReport();

	if (g_ShieldHitActive) {
		for (i = 0; i < 20; i++) {
			if (g_ShieldHits[i].prop) {
				if (g_ShieldHits[i].side == -8) {
					if (g_ShieldHits[i].lvframe60 >= g_Vars.lvframe60 - TICKS(10)) {
						changed = true;
					}
				} else if (g_ShieldHits[i].lvframe60 >= g_Vars.lvframe60 - TICKS(80)) {
					changed = true;
