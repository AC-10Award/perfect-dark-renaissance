...
static u16 g_TexExternalLookupSourceFileId;
#endif

#ifndef PLATFORM_N64
static bool texIsTerminationBodyHandTexture(s32 texturenum)
{
	texturenum &= 0xffff;
...
static s32 texApplyCharacterGdlTextureRedirect(s32 texturenum, u16 sourcefileid)
{
	s32 original = texturenum & 0xffff;
	s32 redirected = original;

	if (sourcefileid == FILE_CBOILERTREVZ) {
		if (original == 0x030a) {
			redirected = 0x0c7a;
		} else if (original == 0x030d) {
			redirected = 0x0c5a;
		} else if (original == 0x030c || original == 0x030b) {
			redirected = 0x0c7e;
		}
	}

	if (sourcefileid == FILE_CEVILCYBORGZ && texIsTerminationBodyHandTexture(texturenum)) {
		redirected = 0x0c78;
	}
	...
	if (redirected != original) {
		sysLogPrintf(
				LOG_NOTE,
				"character-tex: remap gdl model=%04x tex=%04x -> %04x",
				sourcefileid,
				(u32)original,
				(u32)redirected);
	}
...
static bool texShouldUseRobotSkelingtonAlpha(s32 texturenum, u16 sourcefileid)
{
	if (sourcefileid != FILE_CBOILERTREVZ) {
		return false;
	}

	switch (texturenum & 0xffff) {
	case 0x0c5a:
	case 0x0c7a:
	case 0x0c7e:
	case 0x0dd4:
...
static s32 texGetExternalLookupTextureNum(s32 texturenum)
{
	texturenum &= 0xffff;

	if (g_TexExternalLookupSourceFileId == FILE_GTESTGUN) {
		switch (texturenum) {
		case 0x0390: return 0x1b00;
		case 0x0391: return 0x1b01;
		case 0x0392: return 0x1b02;
		case 0x0394: return 0x1b03;
		case 0x0395: return 0x1b04;
		case 0x0396: return 0x1b05;
		case 0x0397: return 0x1b06;
		case 0x0878: return 0x1b07;
		}
	}
...
Gfx *texWriteLoadToTmemAddr(Gfx *gdl, struct tex *tex, s32 tmemoffset)
{
	...
	gDPSetTextureInfoEXT(gdl++, G_TEXTYPE_GENERAL, 0, texGetExternalLookupTextureNum(tex->texturenum), 0);
 ...
 */
s32 texLoadFromGdl(Gfx *instart, s32 gdlsizeinbytes, Gfx *outstart, struct texpool *pool, u8 *vtxstart, u16 sourcefileid)
{
	struct tex *tex1;
	struct tex *tex2;
	Gfx *texcmd;
...
#ifndef PLATFORM_N64
	s32 endoalphamode;
	u16 previouslookupsource = g_TexExternalLookupSourceFileId;
	g_TexExternalLookupSourceFileId = sourcefileid;
#endif
	...
	while (numcmdsremaining > 0) {
		...
		case G_NOOP: // 0xc0, repurposed
...
#ifndef PLATFORM_N64
			texturenum = texApplyCharacterGdlTextureRedirect(texturenum, sourcefileid);
#endif
			...
			if (tex1 != NULL) {
#ifndef PLATFORM_N64
				bool useendoalpha = texShouldUseRobotSkelingtonAlpha(texturenum, sourcefileid);
#endif
...
#ifndef PLATFORM_N64
					texturenum2 = texApplyCharacterGdlTextureRedirect(texturenum2, sourcefileid);
					useendoalpha = useendoalpha || texShouldUseRobotSkelingtonAlpha(texturenum2, sourcefileid);
#endif
...
#ifndef PLATFORM_N64
				if (sourcefileid == FILE_CBOILERTREVZ && endoalphamode != useendoalpha) {
					texSetRenderMode(&outgdl, useendoalpha ? 3 : 1, 1, 1);
					endoalphamode = useendoalpha;
				}
#endif
...
#ifndef PLATFORM_N64
	g_TexExternalLookupSourceFileId = previouslookupsource;
#endif
