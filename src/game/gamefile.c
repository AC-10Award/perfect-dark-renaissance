...
void gamefileApplyOptions(struct gamefile *file)
{
	...
	optionsSetMpFootsteps(pakHasBitflag(GAMEFILEFLAG_MPFOOTSTEPS, file->flags));
...
void gamefileLoadDefaults(struct gamefile *file)
{
	...
	pakSetBitflag(GAMEFILEFLAG_MPFOOTSTEPS, file->flags, false);
	...
	for (i = 0; i < ARRAYCOUNT(g_GameFile.lmscompletions); i++) {
		g_GameFile.lmscompletions[i] = 0;
	}
...
s32 gamefileLoad(s32 device)
{
	...
	if (device >= 0) {
		...
		if (ret == 0) {
			...
			for (i = 0; i < ARRAYCOUNT(g_GameFile.lmscompletions); i++) {
				g_GameFile.lmscompletions[i] = savebufferReadBits(&buffer, 3);
			}
...
s32 gamefileSave(s32 device, s32 fileid, u16 deviceserial)
{
	...
	pakSetBitflag(GAMEFILEFLAG_MPFOOTSTEPS, g_GameFile.flags, optionsGetMpFootsteps());
	...
	if (device >= 0) {
		...
		for (i = 0; i < ARRAYCOUNT(g_GameFile.lmscompletions); i++) {
			savebufferOr(&buffer, g_GameFile.lmscompletions[i] & 7, 3);
		}
...
{
	if (stageindex < 0 || stageindex >= ARRAYCOUNT(g_GameFile.lmscompletions)) {
		return 0;
	}

	return g_GameFile.lmscompletions[stageindex] & 7;
}
...
bool gamefileIsLmsDifficultyComplete(s32 stageindex, s32 difficultyindex)
{
	...
	return (gamefileGetLmsCompletion(stageindex) & (1 << difficultyindex)) != 0;
}
...
void gamefileSetLmsDifficultyComplete(s32 stageindex, s32 difficultyindex, bool completed)
{
	...
	mask = 1 << difficultyindex;

	if (completed) {
		g_GameFile.lmscompletions[stageindex] |= mask;
	} else {
		g_GameFile.lmscompletions[stageindex] &= ~mask;
	}
}
...
{
	gamefileSetFlag(GAMEFILEFLAG_MP_ONLY_UNLOCKS);
	challengeDetermineUnlockedFeatures();
}
