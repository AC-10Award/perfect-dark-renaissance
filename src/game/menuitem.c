...
static s32 menuitemListGetGroupCount(struct menuitem *item)
{
	union handlerdata data;
...
static s32 menuitemListGetGroupStartIndex(struct menuitem *item, s32 groupindex)
{
	union handlerdata data;

	data.list.value = groupindex;
	data.list.unk04 = 0;
	data.list.groupstartindex = 0;
	data.list.unk0c = 0;
	item->handler(MENUOP_GETGROUPSTARTINDEX, item, &data);

	return data.list.groupstartindex;
}

static s32 menuitemListFindGroupIndex(struct menuitem *item, s32 optionindex)
{
	s32 groupcount = menuitemListGetGroupCount(item);
	s32 i;
	...
	for (i = 0; i < groupcount; i++) {
		s32 start = menuitemListGetGroupStartIndex(item, i);
		s32 nextstart = (i + 1 < groupcount) ? menuitemListGetGroupStartIndex(item, i + 1) : 0x7fffffff;

		if (optionindex >= start && optionindex < nextstart) {
			return i;
		}
	}
...
{
	f32 f0;
	bool silentpreview = (item->flags & MENUITEMFLAG_SILENT_PREVIEW) != 0;
	bool preserveleftright = item->handler == menuhandlerInventoryList
		&& (g_MenuData.root == MENUROOT_MPPAUSE || g_MenuData.root == MENUROOT_MPENDSCREEN);
	bool consumedleftright = false;
	...
	if (tickflags & MENUTICKFLAG_ITEMISFOCUSED) {
		...
		if (handlerdata.list.value) {
			...
			if (item->handler == mpArenaMenuHandler && inputs->leftright) {
				s32 groupcount = menuitemListGetGroupCount(item);

				if (groupcount > 0) {
					s32 currentgroup = menuitemListFindGroupIndex(item, data->list.index);
					s32 nextgroup = currentgroup + inputs->leftright;
					...
					data->list.index = menuitemListGetGroupStartIndex(item, nextgroup);
					...
					inputs->leftright = false;
					consumedleftright = true;
				}
			}

#ifdef PLATFORM_N64
			if (inputs->updown || (!preserveleftright && (item->flags & MENUITEMFLAG_00010000) && inputs->leftright)) {
				prev2 = data->list.index;
				data->list.index += inputs->updown;
#else
			if (inputs->updown || inputs->mousescroll || (!preserveleftright && (item->flags & MENUITEMFLAG_00010000) && inputs->leftright)) {
				prev2 = data->list.index;
				data->list.index += inputs->updown + inputs->mousescroll;
#endif

				if (!preserveleftright && (item->flags & MENUITEMFLAG_00010000) && inputs->leftright) {
					data->list.index += inputs->leftright * 10;
					consumedleftright = true;
				}

				if (data->list.index < 0) {
					while (data->list.index < 0) {
			...
			inputs->updown = false;
			if (!preserveleftright && consumedleftright) {
				inputs->leftright = false;
			}
		}
	}
...
bool menuitemDropdownTick(struct menuitem *item, struct menudialog *dialog, struct menuinputs *inputs, u32 tickflags, union menuitemdata *data)
{
	...
	if ((tickflags & MENUTICKFLAG_ITEMISFOCUSED) && item->handler) {
		if (!dialog->dimmed && (item->flags & MENUITEMFLAG_00010000) && inputs->leftright) {
			union handlerdata handlerdata;
			s32 numoptions;
			s32 index;
			...
			if (numoptions > 0) {
				item->handler(MENUOP_GETSELECTEDINDEX, item, &handlerdata);
				index = handlerdata.dropdown.value + inputs->leftright;
				...
				handlerdata.dropdown.value = index;
				item->handler(MENUOP_SET, item, &handlerdata);
...
#if VERSION >= VERSION_NTSC_1_0
	if (context->dialog->definition == &g_MpReadyMenuDialog
			&& context->item->handler == menuhandlerMpStartAllPlayers) {
		// Use the stock selectable baseline now that this row has a larger slot.
		baselinedelta = 0;
	}
#endif
...
bool menuitemCarouselTick(struct menuitem *item, struct menuinputs *inputs, u32 tickflags)
{
	...
	if (((tickflags & MENUTICKFLAG_ITEMISFOCUSED) || (item->flags & MENUITEMFLAG_CAROUSEL_04000000)) && item->handler) {
		...
		if ((item->flags & MENUITEMFLAG_CAROUSEL_SELECTCONFIRM) && inputs->select) {
			item->handler(MENUOP_GETSELECTEDINDEX, item, &data);
			data.carousel.unk04 = 2;
			item->handler(MENUOP_SET, item, &data);
			menuPlaySound(MENUSOUND_SELECT);
		}
