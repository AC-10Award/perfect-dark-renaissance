...
struct chrdata *g_MpBotChrPtrs[MAX_BOTS_RUNTIME];

u8 g_BotCount = 0;

static f32 mpBotNearestHumanSpawnSqDist(struct coord *pos)
{
	f32 bestsqdist = -1.0f;
	s32 i;
	...
	for (i = 0; i < playercount; i++) {
		if (g_Vars.players[i] && g_Vars.players[i]->prop) {
			f32 xdiff = g_Vars.players[i]->prop->pos.x - pos->x;
			f32 ydiff = g_Vars.players[i]->prop->pos.y - pos->y;
			f32 zdiff = g_Vars.players[i]->prop->pos.z - pos->z;
			f32 sqdist = xdiff * xdiff + ydiff * ydiff + zdiff * zdiff;

			if (bestsqdist < 0.0f || sqdist < bestsqdist) {
				bestsqdist = sqdist;
			}
		}
	}
...
static f32 mpBotNearestBotSpawnSqDist(struct coord *pos, struct prop *selfprop)
{
	f32 bestsqdist = -1.0f;
	s32 i;
	...
	for (i = 0; i < g_BotCount; i++) {
		if (g_MpBotChrPtrs[i]
				&& g_MpBotChrPtrs[i]->prop
				&& g_MpBotChrPtrs[i]->prop != selfprop
				&& g_MpBotChrPtrs[i]->aibot
				&& !g_MpBotChrPtrs[i]->aibot->respawning) {
			f32 xdiff = g_MpBotChrPtrs[i]->prop->pos.x - pos->x;
			f32 ydiff = g_MpBotChrPtrs[i]->prop->pos.y - pos->y;
			f32 zdiff = g_MpBotChrPtrs[i]->prop->pos.z - pos->z;
			f32 sqdist = xdiff * xdiff + ydiff * ydiff + zdiff * zdiff;
...
void botReset(struct chrdata *chr, u8 respawning)
{
	...
	if (aibot) {
		...
		if (respawning) {
			if (chr->model) {
				modelFreeVertices(VTXSTORETYPE_CHRVTX, chr->model);
			}
...
void botSpawn(struct chrdata *chr, u8 respawning)
{
	...
	if (respawning && g_Vars.normmplayerisrunning && mpIsDeathsLimitEnabled()) {
		s32 playerindex = mpPlayerGetIndex(chr);
		s32 chrslot = func0f18d074(playerindex);

		if (!mpHasRemainingLives(chrslot)) {
	...
	if (aibot) {
		...
		if (g_Vars.normmplayerisrunning
			...
			s16 preferredpads[24];
			s32 numpreferredpads = 0;
			s32 padindex;

			for (padindex = 0; padindex < g_NumSpawnPoints && numpreferredpads < ARRAYCOUNT(preferredpads); padindex++) {
				struct pad pad;
				f32 human_sqdist;

				padUnpack(g_SpawnPoints[padindex], PADFIELD_POS | PADFIELD_ROOM, &pad);
				human_sqdist = mpBotNearestHumanSpawnSqDist(&pad.pos);

				if (human_sqdist < 0.0f || human_sqdist >= 1400.0f * 1400.0f) {
					preferredpads[numpreferredpads++] = g_SpawnPoints[padindex];
				}
			}

			if (numpreferredpads > 0) {
				thing = playerChooseSpawnLocation(chr->radius, &pos, rooms, chr->prop, preferredpads, numpreferredpads);
			} else {
				thing = scenarioChooseSpawnLocation(chr->radius, &pos, rooms, chr->prop);
			}
		} else if (g_Vars.normmplayerisrunning && !respawning && PLAYERCOUNT() > 0) {
			struct coord bestpos = {0, 0, 0};
			RoomNum bestrooms[8];
			f32 bestangle = 0;
			f32 bestscore = -1.0f;
			s32 attempt;
			...
			for (attempt = 0; attempt < 16; attempt++) {
				struct coord attemptpos = {0, 0, 0};
				RoomNum attemptrooms[8];
				f32 attemptangle = scenarioChooseSpawnLocation(chr->radius, &attemptpos, attemptrooms, chr->prop);
				f32 human_sqdist = mpBotNearestHumanSpawnSqDist(&attemptpos);
				f32 bot_sqdist = mpBotNearestBotSpawnSqDist(&attemptpos, chr->prop);
				f32 attemptscore = human_sqdist;

				if (human_sqdist < 1800.0f * 1800.0f) {
					attemptscore *= 0.25f;
				}

				if (bot_sqdist >= 0.0f) {
					attemptscore += bot_sqdist * 0.5f;
				}

				if (attempt == 0 || attemptscore > bestscore) {
					bestpos = attemptpos;
					roomsCopy(attemptrooms, bestrooms);
					bestangle = attemptangle;
					bestscore = attemptscore;
				}
			}

			pos = bestpos;
			roomsCopy(bestrooms, rooms);
			thing = bestangle;
		} else {
			thing = scenarioChooseSpawnLocation(chr->radius, &pos, rooms, chr->prop);
		}
...
static bool botIsTerminationScenarioBoss(struct chrdata *chr)
{
	return g_Vars.normmplayerisrunning
		&& g_MpSetup.scenario == MPSCENARIO_TERMINATION
		&& chr
		&& chr->aibot
		&& chr->aibot->aibotnum == 0;
}

static bool botIsTerminationScenarioVictim(struct chrdata *chr)
{
	return g_Vars.normmplayerisrunning
		&& g_MpSetup.scenario == MPSCENARIO_TERMINATION
		&& chr
...
static bool botIsTerminationScenarioEndoBoss(struct chrdata *chr)
{
	return botIsTerminationScenarioBoss(chr)
		&& chr->bodynum == BODY_ROBOT_SKELINGTON;
}
...
void botChooseGeneralTarget(struct chrdata *botchr)
{
	...
	s32 i;
	s32 j;
		bool distancesdone[MAX_MPCHRS_RUNTIME];
	RoomNum room = -1;
	...
	if (g_MpSetup.scenario == MPSCENARIO_TERMINATION) {
		if (aibot->aibotnum == 0) {
			struct chrdata *closestvictim = NULL;
			bool closestisinsight = false;
			f32 closestdistance = 0;
			struct chrdata *currentvictim = NULL;

			if (botchr->target >= 0 && botchr->target < g_Vars.maxprops) {
				struct prop *targetprop = &g_Vars.props[botchr->target];

				if (targetprop->type == PROPTYPE_CHR && botIsTerminationScenarioVictim(targetprop->chr)) {
					currentvictim = targetprop->chr;
					closestvictim = currentvictim;
					closestisinsight = aibot->targetinsight != 0;
					closestdistance = chrGetDistanceToCoord(botchr, &currentvictim->prop->pos);
				}
			}

			for (i = 0; i < g_BotCount; i++) {
				struct chrdata *victim = g_MpBotChrPtrs[i];

				if (victim != botchr && botIsTerminationScenarioVictim(victim)) {
					s32 victimplayernum = mpPlayerGetIndex(victim);

					if (victimplayernum >= 0 && victimplayernum < g_MpNumChrs) {
						bool victimisinsight = aibot->chrsinsight[victimplayernum] != 0;
						f32 victimdistance = aibot->chrdistances[victimplayernum];

						if (victim == currentvictim) {
							closestisinsight = victimisinsight;
							closestdistance = victimdistance;
						} else if (!closestvictim
								|| (!closestisinsight && victimisinsight)
								|| (!currentvictim && victimisinsight == closestisinsight && victimdistance < closestdistance)) {
			...
			if (closestvictim) {
				s32 victimpropnum = closestvictim->prop - g_Vars.props;

				botSetTarget(botchr, victimpropnum);
				aibot->command = AIBOTCMD_ATTACK;
				aibot->attackpropnum = victimpropnum;
				aibot->attackingplayernum = mpPlayerGetIndex(closestvictim);
...
void botTickUnpaused(struct chrdata *chr)
{
	...
	if (!chrIsDead(chr)) {
		...
		if (chr->myaction == MA_AIBOTMAINLOOP || aibot->forcemainloop) {
			...
			if (newaction < 0 && !botIsTerminationScenarioBoss(chr)) {
			...
			if (newaction < 0) {
				if (aibot->command == AIBOTCMD_ATTACK) {
					...
					if (aibot->attackpropnum >= 0
							&& aibot->attackpropnum < g_Vars.maxprops
							&& (g_Vars.props + aibot->attackpropnum)->type != 0) {
						targetchr = (g_Vars.props + aibot->attackpropnum)->chr;
					}
		...
		if (chr->myaction == MA_AIBOTGETITEM) {
			...
			if (aibot->attackingplayernum >= 0
				...
				if (!botIsTerminationScenarioBoss(chr) && botCanDoCriticalPickup(chr)) {
					chr->myaction = MA_AIBOTMAINLOOP;
			...
			for (i = 0; i < 2; i++) {
				...
				if (aibot->skrocket == NULL && aibot->changeguntimer60 <= 0) {
					if (aibot->ismeleeweapon) {
						...
						bool canshoot = false;
						bool isterminationendo = botIsTerminationScenarioEndoBoss(chr);
						...
						if (canshoot) {
							if (aibot->cyclonedischarging[i] || aibot->burstsdone[i] > 0) {
								firing = true;
									...
									&& aibot->targetinsight
									&& (isterminationendo || aibot->shootdelaytimer60 >= g_BotDifficulties[aibot->config->difficulty].shootdelay)
									&& (isterminationendo || botIsDizzy(chr) || chrIsTargetInFov(chr, 45, false))
									&& !chrIsDead(chrGetTargetProp(chr)->chr)) {
								firing = true;
