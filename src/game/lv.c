...
static bool lvIsCrashAuditStage(s32 stagenum)
{
	return stagenum == STAGE_CRASHSITE
		|| stagenum == STAGE_PELAGIC
		|| stagenum == STAGE_VILLA
		|| stagenum == STAGE_MP_AREA52;
}

#ifndef PLATFORM_N64
static void lvCrashAuditLog(const char *format, ...)
{
...
void lvReset(s32 stagenum)
{
	...
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: lvReset pre-setupLoadFiles stagenum=%02x scenario=%d players=%d stageindex=%d\n",
				stagenum, g_MpSetup.scenario, PLAYERCOUNT(), g_StageIndex);
	}

	setupLoadFiles(stagenum);

	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: lvReset post-setupLoadFiles stagenum=%02x\n", stagenum);
	}
	...
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: lvReset pre-setupCreateProps stagenum=%02x\n", stagenum);
	}

	setupCreateProps(stagenum);

	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: lvReset post-setupCreateProps stagenum=%02x\n", stagenum);
	}

	tagsReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: tagsReset done stagenum=%02x\n", stagenum);
	}
	explosionsReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: explosionsReset done stagenum=%02x\n", stagenum);
	}
	smokeReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: smokeReset done stagenum=%02x\n", stagenum);
	}
	sparksReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: sparksReset done stagenum=%02x\n", stagenum);
	}
	weatherReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: weatherReset done stagenum=%02x\n", stagenum);
	}
	lvResetMiscSfx();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: lvResetMiscSfx done stagenum=%02x\n", stagenum);
	}
	...
	func0f0099a4();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: func0f0099a4 done stagenum=%02x\n", stagenum);
	}
	boltbeamsReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: boltbeamsReset done stagenum=%02x\n", stagenum);
	}
	lasersightsReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: lasersightsReset done stagenum=%02x\n", stagenum);
	}
	stub0f013540();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: stub0f013540 done stagenum=%02x\n", stagenum);
	}
	shardsReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: shardsReset done stagenum=%02x\n", stagenum);
	}
	frReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: frReset done stagenum=%02x\n", stagenum);
	}
	...
	if (g_Vars.stagenum == STAGE_TITLE) {
		...
		utilsReset();
		if (lvIsCrashAuditStage(stagenum)) {
			lvCrashAuditLog("map-crash: utilsReset done stagenum=%02x\n", stagenum);
		}
		casingsReset();
		if (lvIsCrashAuditStage(stagenum)) {
			lvCrashAuditLog("map-crash: casingsReset done stagenum=%02x\n", stagenum);
		}

		for (i = 0; i < PLAYERCOUNT(); i++) {
			setCurrentPlayerNum(i);
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: player loop begin stagenum=%02x player=%d\n", stagenum, i);
			}
			...
			menuReset();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: menuReset done stagenum=%02x player=%d\n", stagenum, i);
			}
			amReset();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: amReset done stagenum=%02x player=%d\n", stagenum, i);
			}
			invReset();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: invReset done stagenum=%02x player=%d\n", stagenum, i);
			}
			bgunReset();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: bgunReset done stagenum=%02x player=%d\n", stagenum, i);
			}
			playerLoadDefaults();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: playerLoadDefaults done stagenum=%02x player=%d\n", stagenum, i);
			}
			playerReset();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: playerReset done stagenum=%02x player=%d\n", stagenum, i);
			}
			playerSpawn();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: playerSpawn done stagenum=%02x player=%d\n", stagenum, i);
			}
			bheadReset();
			if (lvIsCrashAuditStage(stagenum)) {
				lvCrashAuditLog("map-crash: bheadReset done stagenum=%02x player=%d\n", stagenum, i);
			}

			if (g_Vars.normmplayerisrunning && (g_MpSetup.options & MPOPTION_TEAMSENABLED)) {
				playermgrCalculateAiBuddyNums();
				if (lvIsCrashAuditStage(stagenum)) {
					lvCrashAuditLog("map-crash: playermgrCalculateAiBuddyNums done stagenum=%02x player=%d\n", stagenum, i);
				}
			}
		}
		...
		acousticReset();
		if (lvIsCrashAuditStage(stagenum)) {
			lvCrashAuditLog("map-crash: acousticReset done stagenum=%02x\n", stagenum);
		}
		portalsReset();
		if (lvIsCrashAuditStage(stagenum)) {
			lvCrashAuditLog("map-crash: portalsReset done stagenum=%02x\n", stagenum);
		}
		lightsReset();
		if (lvIsCrashAuditStage(stagenum)) {
			lvCrashAuditLog("map-crash: lightsReset done stagenum=%02x\n", stagenum);
		}
		setCurrentPlayerNum(0);
	}
	...
	paksReset();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: paksReset done stagenum=%02x\n", stagenum);
	}
	sndResetCurMp3();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: sndResetCurMp3 done stagenum=%02x\n", stagenum);
	}
	...
	if (IS8MB()) {
		pheadReset();
		if (lvIsCrashAuditStage(stagenum)) {
			lvCrashAuditLog("map-crash: pheadReset done stagenum=%02x\n", stagenum);
		}
	}

	modelmgrSetLvResetting(false);
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: modelmgrSetLvResetting false stagenum=%02x\n", stagenum);
	}
	var80084018 = 1;
	schedResetArtifacts();
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: schedResetArtifacts done stagenum=%02x\n", stagenum);
	}
	lvSetPaused(0);
	if (lvIsCrashAuditStage(stagenum)) {
		lvCrashAuditLog("map-crash: lvSetPaused done stagenum=%02x\n", stagenum);
	}
