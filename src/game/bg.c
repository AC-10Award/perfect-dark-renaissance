...
#define BGGEODAMAGE_MAX_TRIS 512
#define BGGEODAMAGE_SOURCE_TEXNUM 0x0084
#define BGGEODAMAGE_DAMAGED_TEXNUM 0x0c5b
#define BGGEODAMAGE_DAMAGED_CONFIG_INDEX 43
#define BGGEODAMAGE_SURFACE_LIFT 1.5f
#define BGGEODAMAGE_TILE_ST_SIZE 1024.0f
#define BGGEODAMAGE_SOLVE_EPSILON 0.01f
#define BGGEODAMAGE_MAX_PUFFS 64
#define BGGEODAMAGE_PUFF_NUM_FRAMES 8
#define BGGEODAMAGE_PUFF_FRAME_TICKS 2
#define BGGEODAMAGE_PUFF_TEX_WIDTH 80
#define BGGEODAMAGE_PUFF_TEX_HEIGHT 48
#define BGGEODAMAGE_PUFF_TEX_S0 (24 << 5)
#define BGGEODAMAGE_PUFF_TEX_S1 (68 << 5)
#define BGGEODAMAGE_PUFF_TEX_T0 (26 << 5)
#define BGGEODAMAGE_PUFF_TEX_T1 (48 << 5)
#define BGGEODAMAGE_PUFF_HALF_WIDTH 176.0f
#define BGGEODAMAGE_PUFF_HEIGHT 192.0f
#define BGGEODAMAGE_PUFF_RIGHT_SHIFT 64.0f
#define BGMETALHIT_MAX_SPARKS 96
#define BGMETALHIT_SPARK_NUM_VARIANTS 1
#define BGMETALHIT_SPARK_NUM_FRAMES 4
#define BGMETALHIT_SPARK_FRAME_TICKS 2
#define BGMETALHIT_SPARK_TEX_WIDTH BGGEODAMAGE_PUFF_TEX_WIDTH
#define BGMETALHIT_SPARK_TEX_HEIGHT BGGEODAMAGE_PUFF_TEX_HEIGHT
#define BGMETALHIT_SPARK_TEX_S0 BGGEODAMAGE_PUFF_TEX_S0
#define BGMETALHIT_SPARK_TEX_S1 BGGEODAMAGE_PUFF_TEX_S1
#define BGMETALHIT_SPARK_TEX_T0 BGGEODAMAGE_PUFF_TEX_T0
#define BGMETALHIT_SPARK_TEX_T1 BGGEODAMAGE_PUFF_TEX_T1
#define BGMETALHIT_SPARK_HALF_WIDTH 132.0f
#define BGMETALHIT_SPARK_HEIGHT 144.0f
#define BGMETALHIT_SPARK_RIGHT_SHIFT 36.0f
#define BGMETALHIT_SPARK_DOWN_SHIFT 16.0f
#define BGWALLHIT_TEXNUM 0x0088
#define BGWALLHIT_MAX_HITS 96
#define BGWALLHIT_NUM_FRAMES 8
#define BGWALLHIT_FRAME_TICKS BGGEODAMAGE_PUFF_FRAME_TICKS
#define BGWALLHIT_TEX_WIDTH BGGEODAMAGE_PUFF_TEX_WIDTH
#define BGWALLHIT_TEX_HEIGHT BGGEODAMAGE_PUFF_TEX_HEIGHT
#define BGWALLHIT_TEX_S0 0
#define BGWALLHIT_TEX_S1 (BGWALLHIT_TEX_WIDTH << 5)
#define BGWALLHIT_TEX_T0 BGGEODAMAGE_PUFF_TEX_T0
#define BGWALLHIT_TEX_T1 BGGEODAMAGE_PUFF_TEX_T1
#define BGWALLHIT_CONFIG_TEX_WIDTH BGWALLHIT_TEX_WIDTH
#define BGWALLHIT_CONFIG_TEX_HEIGHT BGWALLHIT_TEX_HEIGHT
#define BGWALLHIT_SURFACE_LIFT 24.0f
#define BGWALLHIT_FRONT_HALF_WIDTH 55.0f
#define BGWALLHIT_FRONT_HALF_HEIGHT 33.0f
#define BGWALLHIT_SIDE_HALF_WIDTH 55.0f
#define BGWALLHIT_SIDE_HALF_HEIGHT 33.0f
#define BGWALLHIT_SIDE_CROSS_TANGENT 0.55f

struct bggeodamagetri {
	RoomNum roomnum;
	Vtx vertices[4];
	struct coord normal;
};

struct bggeodamagepuff {
	RoomNum roomnum;
	s32 startframe;
	struct coord pos;
};

struct bgmetalhitspark {
	RoomNum roomnum;
	s32 variant;
	s32 startframe;
	struct coord pos;
...
struct bggeodamagetri g_BgGeoDamageTris[BGGEODAMAGE_MAX_TRIS];
s32 g_BgGeoDamageTriCount = 0;
s32 g_BgGeoDamageTriNextIndex = 0;
struct bggeodamagepuff g_BgGeoDamagePuffs[BGGEODAMAGE_MAX_PUFFS];
s32 g_BgGeoDamagePuffCount = 0;
s32 g_BgGeoDamagePuffNextIndex = 0;
struct bgmetalhitspark g_BgMetalHitSparks[BGMETALHIT_MAX_SPARKS];
s32 g_BgMetalHitSparkCount = 0;
s32 g_BgMetalHitSparkNextIndex = 0;
struct bgwallhit g_BgWallHits[BGWALLHIT_MAX_HITS];
s32 g_BgWallHitCount = 0;
s32 g_BgWallHitNextIndex = 0;
struct textureconfig g_BgGeoDamageTextureConfig = {
	BGGEODAMAGE_DAMAGED_TEXNUM,
	32,
	32,
	5,
	G_IM_FMT_I,
...
struct textureconfig g_BgGeoDamagePuffTextureConfigs[BGGEODAMAGE_PUFF_NUM_FRAMES] = {
	{ 0x0c5d, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c5e, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c5f, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c6a, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c6b, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c6c, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c6d, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c6e, BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
};
u8 g_BgGeoDamagePuffTextureStub[16];
struct textureconfig g_BgMetalHitSparkTextureConfigs[BGMETALHIT_SPARK_NUM_VARIANTS][BGMETALHIT_SPARK_NUM_FRAMES] = {
	{
		{ 0x0c70, BGMETALHIT_SPARK_TEX_WIDTH, BGMETALHIT_SPARK_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
		{ 0x0c71, BGMETALHIT_SPARK_TEX_WIDTH, BGMETALHIT_SPARK_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
		{ 0x0c72, BGMETALHIT_SPARK_TEX_WIDTH, BGMETALHIT_SPARK_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
		{ 0x0c73, BGMETALHIT_SPARK_TEX_WIDTH, BGMETALHIT_SPARK_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	},
};
struct textureconfig g_BgWallHitFrontTextureConfigs[BGWALLHIT_NUM_FRAMES] = {
	{ 0x0c50, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c51, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c52, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c53, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c59, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c63, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c69, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0c74, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
};
struct textureconfig g_BgWallHitSideTextureConfigs[BGWALLHIT_NUM_FRAMES] = {
	{ 0x02ae, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x007a, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x007b, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x063d, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x063e, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0270, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0383, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
	{ 0x0384, BGWALLHIT_CONFIG_TEX_WIDTH, BGWALLHIT_CONFIG_TEX_HEIGHT, 0, G_IM_FMT_RGBA, G_IM_SIZ_32b, G_TX_CLAMP, G_TX_CLAMP, 0 },
};
#ifndef PLATFORM_N64
s32 g_BgGeoDamageDebugHitCount = 0;
s32 g_BgGeoDamageDebugRenderCount = 0;
s32 g_BgGeoDamageDebugPuffCount = 0;
#endif

static bool bgGeoDamageStageEnabled(void)
{
	return g_Vars.stagenum == STAGE_EXTRA15
		|| g_Vars.stagenum == STAGE_EXTRA4;
}

static void bgResetGeoDamageTextureConfigs(void)
{
	static u32 pufftexnums[BGGEODAMAGE_PUFF_NUM_FRAMES] = {
		0x0c5d, 0x0c5e, 0x0c5f, 0x0c6a, 0x0c6b, 0x0c6c, 0x0c6d, 0x0c6e,
	};
	static u32 metalhittexnums[BGMETALHIT_SPARK_NUM_VARIANTS][BGMETALHIT_SPARK_NUM_FRAMES] = {
		{ 0x0c70, 0x0c71, 0x0c72, 0x0c73 },
	};
	static u32 wallhitfronttexnums[BGWALLHIT_NUM_FRAMES] = {
		0x0c50, 0x0c51, 0x0c52, 0x0c53, 0x0c59, 0x0c63, 0x0c69, 0x0c74,
	};
	static u32 wallhitsidetexnums[BGWALLHIT_NUM_FRAMES] = {
		0x02ae, 0x007a, 0x007b, 0x063d, 0x063e, 0x0270, 0x0383, 0x0384,
	};
	s32 i;
	s32 j;

	g_BgGeoDamageTextureConfig.texturenum = BGGEODAMAGE_DAMAGED_TEXNUM;

	for (i = 0; i < BGGEODAMAGE_PUFF_NUM_FRAMES; i++) {
		g_BgGeoDamagePuffTextureConfigs[i].texturenum = pufftexnums[i];
	}

	for (i = 0; i < BGMETALHIT_SPARK_NUM_VARIANTS; i++) {
		for (j = 0; j < BGMETALHIT_SPARK_NUM_FRAMES; j++) {
			g_BgMetalHitSparkTextureConfigs[i][j].texturenum = metalhittexnums[i][j];
		}
	}

	for (i = 0; i < BGWALLHIT_NUM_FRAMES; i++) {
		g_BgWallHitFrontTextureConfigs[i].texturenum = wallhitfronttexnums[i];
		g_BgWallHitSideTextureConfigs[i].texturenum = wallhitsidetexnums[i];
	}
}

static bool bgGeoDamageVertexMatches(Vtx *a, Vtx *b)
{
	return a->x == b->x
		&& a->y == b->y
		&& a->z == b->z;
...
static f32 bgGeoDamageAbsf(f32 value)
{
	return value < 0.0f ? -value : value;
}

static f32 bgGeoDamageSnapStMin(f32 value)
{
	s32 cell = value >= 0.0f
		? (s32)(value / BGGEODAMAGE_TILE_ST_SIZE)
		: -1 - (s32)((-value - 0.001f) / BGGEODAMAGE_TILE_ST_SIZE);

	return cell * BGGEODAMAGE_TILE_ST_SIZE;
}

static void bgGeoDamageMakeTileVertex(Vtx *dst, Vtx *templatevtx, f32 x, f32 y, f32 z, f32 s, f32 t)
{
	*dst = *templatevtx;
	dst->x = (s16)x;
	dst->y = (s16)y;
...
static bool bgGeoDamageBuildTilePatch(s32 roomnum, struct hitthing *hitthing, Vtx vertices[4])
{
	Vtx *v0;
	Vtx *v1;
	Vtx *v2;
	...
	if (bgGeoDamageAbsf(hitthing->unk0c.y) <= bgGeoDamageAbsf(hitthing->unk0c.x)
			|| bgGeoDamageAbsf(hitthing->unk0c.y) <= bgGeoDamageAbsf(hitthing->unk0c.z)) {
		return false;
	}
	...
	detxz = dxx1 * dzz2 - dzz1 * dxx2;
	detst = dss1 * dtt2 - dtt1 * dss2;

	if (bgGeoDamageAbsf(detxz) < BGGEODAMAGE_SOLVE_EPSILON
			|| bgGeoDamageAbsf(detst) < BGGEODAMAGE_SOLVE_EPSILON) {
		return false;
	}

	hitx = hitthing->pos.x - g_BgRooms[roomnum].pos.x;
	hitz = hitthing->pos.z - g_BgRooms[roomnum].pos.z;
	hitu = ((hitx - x0) * dzz2 - (hitz - z0) * dxx2) / detxz;
	hitv = (dxx1 * (hitz - z0) - dzz1 * (hitx - x0)) / detxz;
	hits = s0 + hitu * dss1 + hitv * dss2;
	hitt = t0 + hitu * dtt1 + hitv * dtt2;
	mins = bgGeoDamageSnapStMin(hits);
	mint = bgGeoDamageSnapStMin(hitt);
	maxs = mins + BGGEODAMAGE_TILE_ST_SIZE;
	maxt = mint + BGGEODAMAGE_TILE_ST_SIZE;
	...
	for (i = 0; i < 4; i++) {
		f32 corneru = ((corners[i][0] - s0) * dtt2 - (corners[i][1] - t0) * dss2) / detst;
		f32 cornerv = (dss1 * (corners[i][1] - t0) - dtt1 * (corners[i][0] - s0)) / detst;

		bgGeoDamageMakeTileVertex(
			&vertices[i],
			v0,
			x0 + corneru * dxx1 + cornerv * dxx2,
			y0 + corneru * dyy1 + cornerv * dyy2,
			z0 + corneru * dzz1 + cornerv * dzz2,
			corners[i][0],
			corners[i][1]);
	}

	if (((vertices[1].y - vertices[0].y) * (vertices[2].z - vertices[0].z)
				- (vertices[1].z - vertices[0].z) * (vertices[2].y - vertices[0].y)) * hitthing->unk0c.x
			+ ((vertices[1].z - vertices[0].z) * (vertices[2].x - vertices[0].x)
				- (vertices[1].x - vertices[0].x) * (vertices[2].z - vertices[0].z)) * hitthing->unk0c.y
			+ ((vertices[1].x - vertices[0].x) * (vertices[2].y - vertices[0].y)
				- (vertices[1].y - vertices[0].y) * (vertices[2].x - vertices[0].x)) * hitthing->unk0c.z < 0.0f) {
		Vtx swap = vertices[1];
...
static void bgGeoDamageRandomizeTileUv(Vtx vertices[4])
{
	static const s16 stsets[4][4][2] = {
		{{0, 0}, {1024, 0}, {1024, 1024}, {0, 1024}},
		{{1024, 0}, {1024, 1024}, {0, 1024}, {0, 0}},
		{{1024, 1024}, {0, 1024}, {0, 0}, {1024, 0}},
	...
	for (i = 0; i < 4; i++) {
		s16 s = stsets[rotation][i][0];
		s16 t = stsets[rotation][i][1];
...
static void bgGeoDamageSpawnPuff(s32 roomnum, struct hitthing *hitthing)
{
	s32 index;
	...
	if (g_BgGeoDamagePuffCount < BGGEODAMAGE_MAX_PUFFS) {
		index = g_BgGeoDamagePuffCount;
		g_BgGeoDamagePuffCount++;
	} else {
		index = g_BgGeoDamagePuffNextIndex;
		g_BgGeoDamagePuffNextIndex = (g_BgGeoDamagePuffNextIndex + 1) % BGGEODAMAGE_MAX_PUFFS;
	}

	g_BgGeoDamagePuffs[index].roomnum = roomnum;
	g_BgGeoDamagePuffs[index].startframe = g_Vars.lvframenum;
	g_BgGeoDamagePuffs[index].pos.x = hitthing->pos.x - g_BgRooms[roomnum].pos.x;
	g_BgGeoDamagePuffs[index].pos.y = hitthing->pos.y - g_BgRooms[roomnum].pos.y + BGGEODAMAGE_SURFACE_LIFT;
	g_BgGeoDamagePuffs[index].pos.z = hitthing->pos.z - g_BgRooms[roomnum].pos.z;
}

static bool bgGeoDamageHitMatchesEntry(s32 roomnum, struct hitthing *hitthing, struct bggeodamagetri *entry)
{
	Vtx vertices[4];

	if (!bgGeoDamageBuildTilePatch(roomnum, hitthing, vertices)) {
		return false;
	}

	return entry->roomnum == roomnum
		&& bgGeoDamageVertexMatches(&entry->vertices[0], &vertices[0])
		&& bgGeoDamageVertexMatches(&entry->vertices[1], &vertices[1])
		&& bgGeoDamageVertexMatches(&entry->vertices[2], &vertices[2])
		&& bgGeoDamageVertexMatches(&entry->vertices[3], &vertices[3]);
}

void bgApplyGeoDamageTexture(s32 roomnum, struct hitthing *hitthing)
{
	s32 i;

	if (!bgGeoDamageStageEnabled() || hitthing == NULL) {
		return;
	}

	for (i = 0; i < g_BgGeoDamageTriCount; i++) {
		if (bgGeoDamageHitMatchesEntry(roomnum, hitthing, &g_BgGeoDamageTris[i])) {
			hitthing->texturenum = BGGEODAMAGE_DAMAGED_TEXNUM;
#ifndef PLATFORM_N64
			if (g_BgGeoDamageDebugHitCount < 32) {
				printf("stack-geodamage: apply room=%d tex=%04x tris=%d\n",
					roomnum, hitthing->texturenum, g_BgGeoDamageTriCount);
			}
#endif
			return;
		}
...
void bgRecordGeoDamageHit(s32 roomnum, struct hitthing *hitthing)
{
	s32 i;
	s32 index;
	Vtx vertices[4];

	if (!bgGeoDamageStageEnabled()
			|| hitthing == NULL
			|| hitthing->texturenum != BGGEODAMAGE_SOURCE_TEXNUM
			|| hitthing->unk18 == NULL
			|| hitthing->unk1c == NULL
			|| hitthing->unk20 == NULL) {
#ifndef PLATFORM_N64
		if (bgGeoDamageStageEnabled() && hitthing != NULL && g_BgGeoDamageDebugHitCount < 32) {
			printf("stack-geodamage: skip room=%d tex=%04x tri=%p/%p/%p count=%d\n",
				roomnum,
				hitthing->texturenum,
				hitthing->unk18,
				hitthing->unk1c,
				hitthing->unk20,
				g_BgGeoDamageTriCount);
			g_BgGeoDamageDebugHitCount++;
		}
#endif
		return;
	}
	...
	if (g_BgGeoDamageTriCount < BGGEODAMAGE_MAX_TRIS) {
		index = g_BgGeoDamageTriCount;
		g_BgGeoDamageTriCount++;
	} else {
		index = g_BgGeoDamageTriNextIndex;
		g_BgGeoDamageTriNextIndex = (g_BgGeoDamageTriNextIndex + 1) % BGGEODAMAGE_MAX_TRIS;
	}

	g_BgGeoDamageTris[index].roomnum = roomnum;
	bgGeoDamageRandomizeTileUv(vertices);
	g_BgGeoDamageTris[index].vertices[0] = vertices[0];
	g_BgGeoDamageTris[index].vertices[1] = vertices[1];
	g_BgGeoDamageTris[index].vertices[2] = vertices[2];
	g_BgGeoDamageTris[index].vertices[3] = vertices[3];
	g_BgGeoDamageTris[index].normal = hitthing->unk0c;
	bgGeoDamageSpawnPuff(roomnum, hitthing);

	mdMaterialImpactPlayTileHit();

#ifndef PLATFORM_N64
	if (g_BgGeoDamageDebugHitCount < 32) {
		printf("stack-geodamage: record room=%d tex=%04x index=%d v0=(%d,%d,%d|%d,%d) v2=(%d,%d,%d|%d,%d)\n",
			roomnum,
			hitthing->texturenum,
			index,
			vertices[0].x, vertices[0].y, vertices[0].z, vertices[0].s, vertices[0].t,
			vertices[2].x, vertices[2].y, vertices[2].z, vertices[2].s, vertices[2].t);
		g_BgGeoDamageDebugHitCount++;
	}
#endif
}

void bgSpawnMetalHitSpark(s32 roomnum, struct hitthing *hitthing)
{
	s32 index;

	if (g_Vars.stagenum != STAGE_EXTRA15
			|| hitthing == NULL
			|| (hitthing->texturenum != 0x007f && hitthing->texturenum != 0x007d)) {
		return;
	}

	if (g_BgMetalHitSparkCount < BGMETALHIT_MAX_SPARKS) {
		index = g_BgMetalHitSparkCount;
		g_BgMetalHitSparkCount++;
	} else {
		index = g_BgMetalHitSparkNextIndex;
		g_BgMetalHitSparkNextIndex = (g_BgMetalHitSparkNextIndex + 1) % BGMETALHIT_MAX_SPARKS;
	}

	g_BgMetalHitSparks[index].roomnum = roomnum;
	g_BgMetalHitSparks[index].variant = rngRandom() % BGMETALHIT_SPARK_NUM_VARIANTS;
	g_BgMetalHitSparks[index].startframe = g_Vars.lvframenum;
	g_BgMetalHitSparks[index].pos.x = hitthing->pos.x - g_BgRooms[roomnum].pos.x;
	g_BgMetalHitSparks[index].pos.y = hitthing->pos.y - g_BgRooms[roomnum].pos.y + BGGEODAMAGE_SURFACE_LIFT;
	g_BgMetalHitSparks[index].pos.z = hitthing->pos.z - g_BgRooms[roomnum].pos.z;
}

void bgSpawnWallHit(s32 roomnum, struct hitthing *hitthing, struct coord *sourcepos)
{
	s32 index;
	f32 offsetmag;
	struct coord offset;
	...
	if (g_Vars.stagenum != STAGE_EXTRA15
			|| hitthing == NULL
			|| hitthing->texturenum != BGWALLHIT_TEXNUM) {
		return;
	}

	if (g_BgWallHitCount < BGWALLHIT_MAX_HITS) {
		index = g_BgWallHitCount;
		g_BgWallHitCount++;
	} else {
		index = g_BgWallHitNextIndex;
		g_BgWallHitNextIndex = (g_BgWallHitNextIndex + 1) % BGWALLHIT_MAX_HITS;
	}

	if (g_Vars.currentplayer != NULL) {
		offset.x = g_Vars.currentplayer->cam_pos.x - hitthing->pos.x;
		offset.y = g_Vars.currentplayer->cam_pos.y - hitthing->pos.y;
		offset.z = g_Vars.currentplayer->cam_pos.z - hitthing->pos.z;
	} else if (sourcepos != NULL) {
		offset.x = sourcepos->x - hitthing->pos.x;
		offset.y = sourcepos->y - hitthing->pos.y;
		offset.z = sourcepos->z - hitthing->pos.z;
	} else {
		offset = hitthing->unk0c;
	}

	offsetmag = sqrtf(offset.x * offset.x + offset.y * offset.y + offset.z * offset.z);
	...
	g_BgWallHits[index].roomnum = roomnum;
	g_BgWallHits[index].startframe = g_Vars.lvframenum;
	g_BgWallHits[index].pos.x = hitthing->pos.x - g_BgRooms[roomnum].pos.x + offset.x * BGWALLHIT_SURFACE_LIFT;
	g_BgWallHits[index].pos.y = hitthing->pos.y - g_BgRooms[roomnum].pos.y + offset.y * BGWALLHIT_SURFACE_LIFT;
	g_BgWallHits[index].pos.z = hitthing->pos.z - g_BgRooms[roomnum].pos.z + offset.z * BGWALLHIT_SURFACE_LIFT;
	g_BgWallHits[index].outward.x = offset.x;
	g_BgWallHits[index].outward.y = 0.0f;
	g_BgWallHits[index].outward.z = offset.z;

	offsetmag = sqrtf(g_BgWallHits[index].outward.x * g_BgWallHits[index].outward.x
			+ g_BgWallHits[index].outward.z * g_BgWallHits[index].outward.z);

	if (offsetmag > 0.0f) {
		g_BgWallHits[index].outward.x /= offsetmag;
		g_BgWallHits[index].outward.z /= offsetmag;
	} else {
		g_BgWallHits[index].outward = offset;
	}
}

static Gfx *bgRenderGeoDamageForRoom(Gfx *gdl, s32 roomnum)
{
	s32 i;
	bool texselected = false;

	if (!bgGeoDamageStageEnabled() || g_BgGeoDamageTriCount == 0) {
		return gdl;
	}

	for (i = 0; i < g_BgGeoDamageTriCount; i++) {
		struct bggeodamagetri *entry = &g_BgGeoDamageTris[i];
		Vtx *vertices;
		f32 nx;
		f32 ny;
		f32 nz;
		...
		if (entry->roomnum != roomnum) {
			continue;
		}

		if (!texselected) {
			texSelect(&gdl, &g_BgGeoDamageTextureConfig, 2, 2, 2, 1, NULL);
			texselected = true;
		}
		...
		vertices[0] = entry->vertices[0];
		vertices[1] = entry->vertices[1];
		vertices[2] = entry->vertices[2];
		vertices[3] = entry->vertices[3];


		nx = entry->normal.x;
		ny = entry->normal.y;
		nz = entry->normal.z;
		invlen = nx * nx + ny * ny + nz * nz;

		if (invlen > 0.0f) {
			invlen = BGGEODAMAGE_SURFACE_LIFT / sqrtf(invlen);
			vertices[0].x += (s16)(nx * invlen);
			vertices[0].y += (s16)(ny * invlen);
			vertices[0].z += (s16)(nz * invlen);
			vertices[1].x += (s16)(nx * invlen);
			vertices[1].y += (s16)(ny * invlen);
			vertices[1].z += (s16)(nz * invlen);
			vertices[2].x += (s16)(nx * invlen);
			vertices[2].y += (s16)(ny * invlen);
			vertices[2].z += (s16)(nz * invlen);
			vertices[3].x += (s16)(nx * invlen);
			vertices[3].y += (s16)(ny * invlen);
			vertices[3].z += (s16)(nz * invlen);
		}

#ifndef PLATFORM_N64
		if (g_BgGeoDamageDebugRenderCount < 16) {
			printf("stack-geodamage: render room=%d tri=%d v0=(%d,%d,%d) n=(%.2f,%.2f,%.2f)\n",
				roomnum,
				i,
				vertices[0].x,
				vertices[0].y,
...
static void bgSelectDirectRgbaImpactTexture(Gfx **gdlptr, struct textureconfig *tconfig, s32 width, s32 height)
{
	Gfx *gdl = *gdlptr;
	s32 line = (width + 3) >> 2;
	s32 lrs = tex0f0b3548(width, height, 1) - 1;
	s32 smask = (tconfig->s & G_TX_CLAMP) ? G_TX_NOMASK : texGetMask(width);
	s32 tmask = (tconfig->t & G_TX_CLAMP) ? G_TX_NOMASK : texGetMask(height);

	texSetRenderMode(&gdl, 2, 1, 1);
	gSPTextureL(gdl++, 0xffff, 0xffff, 0, 0, G_TX_RENDERTILE, G_ON);
	gDPSetTextureLOD(gdl++, G_TL_TILE);
	gDPSetCombineMode(gdl++, G_CC_DECALRGBA, G_CC_DECALRGBA);
	gDPSetTextureInfoEXT(gdl++, G_TEXTYPE_GENERAL, 0, tconfig->texturenum, 0);
	gDPSetTextureImage(gdl++, G_IM_FMT_RGBA, G_IM_SIZ_32b, 1, g_BgGeoDamagePuffTextureStub);
	gDPSetTile(gdl++, G_IM_FMT_RGBA, G_IM_SIZ_32b, 0, 0x0000, 5, 0,
			tconfig->t, G_TX_NOMASK, G_TX_NOLOD,
			tconfig->s, G_TX_NOMASK, G_TX_NOLOD);
	gDPLoadSync(gdl++);
	gDPLoadBlock(gdl++, 5, 0, 0, lrs, 0);
	gDPPipeSync(gdl++);
	gDPSetTextureLUT(gdl++, G_TT_NONE);
	gDPSetTile(gdl++, G_IM_FMT_RGBA, G_IM_SIZ_32b, line, 0x0000, G_TX_RENDERTILE, 0,
			tconfig->t, tmask, G_TX_NOLOD,
			tconfig->s, smask, G_TX_NOLOD);
	gDPSetTileSize(gdl++, G_TX_RENDERTILE, 0, 0, ((width - 1) << 2), ((height - 1) << 2));

	*gdlptr = gdl;
}


static Gfx *bgRenderGeoDamagePuffsForRoom(Gfx *gdl, s32 roomnum)
{
	Mtxf *billboardmtx = camGetProjectionMtxF();
	s32 i;
	s32 prevvariant = -1;
	s32 prevframe = -1;
	bool used = false;

	if (!bgGeoDamageStageEnabled() || g_BgGeoDamagePuffCount == 0) {
		return gdl;
	}
	...
	for (i = 0; i < g_BgGeoDamagePuffCount; i++) {
		struct bggeodamagepuff *puff = &g_BgGeoDamagePuffs[i];
		s32 frameindex;
		struct coord center;
		struct coord left;
		struct coord top;
		...
		if (puff->roomnum != roomnum) {
			continue;
		}

		frameindex = (g_Vars.lvframenum - puff->startframe) / BGGEODAMAGE_PUFF_FRAME_TICKS;

		if (frameindex < 0 || frameindex >= BGGEODAMAGE_PUFF_NUM_FRAMES) {
			continue;
		}

		if (frameindex != prevframe) {
			bgSelectDirectRgbaImpactTexture(&gdl, &g_BgGeoDamagePuffTextureConfigs[frameindex],
					BGGEODAMAGE_PUFF_TEX_WIDTH, BGGEODAMAGE_PUFF_TEX_HEIGHT);
		...
		if (vertices == NULL || colours == NULL) {
			return gdl;
		}

		left.x = billboardmtx->m[0][0] * BGGEODAMAGE_PUFF_HALF_WIDTH;
		left.y = billboardmtx->m[0][1] * BGGEODAMAGE_PUFF_HALF_WIDTH;
		left.z = billboardmtx->m[0][2] * BGGEODAMAGE_PUFF_HALF_WIDTH;
		top.x = billboardmtx->m[1][0] * BGGEODAMAGE_PUFF_HEIGHT;
		top.y = billboardmtx->m[1][1] * BGGEODAMAGE_PUFF_HEIGHT;
		top.z = billboardmtx->m[1][2] * BGGEODAMAGE_PUFF_HEIGHT;

		center.x = puff->pos.x + billboardmtx->m[0][0] * BGGEODAMAGE_PUFF_RIGHT_SHIFT;
		center.y = puff->pos.y + billboardmtx->m[0][1] * BGGEODAMAGE_PUFF_RIGHT_SHIFT;
		center.z = puff->pos.z + billboardmtx->m[0][2] * BGGEODAMAGE_PUFF_RIGHT_SHIFT;

		vertices[0].x = (s16)(center.x - left.x + top.x);
		vertices[0].y = (s16)(center.y - left.y + top.y);
		vertices[0].z = (s16)(center.z - left.z + top.z);
		vertices[0].s = BGGEODAMAGE_PUFF_TEX_S0;
		vertices[0].t = BGGEODAMAGE_PUFF_TEX_T1;
		vertices[0].colour = 0;

		vertices[1].x = (s16)(center.x - left.x);
		vertices[1].y = (s16)(center.y - left.y);
		vertices[1].z = (s16)(center.z - left.z);
		vertices[1].s = BGGEODAMAGE_PUFF_TEX_S0;
		vertices[1].t = BGGEODAMAGE_PUFF_TEX_T0;
		vertices[1].colour = 0;

		vertices[2].x = (s16)(center.x + left.x);
		vertices[2].y = (s16)(center.y + left.y);
		vertices[2].z = (s16)(center.z + left.z);
		vertices[2].s = BGGEODAMAGE_PUFF_TEX_S1;
		vertices[2].t = BGGEODAMAGE_PUFF_TEX_T0;
		vertices[2].colour = 0;

		vertices[3].x = (s16)(center.x + left.x + top.x);
		vertices[3].y = (s16)(center.y + left.y + top.y);
		vertices[3].z = (s16)(center.z + left.z + top.z);
		vertices[3].s = BGGEODAMAGE_PUFF_TEX_S1;
		vertices[3].t = BGGEODAMAGE_PUFF_TEX_T1;
		vertices[3].colour = 0;
...
#ifndef PLATFORM_N64
		if (g_BgGeoDamageDebugPuffCount < 64) {
			printf("stack-geodamage: puff-render room=%d frame=%d pos=(%.1f,%.1f,%.1f) v0=(%d,%d,%d) v2=(%d,%d,%d)\n",
					roomnum, frameindex, puff->pos.x, puff->pos.y, puff->pos.z,
					vertices[0].x, vertices[0].y, vertices[0].z,
					vertices[2].x, vertices[2].y, vertices[2].z);
			g_BgGeoDamageDebugPuffCount++;
		}
#endif
...
static Gfx *bgRenderMetalHitSparksForRoom(Gfx *gdl, s32 roomnum)
{
	Mtxf *billboardmtx = camGetProjectionMtxF();
	s32 i;
	s32 prevvariant = -1;
	...
	if (g_Vars.stagenum != STAGE_EXTRA15 || g_BgMetalHitSparkCount == 0) {
		return gdl;
	}
	...
	for (i = 0; i < g_BgMetalHitSparkCount; i++) {
		struct bgmetalhitspark *spark = &g_BgMetalHitSparks[i];
		s32 frameindex;
		struct coord center;
		struct coord left;
		struct coord top;
		...
		if (spark->roomnum != roomnum) {
			continue;
		}

		frameindex = (g_Vars.lvframenum - spark->startframe) / BGMETALHIT_SPARK_FRAME_TICKS;

		if (frameindex < 0 || frameindex >= BGMETALHIT_SPARK_NUM_FRAMES) {
			continue;
		}

		if (spark->variant != prevvariant || frameindex != prevframe) {
			bgSelectDirectRgbaImpactTexture(&gdl,
					&g_BgMetalHitSparkTextureConfigs[spark->variant][frameindex],
					BGMETALHIT_SPARK_TEX_WIDTH, BGMETALHIT_SPARK_TEX_HEIGHT);
			prevvariant = spark->variant;
			prevframe = frameindex;
		}
		...
		left.x = billboardmtx->m[0][0] * BGMETALHIT_SPARK_HALF_WIDTH;
		left.y = billboardmtx->m[0][1] * BGMETALHIT_SPARK_HALF_WIDTH;
		left.z = billboardmtx->m[0][2] * BGMETALHIT_SPARK_HALF_WIDTH;
		top.x = billboardmtx->m[1][0] * BGMETALHIT_SPARK_HEIGHT;
		top.y = billboardmtx->m[1][1] * BGMETALHIT_SPARK_HEIGHT;
		top.z = billboardmtx->m[1][2] * BGMETALHIT_SPARK_HEIGHT;

		center.x = spark->pos.x
			+ billboardmtx->m[0][0] * BGMETALHIT_SPARK_RIGHT_SHIFT
			- billboardmtx->m[1][0] * BGMETALHIT_SPARK_DOWN_SHIFT;
		center.y = spark->pos.y
			+ billboardmtx->m[0][1] * BGMETALHIT_SPARK_RIGHT_SHIFT
			- billboardmtx->m[1][1] * BGMETALHIT_SPARK_DOWN_SHIFT;
		center.z = spark->pos.z
			+ billboardmtx->m[0][2] * BGMETALHIT_SPARK_RIGHT_SHIFT
			- billboardmtx->m[1][2] * BGMETALHIT_SPARK_DOWN_SHIFT;

		vertices[0].x = (s16)(center.x - left.x + top.x);
		vertices[0].y = (s16)(center.y - left.y + top.y);
		vertices[0].z = (s16)(center.z - left.z + top.z);
		vertices[0].s = BGMETALHIT_SPARK_TEX_S0;
		vertices[0].t = BGMETALHIT_SPARK_TEX_T1;
		vertices[0].colour = 0;

		vertices[1].x = (s16)(center.x - left.x);
		vertices[1].y = (s16)(center.y - left.y);
		vertices[1].z = (s16)(center.z - left.z);
		vertices[1].s = BGMETALHIT_SPARK_TEX_S0;
		vertices[1].t = BGMETALHIT_SPARK_TEX_T0;
		vertices[1].colour = 0;

		vertices[2].x = (s16)(center.x + left.x);
		vertices[2].y = (s16)(center.y + left.y);
		vertices[2].z = (s16)(center.z + left.z);
		vertices[2].s = BGMETALHIT_SPARK_TEX_S1;
		vertices[2].t = BGMETALHIT_SPARK_TEX_T0;
		vertices[2].colour = 0;

		vertices[3].x = (s16)(center.x + left.x + top.x);
		vertices[3].y = (s16)(center.y + left.y + top.y);
		vertices[3].z = (s16)(center.z + left.z + top.z);
		vertices[3].s = BGMETALHIT_SPARK_TEX_S1;
		vertices[3].t = BGMETALHIT_SPARK_TEX_T1;
		vertices[3].colour = 0;
...
static Gfx *bgRenderWallHitPlane(Gfx *gdl, struct coord *center, struct coord *left, struct coord *up)
{
	...
	vertices[0].x = (s16)(center->x - left->x + up->x);
	vertices[0].y = (s16)(center->y - left->y + up->y);
	vertices[0].z = (s16)(center->z - left->z + up->z);
	vertices[0].s = BGWALLHIT_TEX_S0;
	vertices[0].t = BGWALLHIT_TEX_T1;
	vertices[0].colour = 0;

	vertices[1].x = (s16)(center->x - left->x - up->x);
	vertices[1].y = (s16)(center->y - left->y - up->y);
	vertices[1].z = (s16)(center->z - left->z - up->z);
	vertices[1].s = BGWALLHIT_TEX_S0;
	vertices[1].t = BGWALLHIT_TEX_T0;
	vertices[1].colour = 0;

	vertices[2].x = (s16)(center->x + left->x - up->x);
	vertices[2].y = (s16)(center->y + left->y - up->y);
	vertices[2].z = (s16)(center->z + left->z - up->z);
	vertices[2].s = BGWALLHIT_TEX_S1;
	vertices[2].t = BGWALLHIT_TEX_T0;
	vertices[2].colour = 0;

	vertices[3].x = (s16)(center->x + left->x + up->x);
	vertices[3].y = (s16)(center->y + left->y + up->y);
	vertices[3].z = (s16)(center->z + left->z + up->z);
	vertices[3].s = BGWALLHIT_TEX_S1;
	vertices[3].t = BGWALLHIT_TEX_T1;
	vertices[3].colour = 0;
...
static Gfx *bgRenderWallHitSidePlane(Gfx *gdl, struct bgwallhit *hit, f32 tangentscale)
{
	struct coord center;
	struct coord left;
	struct coord up;
	...
	dir.x = hit->outward.x + hit->outward.z * tangentscale;
	dir.y = 0.0f;
	dir.z = hit->outward.z - hit->outward.x * tangentscale;

	mag = sqrtf(dir.x * dir.x + dir.z * dir.z);
	...
	left.x = -dir.x * BGWALLHIT_SIDE_HALF_WIDTH;
	left.y = 0.0f;
	left.z = -dir.z * BGWALLHIT_SIDE_HALF_WIDTH;
	up.x = 0.0f;
	up.y = BGWALLHIT_SIDE_HALF_HEIGHT;
	up.z = 0.0f;

	center.x = hit->pos.x - left.x;
	center.y = hit->pos.y;
	center.z = hit->pos.z - left.z;

	return bgRenderWallHitPlane(gdl, &center, &left, &up);
}

static Gfx *bgRenderWallHitsForRoom(Gfx *gdl, s32 roomnum)
{
	Mtxf *billboardmtx = camGetProjectionMtxF();
	s32 i;
	bool used = false;

	if (g_Vars.stagenum != STAGE_EXTRA15 || g_BgWallHitCount == 0) {
		return gdl;
	}
	...
	for (i = 0; i < g_BgWallHitCount; i++) {
		struct bgwallhit *hit = &g_BgWallHits[i];
		s32 frameindex;
		struct coord center;
		struct coord left;
		struct coord up;

		if (hit->roomnum != roomnum) {
			continue;
		}

		frameindex = (g_Vars.lvframenum - hit->startframe) / BGWALLHIT_FRAME_TICKS;

		if (frameindex < 0 || frameindex >= BGWALLHIT_NUM_FRAMES) {
			continue;
		}

		center = hit->pos;

		left.x = billboardmtx->m[0][0] * BGWALLHIT_FRONT_HALF_WIDTH;
		left.y = billboardmtx->m[0][1] * BGWALLHIT_FRONT_HALF_WIDTH;
		left.z = billboardmtx->m[0][2] * BGWALLHIT_FRONT_HALF_WIDTH;
		up.x = billboardmtx->m[1][0] * BGWALLHIT_FRONT_HALF_HEIGHT;
		up.y = billboardmtx->m[1][1] * BGWALLHIT_FRONT_HALF_HEIGHT;
		up.z = billboardmtx->m[1][2] * BGWALLHIT_FRONT_HALF_HEIGHT;

		bgSelectDirectRgbaImpactTexture(&gdl, &g_BgWallHitFrontTextureConfigs[frameindex],
				BGWALLHIT_TEX_WIDTH, BGWALLHIT_TEX_HEIGHT);

		gdl = bgRenderWallHitPlane(gdl, &center, &left, &up);

		bgSelectDirectRgbaImpactTexture(&gdl, &g_BgWallHitSideTextureConfigs[frameindex],
				BGWALLHIT_TEX_WIDTH, BGWALLHIT_TEX_HEIGHT);

		gdl = bgRenderWallHitSidePlane(gdl, hit, -BGWALLHIT_SIDE_CROSS_TANGENT);
		gdl = bgRenderWallHitSidePlane(gdl, hit, BGWALLHIT_SIDE_CROSS_TANGENT);
...
Gfx *bgRenderArtifacts(Gfx *gdl)
{
	...
	if ((g_Vars.stagenum == STAGE_CITRAINING && !g_Vars.currentplayer->training)
			|| (g_MenuData.count > 0
				&& (g_Vars.stagenum == STAGE_CITRAINING || g_Vars.stagenum == STAGE_DEFENSE))) {
...
void bgReset(s32 stagenum)
{
	...
	g_BgGeoDamageTriCount = 0;
	g_BgGeoDamageTriNextIndex = 0;
	g_BgGeoDamagePuffCount = 0;
	g_BgGeoDamagePuffNextIndex = 0;
	g_BgMetalHitSparkCount = 0;
	g_BgMetalHitSparkNextIndex = 0;
	g_BgWallHitCount = 0;
	g_BgWallHitNextIndex = 0;
	bgResetGeoDamageTextureConfigs();
#ifndef PLATFORM_N64
	g_BgGeoDamageDebugHitCount = 0;
	g_BgGeoDamageDebugRenderCount = 0;
	g_BgGeoDamageDebugPuffCount = 0;
#endif
...
void bgLoadRoom(s32 roomnum)
{
	...
	if (allocation != NULL) {
		...
		for (i = 0; i < numgdls; i++) {
			...
			byteswritten = texLoadFromGdl((void *) gdlpointers[i], len, (void *) itergdl2, NULL, vtxblocks[i], 0);
...
Gfx *bgRenderRoomOpaque(Gfx *gdl, s32 roomnum)
{
	...
	gdl = bgRenderGeoDamageForRoom(gdl, roomnum);
	gdl = lightsSetDefault(gdl);
...
Gfx *bgRenderRoomXlu(Gfx *gdl, s32 roomnum)
{
	...
	if (g_Rooms[roomnum].loaded240) {
		...
		gdl = bgRenderGeoDamagePuffsForRoom(gdl, roomnum);
		gdl = bgRenderMetalHitSparksForRoom(gdl, roomnum);
		gdl = bgRenderWallHitsForRoom(gdl, roomnum);
									...
									if (bgTestLineIntersectsBbox(arg0, arg2, &min, &max)
										...
										if (sqdist < lowestsqdist) {
											...
											if (hit) {
												...
												hitthing->unk18 = dmemvtx[points[0]];
												hitthing->unk1c = dmemvtx[points[1]];
												hitthing->unk20 = dmemvtx[points[2]];
												hitthing->texturenum = texturenum;
