...
{
		if ((g_MpSetup.chrslots & MPCHRSLOT_BIT(chrnum)) == 0) {
		return 0;
	}
...
{
		return (g_MpChallenges[challengeindex].availability & (((g_MpSetup.chrslots & MP_PLAYER_SLOTS_MASK) << 1) | 1)) != 0;
}
...
void challengeDetermineUnlockedFeatures(void)
{
	...
	if (gamefileHasFlag(GAMEFILEFLAG_MP_ONLY_UNLOCKS)) {
		for (j = 1; j < ARRAYCOUNT(g_MpFeaturesUnlocked); j++) {
			g_MpFeaturesUnlocked[j] = 0x1f;
		}
	}
	...
	if (!challengeIsFeatureUnlocked(MPFEATURE_8BOTS)) {
			for (k = 4; k < MAX_BOTS_RUNTIME; k++) {
				if (g_MpSetup.chrslots & MPCHRSLOT_BIT(MAX_PLAYERS + k)) {
					mpRemoveSimulant(k);
				}
			}
		...
		// for this number of players
			...
			for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
				...
				if (g_BotConfigsArray[i].difficulty != BOTDIFF_DISABLED) {
					g_MpSetup.chrslots |= MPCHRSLOT_BIT(i + MAX_PLAYERS);
				}
			}
...
static s32 challengeGetConfigBotCount(void)
{
	return MAX_BOTS;
}
...
s32 challengeForceUnlockSetupFeatures(struct mpsetup *setup, u8 *array, s32 len)
{
	...
	if (setup->scenario < ARRAYCOUNT(g_MpScenarioOverviews)) {
...
void challengeForceUnlockConfigFeatures(struct mpconfig *config, u8 *array, s32 len, s32 challengeindex)
{
	...
	s32 numplayers;
	s32 i;
	s32 configbotcount = challengeGetConfigBotCount();

	for (i = 0; i < configbotcount; i++) {
