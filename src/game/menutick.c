...
void menuTick(void)
{
	...
	// On a normal boot from the title logos, open the menu flow immediately
	// instead of waiting for the CI intro camera/cutscene sequence to finish.
	if (g_TitleBootToMenu && g_Vars.stagenum == STAGE_CITRAINING) {
		g_PlayersWithControl[0] = false;
		...
		if (g_Vars.lvframenum > 5) {
			...
			playerPause(g_FileState == FILESTATE_UNSELECTED ? MENUROOT_FILEMGR : MENUROOT_MAINMENU);
