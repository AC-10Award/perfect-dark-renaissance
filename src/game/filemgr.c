...
struct menudialogdef g_FilemgrIntroMenuDialog;
...
bool g_FilemgrIntroAcknowledged = false;
static bool g_FilemgrIntroCanDismiss = false;
static const char g_FilemgrIntroTitle[] = "WARNING";
static const char g_FilemgrIntroBody[] =
		"Perfect Dark Renaissance is an experimental mod\n"
		"designed to expand Perfect Dark's Multiplayer and\n"
		"Combat Simulator modes.\n\n"
		"Some new modes are prototypes and have not been\n"
		"fully playtested. These modifications may cause\n"
		"unforeseen issues or affect other parts of the game,\n"
		"including the single-player campaign.\n\n"
		"Existing Perfect Dark save files or EEPROM data may\n"
		"cause instability. We recommend creating a new game\n"
		"profile and save data when playing this mod.\n";
static const char g_FilemgrIntroContinueText[] = "Press any button to continue";
static const char g_FilemgrProfileMenuTitle[] = "PERFECT DARK RENAISSANCE v1.1\n";
static const char g_FilemgrProfileMenuSubtitle[] = "Choose a Game Profile\n";

static void filemgrPushMainFileSelectRoot(void)
{
...
static MenuDialogHandlerResult filemgrIntroMenuDialog(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	if (operation == MENUOP_OPEN) {
		g_FilemgrIntroCanDismiss = false;
	} else if (operation == MENUOP_TICK) {
		if (!g_FilemgrIntroCanDismiss) {
			if (joyGetButtons(0, 0xffffffff) == 0) {
				g_FilemgrIntroCanDismiss = true;
			}
		} else if (joyGetButtonsPressedThisFrame(0, 0xffffffff) != 0) {
			g_FilemgrIntroAcknowledged = true;
			filemgrPushMainFileSelectRoot();
		}
	}
...
{
	static char profileslabel[] = "GAME PROFILES:\n";
	u16 names[] = {
		L_OPTIONS_111, // "Game Pak"
		L_OPTIONS_112, // "Controller Pak 1"
		L_OPTIONS_113, // "Controller Pak 2"
		...
		if (g_FileLists[listnum]->devicestartindexes[i] != -1) {
			if (remaining == 0) {
				if (operation == MENUOP_GETOPTGROUPTEXT) {
					if (i == 0) {
						return (uintptr_t) profileslabel;
					}
...
char *filemgrMenuTextInsertOriginalPak(struct menuitem *item)
{
	...
	// Replace first line break in namebuffer with a termination
	i = 0;
...
bool filemgrSaveOrLoad(struct fileguid *guid, s32 fileop, uintptr_t playernum)
{
	...
	if (g_Menus[g_MpPlayerNum].fm.fileop == FILEOP_SAVE_MPPLAYER
			|| g_Menus[g_MpPlayerNum].fm.fileop == FILEOP_LOAD_MPPLAYER) {
		if (filemgrAttemptOperation(-1, false) != 0) {
...
char *filemgrMenuTextLocationName2(struct menuitem *item)
{
	...
	if (item->param == SAVEDEVICE_GAMEPAK) {
		return profileslabel;
	}
...
bool filemgrConsiderPushingFileSelectDialog(void)
{
	if (g_Menus[g_MpPlayerNum].openinhibit == 0) {
		if (g_FilemgrIntroAcknowledged) {
			filemgrPushMainFileSelectRoot();
		} else {
			g_Menus[g_MpPlayerNum].playernum = 0;
			menuPushRootDialog(&g_FilemgrIntroMenuDialog, MENUROOT_FILEMGR);
		}
...
void bootmenuReset(void)
{
	...
	g_MpPlayerNum = 0;
	g_FilemgrIntroAcknowledged = false;
	menuPushRootDialog(&g_PakChoosePakMenuDialog, MENUROOT_BOOTPAKMGR);
	g_MpPlayerNum = prevplayernum;
}

struct menuitem g_FilemgrIntroMenuItems[] = {
	{
		MENUITEMTYPE_LABEL,
		0,
		MENUITEMFLAG_LESSLEFTPADDING | MENUITEMFLAG_LITERAL_TEXT | MENUITEMFLAG_SMALLFONT,
		(uintptr_t)g_FilemgrIntroBody,
		0,
		NULL,
	},
	{
	...
	},
	{
		...
		(uintptr_t)g_FilemgrIntroContinueText,
		0,
		NULL,
	},
	{ MENUITEMTYPE_END },
...
struct menudialogdef g_FilemgrIntroMenuDialog = {
	MENUDIALOGTYPE_DANGER,
	(uintptr_t)g_FilemgrIntroTitle,
	g_FilemgrIntroMenuItems,
	filemgrIntroMenuDialog,
...
struct menuitem g_FilemgrFileSelectMenuItems[] = {
	{
		MENUITEMTYPE_LABEL,
		0,
		MENUITEMFLAG_LESSLEFTPADDING | MENUITEMFLAG_DARKERBG | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)g_FilemgrProfileMenuSubtitle,
		0,
		NULL,
	},
	{
...
struct menudialogdef g_FilemgrFileSelectMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
	(uintptr_t)g_FilemgrProfileMenuTitle,
