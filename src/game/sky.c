...
Gfx *skyRenderSuns(Gfx *gdl, bool xray)
{
	...
	// The CI backdrop path uses the training stage outside of active training.
	// Skip the sun orb there, and also when menu flows keep CI/Carrington live.
...
Gfx *skyRenderArtifacts(Gfx *gdl)
{
	...
	// The CI backdrop path uses the training stage outside of active training.
	// Skip the sun flares there, and also when menu flows keep CI/Carrington live.
	if ((g_Vars.stagenum == STAGE_CITRAINING && !g_Vars.currentplayer->training)
			|| (g_MenuData.count > 0
				&& (g_Vars.stagenum == STAGE_CITRAINING || g_Vars.stagenum == STAGE_DEFENSE))) {
		return gdl;
