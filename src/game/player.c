...
#ifndef PLATFORM_N64
static void logAlternateWeaponModelTextures(s32 fileid, struct modeldef *modeldef)
{
	static bool loggedfalcontextures = false;
	s32 i;

	if (loggedfalcontextures || fileid != FILE_GFALCON2 || modeldef == NULL) {
		return;
	}

	loggedfalcontextures = true;
	sysLogPrintf(LOG_NOTE, "alternate-weapon: weapon model %04x has %d texconfigs", fileid, modeldef->numtexconfigs);

	for (i = 0; i < modeldef->numtexconfigs; i++) {
		sysLogPrintf(
				LOG_NOTE,
				"alternate-weapon: texconfig[%d] texturenum=%08x size=%dx%d format=%d depth=%d",
				i,
				(u32) modeldef->texconfigs[i].texturenum,
...
static s32 playerGetChrBodyWeaponNum(s32 handnum)
{
	s32 weaponnum = bgunGetWeaponNum2(handnum);

	if (weaponnum == WEAPON_FALCON2
			&& g_Vars.currentplayer->hands[handnum].gset.weaponfunc == FUNC_SECONDARY) {
		return WEAPON_FALCON2_SILENCER;
	}
...
void playerTickChrBody(void)
{
	...
	if (g_Vars.currentplayer->haschrbody == false) {
		...
		s32 weaponmodelnum;
		s32 weaponnum = playerGetChrBodyWeaponNum(HAND_RIGHT);
		...
		if (g_Vars.currentplayer->model00d4 == NULL) {
			sysLogPrintf(LOG_ERROR,
					"playerTickChrBody: body allocation failed player=%d mpindex=%d bodynum=%d headnum=%d bodydef=%p headdef=%p mplayer=%d botcount=%d mpchrs=%d",
					g_Vars.currentplayernum,
					g_Vars.currentplayerstats ? g_Vars.currentplayerstats->mpindex : -1,
					bodynum,
					headnum,
					bodymodeldef,
					headmodeldef,
					g_Vars.mplayerisrunning ? 1 : 0,
					g_BotCount,
					g_MpNumChrs);
		}
#endif
		...
		chr->noblood = chr->bodynum == BODY_ROBOT_SKELINGTON;
...
#ifndef PLATFORM_N64
				logAlternateWeaponModelTextures(g_ModelStates[weaponmodelnum].fileid, weaponmodeldef);
#endif
			} else {
				weaponobj = NULL;
				weaponmodeldef = NULL;
		...
		if (g_Vars.mplayerisrunning) {
			s32 weaponnum = playerGetChrBodyWeaponNum(HAND_RIGHT);
			s32 weaponmodelnum = playermgrGetModelOfWeapon(weaponnum);
			struct prop *heldprop = chr->weapons_held[HAND_RIGHT];
			s32 heldweaponnum = heldprop ? heldprop->weapon->weaponnum : WEAPON_NONE;
			s32 heldmodelnum = heldprop ? heldprop->obj->modelnum : -1;

#ifndef PLATFORM_N64
			if (g_Vars.currentplayer->hands[HAND_RIGHT].gset.weaponnum == WEAPON_FALCON2
					|| heldweaponnum == WEAPON_FALCON2
					|| heldweaponnum == WEAPON_FALCON2_SILENCER
					|| heldmodelnum == MODEL_CHRFALCON2
					|| heldmodelnum == MODEL_CHRFALCON2SIL) {
				static s32 prevdesiredweaponnums[MAX_PLAYERS] = { -999, -999, -999, -999 };
				static s32 prevdesiredmodelnums[MAX_PLAYERS] = { -999, -999, -999, -999 };
				static s32 prevheldweaponnums[MAX_PLAYERS] = { -999, -999, -999, -999 };
				static s32 prevheldmodelnums[MAX_PLAYERS] = { -999, -999, -999, -999 };
				static s32 prevgunfuncs[MAX_PLAYERS] = { -999, -999, -999, -999 };
				s32 playernum = g_Vars.currentplayernum;
				s32 gunfunc = g_Vars.currentplayer->hands[HAND_RIGHT].gset.weaponfunc;

				if (playernum >= 0 && playernum < MAX_PLAYERS
						&& (weaponnum != prevdesiredweaponnums[playernum]
							|| weaponmodelnum != prevdesiredmodelnums[playernum]
							|| heldweaponnum != prevheldweaponnums[playernum]
							|| heldmodelnum != prevheldmodelnums[playernum]
							|| gunfunc != prevgunfuncs[playernum])) {
					prevdesiredweaponnums[playernum] = weaponnum;
					prevdesiredmodelnums[playernum] = weaponmodelnum;
					prevheldweaponnums[playernum] = heldweaponnum;
					prevheldmodelnums[playernum] = heldmodelnum;
					prevgunfuncs[playernum] = gunfunc;

					sysLogPrintf(LOG_NOTE,
							"falcon-chrbody p%d handfunc=%d desiredweapon=%d desiredmodel=%d heldweapon=%d heldmodel=%d heldprop=%p",
							playernum,
							gunfunc,
							weaponnum,
							weaponmodelnum,
			...
			if (weaponnum != heldweaponnum || weaponmodelnum != heldmodelnum) {
				if (heldprop) {
				...
				if (weaponnum != WEAPON_NONE) {
					chrGiveWeaponWithAutoModel(chr, weaponnum, 0);
				}
			}
		}
			...
			if (playerIsFadeComplete()) {
				...
				if (g_Vars.mplayerisrunning) {
					if (g_Vars.coopplayernum >= 0 || g_Vars.antiplayernum >= 0) {
						...
						u32 playernum = g_Vars.currentplayerstats->mpindex;
						s32 playercount = PLAYERCOUNT();
						struct chrdata *chr = g_Vars.currentplayer->prop->chr;
						s32 numdeaths = 0;
						s32 i;
						...
						if (mpIsDeathsLimitEnabled() && !mpHasRemainingLives(playernum)) {
							canrestart = false;
						} else if (joyGetButtons(optionsGetContpadNum1(g_Vars.currentplayerstats->mpindex), 0xb000)
								&& !mpIsPaused()
