...
void doorTick(struct prop *doorprop)
{
	...
	if ((door->base.flags & OBJFLAG_DOOR_KEEPOPEN)
			&& (door->base.flags2 & OBJFLAG2_INVISIBLE) == 0
			&& door->base.damage < 0) {
		door->base.damage += g_Vars.lvupdate60;

		if (door->base.damage >= 0) {
			door->base.damage = 0;
			door->base.flags2 |= OBJFLAG2_INVISIBLE;
			doorUpdateTiles(door);
			setup0f0923d4(&door->base);
			func0f069c70(&door->base, false, false);
			func0f08d3dc(door);
		}
	}
...
u32 objTick(struct prop *prop)
{
	...
	if (prop->timetoregen > 0) {
		...
		if (prop->timetoregen <= 0) {
			...
			if (!silent) {
				if (propShouldPlayRespawnSound(prop)) {
		...
		if (objGetDestroyedLevel(obj) == 0) {
			...
			if (objGetDestroyedLevel(obj) > 0) {
				if (obj->hidden2 & OBJH2FLAG_CANREGEN) {
					...
					prop->timetoregen = propGetRespawnTimeTicks(prop);
				}
			}
...
struct nameinfo var8006aa94pf[] = {
	{ WEAPON_FALCON2,          MD_WEAPON_TEXT_FALCON2,          0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_FALCON2_SILENCER, MD_WEAPON_TEXT_FALCON2_SILENCER, 0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_FALCON2_SCOPE,    MD_WEAPON_TEXT_FALCON2_SCOPE,    0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_MAGSEC4,          L_GUN_010,                       0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ MD_WEAPON_MATRIX85,      MD_WEAPON_TEXT_MATRIX85,         0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_MAULER,           MD_WEAPON_TEXT_MAULER,           0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_PHOENIX,          MD_WEAPON_TEXT_PHOENIX,          0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_DY357MAGNUM,      MD_WEAPON_TEXT_DY357_MAGNUM,     0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_DY357LX,          MD_WEAPON_TEXT_DY357_LX,         0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_CMP150,           MD_WEAPON_TEXT_CMP150,           0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN    } },
	{ WEAPON_CYCLONE,          MD_WEAPON_TEXT_CYCLONE,          0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_CALLISTO,         MD_WEAPON_TEXT_CALLISTO_NTG,     0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_RCP120,           MD_WEAPON_TEXT_RCP120,           0, { DETERMINER_AN,    DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_LAPTOPGUN,        MD_WEAPON_TEXT_LAPTOP_GUN,       0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN    } },
	{ WEAPON_DRAGON,           MD_WEAPON_TEXT_DRAGON,           0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_K7AVENGER,        MD_WEAPON_TEXT_K7_AVENGER,       0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN    } },
	{ WEAPON_AR34,             MD_WEAPON_TEXT_AR34,             0, { DETERMINER_AN,    DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_THE   } },
	{ WEAPON_SUPERDRAGON,      MD_WEAPON_TEXT_SUPERDRAGON,      0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_SHOTGUN,          MD_WEAPON_TEXT_SHOTGUN,          0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_AN    } },
	{ WEAPON_REAPER,           MD_WEAPON_TEXT_REAPER,           0, { DETERMINER_THE,   DETERMINER_A,     DETERMINER_A,     DETERMINER_AN,    DETERMINER_A     } },
	{ WEAPON_SNIPERRIFLE,      MD_WEAPON_TEXT_SNIPER_RIFLE,     0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A     } },
	{ WEAPON_FARSIGHT,         MD_WEAPON_TEXT_FARSIGHT_XR20,    0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_AN    } },
	{ WEAPON_DEVASTATOR,       MD_WEAPON_TEXT_DEVASTATOR,       0, { DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A,     DETERMINER_A     } },
...
static void weaponApplyChrGunPartVisibility(struct weaponobj *weapon)
{
	struct model *model;
	struct modelnode *node;

	if (!weapon || !weapon->base.model || weapon->base.model->definition->skel != &g_SkelChrGun) {
		return;
	}

	model = weapon->base.model;

	if (weapon->weaponnum == WEAPON_FALCON2
			|| weapon->weaponnum == WEAPON_FALCON2_SILENCER
			|| weapon->weaponnum == WEAPON_FALCON2_SCOPE) {
		node = modelGetPart(model->definition, MODELPART_FALCON2_SILENCER);

		if (node) {
			union modelrwdata *rwdata = modelGetNodeRwData(model, node);
			rwdata->toggle.visible = (weapon->weaponnum == WEAPON_FALCON2_SILENCER);
		}

		node = modelGetPart(model->definition, MODELPART_FALCON2_SCOPE);

		if (node) {
			union modelrwdata *rwdata = modelGetNodeRwData(model, node);
			rwdata->toggle.visible = (weapon->weaponnum == WEAPON_FALCON2_SCOPE);
		}

#ifndef PLATFORM_N64
		{
			static s32 prevweaponnum = -999;
			static s32 prevmodelnum = -999;
			struct modelnode *silnode = modelGetPart(model->definition, MODELPART_FALCON2_SILENCER);
			struct modelnode *scopenode = modelGetPart(model->definition, MODELPART_FALCON2_SCOPE);
			bool silvisible = false;
			bool scopevisible = false;

			if (silnode) {
				union modelrwdata *silrwdata = modelGetNodeRwData(model, silnode);
				silvisible = silrwdata->toggle.visible;
			}

			if (scopenode) {
				union modelrwdata *scoperwdata = modelGetNodeRwData(model, scopenode);
				scopevisible = scoperwdata->toggle.visible;
			}

			if (weapon->weaponnum != prevweaponnum || weapon->base.modelnum != prevmodelnum) {
				prevweaponnum = weapon->weaponnum;
				prevmodelnum = weapon->base.modelnum;

				sysLogPrintf(LOG_NOTE,
						"falcon-chrgun attach weapon=%d model=%d modeldef=%p silnode=%p silvis=%d scopenode=%p scopevis=%d",
						weapon->weaponnum,
						weapon->base.modelnum,
						model->definition,
						silnode,
						silvisible ? 1 : 0,
						scopenode,
...
struct prop *weaponCreateForChr(struct chrdata *chr, s32 modelnum, s32 weaponnum, u32 flags, struct weaponobj *obj, struct modeldef *modeldef)
{
	...
	if (obj && prop && model) {
		...
		*obj = tmp;
		...
		weaponApplyChrGunPartVisibility(obj);
	} else {
		if (model) {
			modelmgrFreeModel(model);
		}
...
static bool doorIsDestructibleInMplayer(struct doorobj *door)
{
	if (!g_Vars.normmplayerisrunning || !(g_MpSetup.options & MPOPTION_DESTRUCTIBLEDOORS)) {
		return false;
	}

	if (door == NULL || door->base.prop == NULL) {
		return false;
	}
	...
	if (door->base.flags2 & OBJFLAG2_AIRLOCKDOOR) {
		return false;
	}
...
static s32 doorGetMplayerDestroyHideDelay(void)
{
	return TICKS(10);
}

static s32 doorGetMplayerExplosionThreshold(struct doorobj *door)
{
	switch (door->doortype) {
	case DOORTYPE_SWINGING:
		return 1;
...
static s32 doorGetGroupExplosionDamage(struct doorobj *door)
{
	s32 damage = door->base.damage;
	struct doorobj *loopdoor = door->sibling;

	while (loopdoor && loopdoor != door) {
		if (loopdoor->base.damage > damage) {
			damage = loopdoor->base.damage;
		}
...
static void doorSetGroupExplosionDamage(struct doorobj *door, s32 damage)
{
	struct doorobj *loopdoor = door;

	while (loopdoor) {
		loopdoor->base.damage = damage;
...
static void doorCreateDebrisSparks(struct doorobj *door)
{
	struct modelrodata_bbox bbox;
	struct coord emitpos;
	f32 xmin;
	f32 xmax;
	f32 ymin;
	f32 ymax;
	...
	if (door == NULL || door->base.prop == NULL || door->base.prop->rooms[0] < 0) {
		return;
	}
	...
	points[3][1] = ymax;
	points[3][2] = zmid;
	points[4][0] = (xmin + xmax) * 0.5f;
	points[4][1] = (ymin + ymax) * 0.5f;
	points[4][2] = zmid;

	for (i = 0; i < 5; i++) {
		emitpos.x = door->base.prop->pos.x
			+ door->base.realrot[0][0] * points[i][0]
			+ door->base.realrot[1][0] * points[i][1]
			+ door->base.realrot[2][0] * points[i][2];
		emitpos.y = door->base.prop->pos.y
			+ door->base.realrot[0][1] * points[i][0]
			+ door->base.realrot[1][1] * points[i][1]
			+ door->base.realrot[2][1] * points[i][2];
		emitpos.z = door->base.prop->pos.z
			+ door->base.realrot[0][2] * points[i][0]
			+ door->base.realrot[1][2] * points[i][1]
			+ door->base.realrot[2][2] * points[i][2];

		sparksCreate(door->base.prop->rooms[0], door->base.prop, &emitpos, NULL, NULL, SPARKTYPE_DOORDEBRIS);
	}
}

static void doorDestroyInMplayer(struct doorobj *door)
{
	struct doorobj *loopdoor = door;

	mdMaterialImpactPlayDoorDebris();

	while (loopdoor) {
		doorCreateDebrisSparks(loopdoor);

		loopdoor->base.flags |= OBJFLAG_CANNOT_ACTIVATE | OBJFLAG_DOOR_KEEPOPEN;
		loopdoor->autoclosetime = 0;
		loopdoor->mode = DOORMODE_IDLE;
		loopdoor->frac = loopdoor->maxfrac;
		loopdoor->fracspeed = 0;
		loopdoor->lastopen60 = g_Vars.lvframe60;
		loopdoor->perimfrac = 0;
		loopdoor->base.damage = -doorGetMplayerDestroyHideDelay();

		doorActivatePortal(loopdoor);
		doorPlayOpenedSound(loopdoor->soundtype, loopdoor->base.prop);
...
bool doorTestDestroyedByMplayerExplosion(struct doorobj *door, s32 playernum)
{
	s32 threshold;
	s32 damage;

	if (!doorIsDestructibleInMplayer(door)) {
		return false;
	}

	threshold = doorGetMplayerExplosionThreshold(door);

	if (threshold <= 0) {
		return false;
	}

	damage = doorGetGroupExplosionDamage(door) + 1;
	doorSetGroupExplosionDamage(door, damage);

	if (playernum >= 0) {
		door->base.hidden &= 0x0fffffff;
		door->base.hidden |= (playernum << 28) & 0xf0000000;
	}

	if (damage >= threshold) {
		doorDestroyInMplayer(door);
	}
