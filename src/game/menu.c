...
#define MP_TERMINATION_MENU_BG_TEX_NUM 0x0c18
#define MP_TERMINATION_MENU_BG_TEX_WIDTH 64
#define MP_TERMINATION_MENU_BG_TEX_HEIGHT 64
...
void menuCalculateItemSize(struct menuitem *item, s16 *width, s16 *height, struct menudialog *dialog)
{
	...
	case MENUITEMTYPE_SELECTABLE:
		...
		if (dialog->definition == &g_MpReadyMenuDialog
				&& g_MpPlayerNum == 0
				&& g_MpNumJoined >= 2
				&& item->param2 == L_MPMENU_106) {
			// The stock success-style Ready dialog packs body rows tightly.
			// Give the "...and waiting" row some extra slot height so the
			// added P1-only action can sit on its own line below it.
			*height += 9;
		}
		...
		if (dialog->definition == &g_MpReadyMenuDialog
				&& g_MpPlayerNum == 0
				&& g_MpNumJoined >= 2
				&& item->handler == menuhandlerMpStartAllPlayers) {
			// Give the P1-only action row a taller clip region so the stock
			// selectable renderer can show the full glyphs.
			*height += 9;
		}
#endif
		break;
...
#if VERSION >= VERSION_NTSC_1_0
	if (dialogdef == &g_MpReadyMenuDialog && g_MpPlayerNum == 0 && g_MpNumJoined >= 2) {
		// The Ready dialog has a special success-dialog presentation path and
		// needs a little extra body height to show the added P1-only option.
		contentheight += LINEHEIGHT + 6;
	}
#endif
		...
		if (menumodel->newparams == menumodel->curparams) {
			...
			if (menumodel->loaddelay == 0) {
				if (MENUMODELPARAMS_GET_FILENUM(menumodel->newparams) == 0xffff || MENUMODELPARAMS_HAS_MASTER_HEADBODY(menumodel->newparams)) {
					...
					if (totalfilelen >= menumodel->alloclen) {
						menumodel->newparams = 0;
						return gdl;
					}
					...
					if (!offscreen) {
						...
						if (focused) {
							...
							if ((item->type == MENUITEMTYPE_SELECTABLE
									...
									|| item->type == MENUITEMTYPE_0A
									|| item->type == MENUITEMTYPE_DROPDOWN)
									&& !(dialog->definition == &g_MpReadyMenuDialog
										&& item->handler == menuhandlerMpStartAllPlayers)) {
	...
	if (g_Menus[g_MpPlayerNum].curdialog) {
		if (g_MenuData.root == MENUROOT_MPPAUSE
			...
			if (g_MenuData.root == MENUROOT_MPENDSCREEN
					&& g_Menus[g_MpPlayerNum].curdialog->definition == &g_MpEndscreenLmsResultMenuDialog) {
				return gdl;
			}
...
void menuReset(void)
{
	...
	if (!g_Vars.mplayerisrunning) {
		...
		g_MenuData.hudpiece.newparams = 0;
		g_MenuData.hudpiece.curparams = 0;
...
Gfx *menuRenderBackgroundLayer1(Gfx *gdl, u8 bg, f32 frac)
{
	...
	case MENUBG_CONEALPHA:
		gdl = menugfxRenderBgStatic(gdl, 0xffffff00 | (u32)(255.0f * frac));
		break;
	...
	case MENUBG_CONEOPAQUE:
		gdl = menugfxRenderBgStatic(gdl, 0xffffffff);
		break;
	}
...
static bool menuShouldRenderTerminationStageSelectBackground(void)
{
	return g_MenuData.root == MENUROOT_MPSETUP
		&& g_Menus[g_MpPlayerNum].curdialog != NULL
		&& g_Menus[g_MpPlayerNum].curdialog->definition == &g_MpTerminationMenuDialog;
}

static Gfx *menuRenderTerminationStageSelectBackground(Gfx *gdl)
{
	struct textureconfig texture = {
		MP_TERMINATION_MENU_BG_TEX_NUM,
		MP_TERMINATION_MENU_BG_TEX_WIDTH,
		MP_TERMINATION_MENU_BG_TEX_HEIGHT,
		0,
		G_IM_FMT_RGBA,
		G_IM_SIZ_32b,
		G_TX_CLAMP,
	...
	dsdx = (MP_TERMINATION_MENU_BG_TEX_WIDTH << 10) / viGetWidth();
	dtdy = -((MP_TERMINATION_MENU_BG_TEX_HEIGHT << 10) / viGetHeight());
	...
	gSPTextureRectangle(gdl++,
			...
			G_TX_RENDERTILE,
			0,
			MP_TERMINATION_MENU_BG_TEX_HEIGHT << 5,
			dsdx,
			dtdy);
...
Gfx *menuRender(Gfx *gdl)
{
	...
	if (menuShouldRenderTerminationStageSelectBackground()) {
		gdl = menuRenderTerminationStageSelectBackground(gdl);
	}
	...
	if (g_MenuData.root == MENUROOT_MPENDSCREEN && mpLmsCuratedHasResult()) {
		gdl = mpRenderLmsResultBackground(gdl);
	}
...
{
	if ((u8)g_Menus[g_MpPlayerNum].fm.device3 == SAVEDEVICE_GAMEPAK) {
