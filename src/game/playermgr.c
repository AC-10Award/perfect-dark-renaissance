...
s32 playermgrGetModelOfWeapon(s32 weapon)
{
	...
	case MD_WEAPON_MATRIX85:      model = MODEL_CHRLEEGUN1; break;
...
static s32 playermgrGetChrWeaponNum(s32 hand)
{
	s32 weaponnum = bgunGetWeaponNum(hand);

	if (weaponnum == WEAPON_FALCON2
			&& g_Vars.currentplayer->hands[hand].gset.weaponfunc == FUNC_SECONDARY) {
...
void playermgrCreateWeapon(s32 hand)
{
	...
	if (chr->weapons_held[hand] == NULL) {
		s32 weaponnum = playermgrGetChrWeaponNum(hand);
