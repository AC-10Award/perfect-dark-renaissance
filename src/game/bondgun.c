...
#define NUM_DELAYED_CHR_IMPACTS 8

#ifndef PLATFORM_N64
#define MD_MATRIX85_RELOAD_SFX      "Reload-Matrix85.wav"
#define MD_MATRIX85_RELOAD_DUAL_SFX "Reload-Matrix85-Dual.wav"

static bool g_MdMatrix85ReloadExtSfxActive[2];

static s32 mdGetPlayerHandIndex(struct hand *hand)
{
...
static bool mdTryHandleMatrix85ReloadSfx(struct hand *hand, struct guncmd *cmd)
{
	const char *filename = NULL;
	s32 handindex;
	bool isfirstreloadchunk;
	bool isreloadchunk;

	if (hand->gset.weaponnum != MD_WEAPON_MATRIX85 || cmd->type != GUNCMD_PLAYSOUND) {
		return false;
	}

	isfirstreloadchunk = cmd->unk04 == SFX_01D8 && (cmd->unk02 == 3 || cmd->unk02 == 5);
	isreloadchunk = cmd->unk04 == SFX_01D8 || cmd->unk04 == SFX_01D9 || cmd->unk04 == SFX_01DB;

#if VERSION >= VERSION_NTSC_1_0
	isreloadchunk = isreloadchunk || cmd->unk04 == SFX_80F6;
#endif
	...
	handindex = mdGetPlayerHandIndex(hand);

	if (isfirstreloadchunk) {
		if (handindex >= 0) {
			g_MdMatrix85ReloadExtSfxActive[handindex] = false;
		}

		filename = cmd->unk02 == 5 ? MD_MATRIX85_RELOAD_DUAL_SFX : MD_MATRIX85_RELOAD_SFX;

		if (extSfxPlayPriority(filename, AL_VOL_FULL, EXT_SFX_PRIORITY_RELOAD)) {
			if (handindex >= 0) {
				g_MdMatrix85ReloadExtSfxActive[handindex] = true;
			}
	...
	return handindex >= 0 && g_MdMatrix85ReloadExtSfxActive[handindex];
}
#endif

struct delayedchrimpactsound {
	struct prop *prop;
	s16 soundnum;
	s16 delay;
};

void bgunQueueDelayedChrImpactSound(struct prop *prop, s16 soundnum, s16 delay);
void bgunTickDelayedChrImpactSounds(void);
...
struct delayedchrimpactsound g_DelayedChrImpactSounds[NUM_DELAYED_CHR_IMPACTS];
...
void bgun0f098030(struct hand *hand, struct modeldef *modeldef)
{
	...
	struct weapon *visweapon = weapon;
	s32 i;
	s32 j;

	if (hand->gset.weaponnum == WEAPON_FALCON2
			&& hand->gset.weaponfunc == FUNC_SECONDARY) {
		struct weapon *silencedweapon = weaponFindById(WEAPON_FALCON2_SILENCER);

		if (silencedweapon != NULL) {
			visweapon = silencedweapon;
		}
	}

	bgunExecuteGunVisCommands(hand, modeldef, visweapon->gunviscmds);
...
#ifndef PLATFORM_N64
								if (mdTryHandleMatrix85ReloadSfx(hand, cmd)) {
									hasspeed = false;
									break;
								}
#endif
...
void bgunGetWeaponInfo(struct handweaponinfo *info, s32 handnum)
{
	...
	info->definition = weaponFindById(weaponnum);
...
s32 bgunTickIncChangeFunc(struct handweaponinfo *info, s32 handnum, struct hand *hand, s32 lvupdate)
{
	...
	if (hand->gset.weaponnum == WEAPON_FALCON2) {
		if (hand->stateminor == 0) {
			cmd = handGetUnequipAnim(&hand->gset);
			if (!extSfxPlay(MD_SFX_FALCON_SILENCER_TOGGLE, AL_VOL_FULL)) {
				sndStart(var80095200, SFX_0022, NULL, -1, -1, -1, -1, -1);
			}
			...
			hand->stateminor = 1;
		} else if (hand->stateminor == 1) {
			if (hand->animmode == HANDANIMMODE_BUSY) {
				more = true;
			} else {
				hand->gset.weaponfunc = 1 - hand->gset.weaponfunc;
				...
				cmd = handGetEquipAnim(&hand->gset);
	...
	if (hand->gset.weaponnum == WEAPON_DMC) {
		s32 delay = g_Vars.normmplayerisrunning ? TICKS(24) : TICKS(32);
		s32 bottompause = g_Vars.normmplayerisrunning ? TICKS(8) : TICKS(10);
		s32 raisedelay = g_Vars.normmplayerisrunning ? TICKS(24) : TICKS(32);
		...
		if (hand->stateminor == 1) {
			if (hand->stateframes >= delay || !hand->visible) {
				bgunSetArmPitch(hand, MAX_PITCH);
				hand->stateminor = 2;
				hand->stateframes = 0;
			} else {
		...
		if (hand->stateminor == 2) {
			...
			if (hand->stateframes >= bottompause) {
				hand->gset.weaponfunc = 1 - hand->gset.weaponfunc;
				hand->stateminor = 3;
				hand->stateframes = 0;
			} else {
		...
		if (hand->stateminor == 3) {
			if (hand->stateframes >= raisedelay
					|| !weaponGetFileNum2(info->weaponnum)
					|| !weaponHasFlag(info->weaponnum, WEAPONFLAG_00000040)
					|| weaponHasFlag(info->weaponnum, WEAPONFLAG_00000080)) {
				hand->mode = HANDMODE_NONE;
				...
				bgunSetArmPitch(hand, (raisedelay - hand->stateframes) * MAX_PITCH / raisedelay);
				more = true;
			}
		}
	...
	if (hand->firing) {
		...
		if (playsound) {
			...
			if (gsetGetSingleShootSound(&hand->gset)) {
				...
				bool playstockshotsound = true;

					if (mdWeaponAudioPlayFire(&hand->gset, AL_VOL_FULL)) {
						playstockshotsound = false;
					}

					if (playstockshotsound && hand->audiohandle2 == NULL) {
						handle = sndStart(var80095200, gsetGetSingleShootSound(&hand->gset), &hand->audiohandle2, -1, -1, -1, -1, -1);
					} else if (playstockshotsound && hand->audiohandle3 == NULL) {
...
void bgunTickHand(s32 handnum)
{
	...
	if (handnum == HAND_RIGHT) {
		bgunTickDelayedChrImpactSounds();
	}
...
void bgunTickSwitch2(void)
{
	...
	if (ctrl->switchtoweaponnum >= 0) {
		if (bgunCanFreeWeapon(HAND_RIGHT) && bgunCanFreeWeapon(HAND_LEFT)) {
			...
			/*
			 * Falcon silencer mode should only change when the player explicitly
			 * toggles it, not via the MP "remember alt function" bitfield that
			 * other weapons use across equips.
			 */
			if (ctrl->weaponnum == WEAPON_FALCON2) {
				g_PlayerConfigsArray[g_Vars.currentplayerstats->mpindex].gunfuncs[(ctrl->weaponnum - 1) >> 3] &=
					~(1 << ((ctrl->weaponnum - 1) & 7));
			}
...
u8 g_AutoSwitchWeaponsPrimary[] = {
	MD_WEAPON_RCP120,
	WEAPON_RCP45,
	MD_WEAPON_SUPERDRAGON, // primary function
	MD_WEAPON_K7_AVENGER,
	MD_WEAPON_AR34,
	WEAPON_AR53,
	WEAPON_KF7SPECIAL,
	MD_WEAPON_CALLISTO_NTG,
	MD_WEAPON_LAPTOP_GUN,
	MD_WEAPON_DRAGON,
	MD_WEAPON_CMP150,
	MD_WEAPON_CYCLONE,
	WEAPON_ZZT,
	WEAPON_DMC,
	WEAPON_KL01313,
	MD_WEAPON_FARSIGHT_XR20,
	MD_WEAPON_SHOTGUN,
	MD_WEAPON_REAPER,
	MD_WEAPON_DY357_LX,
	MD_WEAPON_MAULER,
	MD_WEAPON_DY357_MAGNUM,
	MD_WEAPON_MATRIX85,
	WEAPON_MAGSEC4,
	MD_WEAPON_PHOENIX,
	MD_WEAPON_FALCON2_SCOPE,
	MD_WEAPON_FALCON2,
	MD_WEAPON_FALCON2_SILENCER,
	WEAPON_PP9I,
	WEAPON_CC13,
	MD_WEAPON_SNIPER_RIFLE,
	WEAPON_CROSSBOW,
	WEAPON_TRANQUILIZER,
	WEAPON_LASER,
	MD_WEAPON_SUPERDRAGON, // secondary function
	MD_WEAPON_DEVASTATOR,
...
void bgunQueueDelayedChrImpactSound(struct prop *prop, s16 soundnum, s16 delay)
{
	...
	for (i = 0; i < ARRAYCOUNT(g_DelayedChrImpactSounds); i++) {
		if (g_DelayedChrImpactSounds[i].delay <= 0) {
			g_DelayedChrImpactSounds[i].prop = prop;
			g_DelayedChrImpactSounds[i].soundnum = soundnum;
			g_DelayedChrImpactSounds[i].delay = delay;
			return;
		}
	}
}
...
void bgunTickDelayedChrImpactSounds(void)
{
	...
	for (i = 0; i < ARRAYCOUNT(g_DelayedChrImpactSounds); i++) {
		if (g_DelayedChrImpactSounds[i].delay > 0) {
			g_DelayedChrImpactSounds[i].delay -= g_Vars.lvupdate60;

			if (g_DelayedChrImpactSounds[i].delay <= 0 && g_DelayedChrImpactSounds[i].prop != NULL) {
				struct sndstate **handle = bgunAllocateAudioHandle();
				struct prop *prop = g_DelayedChrImpactSounds[i].prop;
				s16 soundnum = g_DelayedChrImpactSounds[i].soundnum;
				...
				g_DelayedChrImpactSounds[i].prop = NULL;
			}
		}
	}
}
	...
	if (handle) {
		if (prop->type == PROPTYPE_CHR || prop->type == PROPTYPE_PLAYER) {
			...
			if (chrGetShield(chr) > 0) {
				...
				s16 sounds[] = { SFX_0045, SFX_004B, SFX_00D9 };
				soundnum = sounds[rand1 % ARRAYCOUNT(sounds)];
				delayimpact = true;
			}
			...
			if (soundnum != -1) {
				if (delayimpact) {
					bgunQueueDelayedChrImpactSound(prop, soundnum, TICKS(3));
				} else {
...
s32 bgunConsiderToggleGunFunction(s32 usedowntime, bool trigpressed, bool fromactivemenu, bool fromdedicatedbutton)
{
	...
	case MD_WEAPON_MATRIX85:
		// Matrix-85 is outside the original saved gunfunc bitfield range.
		// Keep its automatic secondary mode as a per-equip runtime toggle.
		if (!trigpressed) {
	...
	case WEAPON_DMC:
		...
		if (g_Vars.currentplayer->hands[HAND_RIGHT].gset.weaponnum == WEAPON_DMC) {
			bgunSetState(HAND_RIGHT, HANDSTATE_CHANGEFUNC);
		}
...
{
	if (bgunGetWeaponNum(HAND_RIGHT) == MD_WEAPON_MATRIX85) {
		return;
	}
...
bool bgunIsUsingSecondaryFunction(void)
{
	...
	if (weaponnum == MD_WEAPON_MATRIX85) {
		return player->gunctrl.invertgunfunc;
	}

	if (weaponnum == WEAPON_DMC && player->hands[HAND_RIGHT].gset.weaponnum == WEAPON_DMC) {
		return player->hands[HAND_RIGHT].gset.weaponfunc == FUNC_SECONDARY;
	}
