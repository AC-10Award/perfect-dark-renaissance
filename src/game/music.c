...
bool g_MusicMpDeathDucking = false;
u16 g_MusicMpDeathSavedVolume = 0x5000;
...
static void musicSetTrackVolume(s32 tracktype, u16 volume)
{
	s32 i;
	...
	for (i = 0; i < ARRAYCOUNT(g_SeqChannels); i++) {
		if (g_SeqChannels[i].tracktype == tracktype && g_SeqChannels[i].inuse) {
			seqSetVolume(&g_SeqInstances[i], volume);
		}
	}
}
...
void musicReset(void)
{
	...
	if (!g_SndDisabled) {
		...
		g_MusicMpDeathDucking = false;
		g_MusicMpDeathSavedVolume = musicGetVolume();
	}
}
...
void musicStartMpDeath(void)
{
	...
	duckvolume = musicGetVolume() / 3;

	if (duckvolume < 0x0800) {
		duckvolume = 0x0800;
	}

	if (!g_MusicMpDeathDucking) {
		g_MusicMpDeathSavedVolume = musicGetVolume();
		g_MusicMpDeathDucking = true;
	}

	if (g_MusicNrgIsActive) {
		musicSetTrackVolume(TRACKTYPE_NRG, duckvolume);
	} else {
		musicSetTrackVolume(TRACKTYPE_PRIMARY, duckvolume);
	}
...
void musicEndDeath(void)
{
	...
	if (g_MusicMpDeathDucking) {
		if (g_MusicNrgIsActive) {
			musicSetTrackVolume(TRACKTYPE_NRG, g_MusicMpDeathSavedVolume);
		} else {
			musicSetTrackVolume(TRACKTYPE_PRIMARY, g_MusicMpDeathSavedVolume);
		}
	}
