...
static s32 bmoveGetAimModeTurnThreshold(void)
{
	f32 bounds = PLAYER_EXTCFG().aimstickbounds;
	...
	return 60 + (s32)(bounds * 19.0f + 0.5f);
}

static f32 bmoveGetAimModeReticleLimit(void)
{
	f32 bounds = PLAYER_EXTCFG().aimstickbounds;
	...
	return 0.65f + bounds * 0.35f;
}

static f32 bmoveGetAimModeMouseTurnEdge(void)
{
	f32 edge = bmoveGetAimModeReticleLimit();
...
static f32 bmoveGetAimModeManualDamp(void)
{
	f32 speed = PLAYER_EXTCFG().aimstickspeed;
	...
	return 0.99f - speed * 0.045f;
}

static f32 bmoveGetAimModeTurnSpeed(s32 rawvalue)
{
	s32 threshold = bmoveGetAimModeTurnThreshold();
	s32 magnitude = rawvalue < 0 ? -rawvalue : rawvalue;
	s32 excess = magnitude - threshold;

	if (excess <= 0) {
		return 0.0f;
	}

	return (excess / 20.0f) * PLAYER_EXTCFG().aimstickspeed;
}
...
bool bmoveIsAutoAimYEnabled(void)
{
	...
	// In MP, Super Auto Aim should act as its own enable state rather than
	// requiring the standard Auto-Aim checkbox to also be on.
	return optionsGetAutoAim(g_Vars.currentplayerstats->mpindex)
#ifndef PLATFORM_N64
		|| PLAYER_EXTCFG().superautoaim
#endif
		;
}
	...
	if (g_Vars.currentplayer->pausemode == PAUSEMODE_UNPAUSED) {
		if (g_Vars.currentplayer->isdead == false) {
			if (controlmode == CONTROLMODE_23 || controlmode == CONTROLMODE_24 || controlmode == CONTROLMODE_22 || controlmode == CONTROLMODE_21) {
				...
				if (scenarioIsPlayerInputLocked(g_Vars.currentplayernum)) {
				...
				if (optionsGetAimControl(g_Vars.currentplayerstats->mpindex) == AIMCONTROL_HOLD) {
					bool prevaimmode = g_Vars.currentplayer->insightaimmode;
					...
					if (prevaimmode != g_Vars.currentplayer->insightaimmode) {
				...
				if (!lvIsPaused()) {
					...
					if (g_Vars.currentplayer->insightaimmode && movedata.c1stickyraw > bmoveGetAimModeTurnThreshold()) {
						movedata.speedvertadown = bmoveGetAimModeTurnSpeed(movedata.c1stickyraw);
					...
					if (g_Vars.currentplayer->insightaimmode && movedata.c1stickyraw < -bmoveGetAimModeTurnThreshold()) {
						movedata.speedvertaup = bmoveGetAimModeTurnSpeed(movedata.c1stickyraw);
					...
					if (g_Vars.currentplayer->insightaimmode && movedata.c1stickxraw < -bmoveGetAimModeTurnThreshold()) {
						movedata.aimturnleftspeed = bmoveGetAimModeTurnSpeed(movedata.c1stickxraw);
					...
					if (g_Vars.currentplayer->insightaimmode && movedata.c1stickxraw > bmoveGetAimModeTurnThreshold()) {
						movedata.aimturnrightspeed = bmoveGetAimModeTurnSpeed(movedata.c1stickxraw);
				...
				if (!lvIsPaused()) {
					...
					} else if (g_Vars.currentplayer->insightaimmode && movedata.c1stickyraw < -bmoveGetAimModeTurnThreshold()) {
						movedata.speedvertaup = bmoveGetAimModeTurnSpeed(movedata.c1stickyraw);
					...
					} else if (g_Vars.currentplayer->insightaimmode && movedata.c1stickxraw > bmoveGetAimModeTurnThreshold()) {
						movedata.aimturnrightspeed = bmoveGetAimModeTurnSpeed(movedata.c1stickxraw);
					...
					if (g_Vars.currentplayer->insightaimmode && allowmcross && bgunGetWeaponNum(HAND_RIGHT) != WEAPON_HORIZONSCANNER) {
						f32 edge = bmoveGetAimModeMouseTurnEdge();
						f32 edgewidth = 1.0f - edge;

						if (g_Vars.currentplayer->swivelpos[0] > edge) {
							movedata.aimturnrightspeed = (g_Vars.currentplayer->swivelpos[0] - edge) / edgewidth;
							movedata.aimturnleftspeed = 0.f;
						} else if (g_Vars.currentplayer->swivelpos[0] < -edge) {
							movedata.aimturnleftspeed = (g_Vars.currentplayer->swivelpos[0] + edge) / -edgewidth;
							movedata.aimturnrightspeed = 0.f;
						}
						f32 vertaup = 0.f, vertadown = 0.f;
						if (g_Vars.currentplayer->swivelpos[1] > edge) {
							vertaup = (g_Vars.currentplayer->swivelpos[1] - edge) / edgewidth;
						} else if (g_Vars.currentplayer->swivelpos[1] < -edge) {
							vertadown = (g_Vars.currentplayer->swivelpos[1] + edge) / -edgewidth;
						}
	...
	if (g_Vars.currentplayer->unk1c64) {
		...
		if (
			...
			if (g_Vars.currentplayer->autoaimdamp > (PAL ? 0.955f : 0.963f)) {
				f32 step = (PAL ? 0.00037999986670911f : 0.00031999943894334f) * g_Vars.lvupdate60freal;

#ifndef PLATFORM_N64
				if (PLAYER_EXTCFG().superautoaim) {
					step *= 10.0f;
				}
#endif

				g_Vars.currentplayer->autoaimdamp -= step;
			}

			if (g_Vars.currentplayer->autoaimdamp < (
#ifndef PLATFORM_N64
					PLAYER_EXTCFG().superautoaim ? (PAL ? 0.66f : 0.68f) :
#endif
					(PAL ? 0.955f : 0.963f))) {
...
#ifndef PLATFORM_N64
				if (PLAYER_EXTCFG().superautoaim) {
					bgunSwivel(x, y, PAL ? 0.60f : 0.62f, PAL ? 0.60f : 0.62f);
				} else
#endif
...
#endif
		{
			f32 limit = bmoveGetAimModeReticleLimit();
			f32 damp = bmoveGetAimModeManualDamp();

			bgunSwivel(
					(movedata.c1stickxraw / 80.0f) * limit,
					(movedata.c1stickyraw / 80.0f) * limit,
					damp,
					damp);
		}
	}
...
void bmoveTick(bool allowc1x, bool allowc1y, bool allowc1buttons, bool ignorec2)
{
	...
	if ((g_Vars.currentplayer->bondmovemode == MOVEMODE_WALK || g_Vars.currentplayer->bondmovemode == MOVEMODE_GRAB)
			...
			&& (!g_Vars.normmplayerisrunning || PLAYERCOUNT() == 1 || optionsGetMpFootsteps())) {
