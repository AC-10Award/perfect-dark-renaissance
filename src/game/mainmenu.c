...
extern MenuItemHandlerResult menuhandlerDisableMpDeathMusic(s32 operation, struct menuitem *item, union handlerdata *data);
extern MenuItemHandlerResult menuhandlerSfxBrowserSound(s32 operation, struct menuitem *item, union handlerdata *data);
extern MenuDialogHandlerResult menudialogExtendedAudio(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data);
#endif
...
MenuItemHandlerResult menuhandlerMpFootsteps(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GET:
		return optionsGetMpFootsteps();
	case MENUOP_SET:
		optionsSetMpFootsteps(data->checkbox.value);
		g_Vars.modifiedfiles |= MODFILE_GAME;
	}
...
{
	if (operation == MENUOP_SET) {
		filemgrConsiderPushingFileSelectDialog();
	}
...
{
	if (operation == MENUOP_SET) {
		sysLogPrintf(LOG_ERROR, "exit path: menuhandlerExitGame -> exit(0)");
		exit(0);
	}
...
struct menuitem g_AudioOptionsMenuItems[] = {
...
#ifndef PLATFORM_N64
	{
		...
		(intptr_t)"Multiplayer Footsteps\n",
		0,
		menuhandlerMpFootsteps,
	},
	{
		MENUITEMTYPE_CHECKBOX,
		0,
		MENUITEMFLAG_LITERAL_TEXT,
		(intptr_t)"Disable MP Death Music",
		0,
...
MenuItemHandlerResult menuhandlerMainMenuCombatSimulator(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		...
		func0f0f8300();
		sndStart(var80095200, SFX_0067, NULL, -1, -1, -1, -1, -1);
	}
