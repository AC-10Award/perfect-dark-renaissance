...
static bool setupIsCrashAuditStage(s32 stagenum)
{
...
#ifndef PLATFORM_N64
static void setupCrashAuditLog(const char *format, ...)
{
...
void setupLoadFiles(s32 stagenum)
{
	...
	if (stagenum < STAGE_TITLE) {
		if (setupIsCrashAuditStage(stagenum)) {
			setupCrashAuditLog("map-crash: setupLoadFiles begin stagenum=%02x mp=%d stageindex=%d\n",
					stagenum, g_Vars.normmplayerisrunning ? 1 : 0, g_StageIndex);
		}
		...
		if (setupIsCrashAuditStage(stagenum)) {
			setupCrashAuditLog("map-crash: setup files loaded stagenum=%02x setupfile=%d padsfile=%d intro=%p props=%p paths=%p ailists=%p pads=%p\n",
					stagenum,
					filenum,
					g_Stages[g_StageIndex].padsfileid,
					g_StageSetup.intro,
					g_StageSetup.props,
					g_StageSetup.paths,
					g_StageSetup.ailists,
		...
		if (g_Vars.normmplayerisrunning) {
			numchrs += mpGetNumChrs() - PLAYERCOUNT();
			numchrs += scenarioGetExtraChrSlots();
			numobjs += scenarioNumProps();
		}

		if (setupIsCrashAuditStage(stagenum)) {
			setupCrashAuditLog("map-crash: slot sizing stagenum=%02x numobjs=%d numchrs=%d scenario=%d players=%d mpchrs=%d extraprops=%d extrachrs=%d\n",
					stagenum,
					numobjs,
					numchrs,
					g_MpSetup.scenario,
					PLAYERCOUNT(),
					g_Vars.normmplayerisrunning ? mpGetNumChrs() : 0,
					g_Vars.normmplayerisrunning ? scenarioNumProps() : 0,
					g_Vars.normmplayerisrunning ? scenarioGetExtraChrSlots() : 0);
		}
	...
	if (stagenum < STAGE_TITLE && setupIsCrashAuditStage(stagenum)) {
		setupCrashAuditLog("map-crash: setupLoadFiles end stagenum=%02x maxprops=%d extra=%d\n",
				stagenum, g_Vars.maxprops, extra);
	}
}
...
void setupCreateProps(s32 stagenum)
{
	...
	if (stagenum < STAGE_TITLE) {
		if (setupIsCrashAuditStage(stagenum)) {
			setupCrashAuditLog("map-crash: setupCreateProps begin stagenum=%02x withchrs=%d withobjs=%d props=%p pads=%p\n",
					stagenum, withchrs, withobjs, g_StageSetup.props, g_StageSetup.padfiledata);
		}
		...
		if (setupIsCrashAuditStage(stagenum)) {
			setupCrashAuditLog("map-crash: pads+waypoints ready stagenum=%02x\n", stagenum);
		}
		...
		if (withchrs) {
				...
				if (setupIsCrashAuditStage(stagenum)) {
					setupCrashAuditLog("map-crash: chrmgr configured stagenum=%02x numchrs=%d\n", stagenum, numchrs);
				}
		} else {
			chrmgrConfigure(0);
		}
		...
		if (g_StageSetup.props) {
			...
			if (setupIsCrashAuditStage(stagenum)) {
				setupCrashAuditLog("map-crash: prop loop begin stagenum=%02x\n", stagenum);
			}
			...
			if (g_Vars.normmplayerisrunning && mpHasSimulants()) {
				u32 stack[4];
				s32 i;
				s32 slotsdone[MAX_BOTS_RUNTIME];
				s32 chrnum = 0;
				s32 maxsimulants;
				s32 slotnum;

				if (challengeIsFeatureUnlocked(MPFEATURE_8BOTS)) {
					maxsimulants = MAX_BOTS_RUNTIME;
				} else {
					maxsimulants = 4;
				}

				if (scenarioGetReservedSimulantSlots() > 0 && maxsimulants > scenarioGetReservedSimulantSlots()) {
					maxsimulants -= scenarioGetReservedSimulantSlots();
				} else if (scenarioGetReservedSimulantSlots() > 0) {
					maxsimulants = 0;
				}
				...
				for (i = 0; i < maxsimulants; i++) {
					...
					if ((g_MpSetup.chrslots & MPCHRSLOT_BIT(slotnum + 4))
...
#ifndef PLATFORM_N64
				sysLogPrintf(LOG_NOTE, "setupCreateProps: requested_maxsimulants=%d allocated_bots=%d total_mpchrs=%d", maxsimulants, g_BotCount, g_MpNumChrs);
#endif
			}
