...
struct prop *shotCalculateHits(s32 handnum, bool isshooting, struct coord *gunpos2d, struct coord *gundir2d, struct coord *gunpos3d, struct coord *gundir3d, u32 arg6, f32 distance, bool cheap)
{
	...
	if (isshooting) {
			...
			if (hitprop && !(laserstream && shotdata.hits[i].distance > 300)) {
				...
				if (shotdata.hits[i].bulletproof) {
					...
					hitpos.z = shotdata.hits[i].pos.z;
						...
						|| (explosiveshells && obj && (obj->type == OBJTYPE_GLASS || obj->type == OBJTYPE_TINTEDGLASS))) {
					s1++;
		...
		if (hitbg && !blockedbyprop) {
			...
			bgApplyGeoDamageTexture(room, &sp694);
			bgRecordGeoDamageHit(room, &sp694);
			bgSpawnMetalHitSpark(room, &sp694);
			bgSpawnWallHit(room, &sp694, &shotdata.gunpos3d);
			...
			if (surfacetype->numwallhittexes > 0 && (!func || (func->type & 0xff) != INVENTORYFUNCTYPE_MELEE)) {
				...
				if (!mdMaterialImpactTryPlayCustomBgMetalHit(g_Vars.stagenum, sp694.texturenum)
						&& !mdMaterialImpactTryPlayCustomBgWallHit(g_Vars.stagenum, sp694.texturenum))
				{
				...
				if (explosiveshells) {
					...
					if (PLAYERCOUNT() <= 2 || g_Vars.lvupdate240 <= 8 || (rngRandom() % 4) == 0) {
						if (sp694.pos.x > -32000 && sp694.pos.x < 32000
							...
							// Disabled stock triangle-spark surface impacts while custom tile-hit puffs are being prototyped.
...
static bool propIsMpPickupRespawnTarget(struct prop *prop)
{
	if (!g_Vars.normmplayerisrunning || prop == NULL || prop->obj == NULL) {
		return false;
	}
...
u32 propGetRespawnTimeTicks(struct prop *prop)
{
	if (!propIsMpPickupRespawnTarget(prop)) {
		return TICKS(1200);
	}

	switch (g_MpPickupRespawnSpeed) {
	case 1:
		return TICKS(600);
	case 2:
		return TICKS(300);
...
bool propShouldPlayRespawnSound(struct prop *prop)
{
	if (propIsMpPickupRespawnTarget(prop) && !g_MpPickupRespawnSfx) {
		return false;
	}
...
#ifndef PLATFORM_N64
				if (sp50) {
					// Super Auto Aim should pick up any valid target that is
					// already on-screen, not just targets near the crosshair.
					sp48 = camGetScreenWidth();
				}
#endif
