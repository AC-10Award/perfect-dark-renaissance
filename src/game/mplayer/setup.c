...
#include "mpsetups.h"
#include "mpplayers.h"
#include "md/mp_menu_aliases.h"
#include "md/mp_stage_aliases.h"


char *func0f17d3dc(struct menuitem *item);
MenuItemHandlerResult menuhandlerMpSimulantName(s32 operation, struct menuitem *item, union handlerdata *data);
MenuItemHandlerResult menuhandlerMpRandomSimulantName(s32 operation, struct menuitem *item, union handlerdata *data);
...
struct menudialogdef g_MpChangeSimulantNameMenuDialog;
struct menudialogdef g_MpChangeTeamNameMenuDialog;
struct menudialogdef g_MpEditSimulantMenuDialog;
struct menudialogdef g_MpLastManStandingDifficultyMenuDialog;
struct menudialogdef g_MpTerminationMenuDialog;
...
static MenuItemHandlerResult menuhandlerMpProfileErrorOk(s32 operation, struct menuitem *item, union handlerdata *data);
static MenuDialogHandlerResult mpLoadPlayerDialogHandler(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data);

#define MPCAROUSEL_SETFLAG_SHOULDER 0x01
#define MPCAROUSEL_SETFLAG_CONFIRM  0x02

static char g_MpPlayerProfileErrorText[96];

#define MP_CHARACTER_ORIGINAL_BODY_COUNT 0x3f
#define MP_CHARACTER_REMOVED_BODY_COUNT 1
#define MP_CHARACTER_BODY_COUNT (MP_CHARACTER_ORIGINAL_BODY_COUNT - MP_CHARACTER_REMOVED_BODY_COUNT + 2)

static bool mpCharacterBodyIsRemoved(s32 mpbodynum)
{
	return mpbodynum == MPBODY_STRIPES;
}

static s32 mpCharacterBodyUiToMpbodynum(s32 uiindex)
{
	s32 mpbodynum;
	s32 displayindex = 0;

	for (mpbodynum = 0; mpbodynum < MP_CHARACTER_ORIGINAL_BODY_COUNT; mpbodynum++) {
		if (mpCharacterBodyIsRemoved(mpbodynum)) {
			continue;
		}

		if (displayindex++ == uiindex) {
			return mpbodynum;
		}
	}

	return uiindex == MP_CHARACTER_ORIGINAL_BODY_COUNT - MP_CHARACTER_REMOVED_BODY_COUNT
			? MPBODY_EVIL_CYBORG
			: MPBODY_ROBOT_SKELINGTON;
}

static s32 mpCharacterBodyMpbodynumToUi(s32 mpbodynum)
{
	if (mpbodynum == MPBODY_EVIL_CYBORG) {
		return MP_CHARACTER_ORIGINAL_BODY_COUNT - MP_CHARACTER_REMOVED_BODY_COUNT;
	}

	if (mpbodynum == MPBODY_ROBOT_SKELINGTON) {
		return MP_CHARACTER_ORIGINAL_BODY_COUNT - MP_CHARACTER_REMOVED_BODY_COUNT + 1;
	}

	if (!mpCharacterBodyIsRemoved(mpbodynum) && mpbodynum < MP_CHARACTER_ORIGINAL_BODY_COUNT) {
		s32 curmpbodynum;
		s32 displayindex = 0;

		for (curmpbodynum = 0; curmpbodynum < mpbodynum; curmpbodynum++) {
			if (!mpCharacterBodyIsRemoved(curmpbodynum)) {
				displayindex++;
			}
		}
...
static void mpCharacterSelectionConfirmed(s32 mpheadnum, bool isplayer)
{
	/* Hook point for future per-head confirmation SFX. */
	(void) mpheadnum;
	(void) isplayer;
...
static struct menuitem g_MpPlayerProfileErrorMenuItems[] = {
	{
		MENUITEMTYPE_LABEL,
		0,
		MENUITEMFLAG_LITERAL_TEXT | MENUITEMFLAG_LESSLEFTPADDING,
		(uintptr_t)g_MpPlayerProfileErrorText,
		0,
		NULL,
	},
	{
		...
		L_OPTIONS_347, // "OK"
		0,
		menuhandlerMpProfileErrorOk,
	},
	{ MENUITEMTYPE_END },
};

static struct menudialogdef g_MpPlayerProfileErrorMenuDialog = {
	MENUDIALOGTYPE_DANGER,
	L_OPTIONS_277, // "Failed"
	g_MpPlayerProfileErrorMenuItems,
	NULL,
...
struct mparena g_MpArenas[] = {
	...
	{ STAGE_MP_RANDOM_GEX,   0, MD_MPSTAGE_TEXT_RANDOM_GOLDENEYE_X }, // Random GoldenEye X
#endif
	{ 1,                   0,                          L_MPMENU_136 }, // "Random"
};

static char *mpGetArenaDisplayNameByStage(s32 stagenum, u16 name)
{
	switch (stagenum) {
	case STAGE_EXTRA6:
		return "Temple GE64";
		...
		for (i = 0; i < ARRAYCOUNT(g_MpArenas); i++) {
			if (challengeIsFeatureUnlocked(g_MpArenas[i].requirefeature)) {
				if (count == data->list.value) {
					return (uintptr_t)mpGetArenaDisplayNameByStage(g_MpArenas[i].stagenum, g_MpArenas[i].name);
				}
...
MenuItemHandlerResult menuhandlerMpSuperAutoAim(s32 operation, struct menuitem *item, union handlerdata *data)
{
#ifndef PLATFORM_N64
	switch (operation) {
	case MENUOP_CHECKDISABLED:
		return (g_MpSetup.options & MPOPTION_NOAUTOAIM) != 0;
	case MENUOP_GET:
		return g_PlayerExtCfg[g_MpPlayerNum & 3].superautoaim;
	case MENUOP_SET:
		g_PlayerExtCfg[g_MpPlayerNum & 3].superautoaim = data->checkbox.value;
		break;
	}
#endif
...
MenuItemHandlerResult menuhandlerMpOneHitBots(s32 operation, struct menuitem *item, union handlerdata *data)
{
#ifndef PLATFORM_N64
	switch (operation) {
	case MENUOP_GET:
		return g_OneHitBots;
	case MENUOP_SET:
		g_OneHitBots = data->checkbox.value;
		break;
	}
#endif
...
MenuItemHandlerResult menuhandlerMpPickupRespawnSpeed(s32 operation, struct menuitem *item, union handlerdata *data)
{
#ifndef PLATFORM_N64
	...
	case MENUOP_SET:
		g_MpPickupRespawnSpeed = data->dropdown.value;
		break;
	case MENUOP_GETSELECTEDINDEX:
		data->dropdown.value = g_MpPickupRespawnSpeed;
		break;
	}
#endif
...
MenuItemHandlerResult menuhandlerMpPickupRespawnSfx(s32 operation, struct menuitem *item, union handlerdata *data)
{
#ifndef PLATFORM_N64
	switch (operation) {
	case MENUOP_GET:
		return g_MpPickupRespawnSfx;
	case MENUOP_SET:
		g_MpPickupRespawnSfx = data->checkbox.value;
		break;
	}
#endif
...
MenuItemHandlerResult menuhandlerMpSpHitReactions(s32 operation, struct menuitem *item, union handlerdata *data)
{
#ifndef PLATFORM_N64
	switch (operation) {
	case MENUOP_GET:
		return g_MpSpHitReactions;
	case MENUOP_SET:
		g_MpSpHitReactions = data->checkbox.value;
		break;
	}
#endif
...
MenuItemHandlerResult menuhandlerMpConfirmSaveChr(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		...
		menuPopDialog();
		mpplayersLoadCurrentFile();
		slotindex = mpplayersSavePlayer(g_MpPlayerNum, -1);

		if (slotindex >= 0) {
			g_PlayerConfigsArray[g_MpPlayerNum].fileguid.fileid = slotindex + 1;
			g_PlayerConfigsArray[g_MpPlayerNum].fileguid.deviceserial = 1;
			menuPushDialog(&g_FilemgrFileSavedMenuDialog);
		} else {
			strcpy(g_MpPlayerProfileErrorText, "No more player profile slots.\n");
			menuPushDialog(&g_MpPlayerProfileErrorMenuDialog);
		}
	}
...
MenuItemHandlerResult mpCharacterBodyMenuHandler(s32 operation, struct menuitem *item, union handlerdata *data, s32 mpbodynum, s32 mpheadnum, bool isplayer)
{
	switch (operation) {
	case MENUOP_GETOPTIONCOUNT:
		data->carousel.value = MP_CHARACTER_BODY_COUNT;
		break;
	case MENUOP_11:
	...
	case MENUOP_CHECKPREFOCUSED:
		...
		g_Menus[g_MpPlayerNum].menumodel.flags = MENUMODELFLAG_HASSCALE;
		g_Menus[g_MpPlayerNum].menumodel.configuring = false;
		...
		g_Menus[g_MpPlayerNum].menumodel.curscale = 1;
...
{
	union handlerdata mappeddata;

	switch (operation) {
	case MENUOP_GETSELECTEDINDEX:
		data->carousel.value = mpCharacterBodyMpbodynumToUi(g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum);
		return 0;
	case MENUOP_21:
		mappeddata = *data;
		mappeddata.carousel.value = mpCharacterBodyUiToMpbodynum(data->carousel.value);
		return mpCharacterBodyMenuHandler(operation, item, &mappeddata,
...
#if VERSION >= VERSION_NTSC_1_0
			if ((mappeddata.carousel.unk04 & MPCAROUSEL_SETFLAG_SHOULDER) == 0)
#endif
			{
				g_PlayerConfigsArray[g_MpPlayerNum].base.mpheadnum = mpGetMpheadnumByMpbodynum(mappeddata.carousel.value);
			}
		}
		g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum = mappeddata.carousel.value;
		func0f17b8f0();

		if (mappeddata.carousel.unk04 & MPCAROUSEL_SETFLAG_CONFIRM) {
			mpCharacterSelectionConfirmed(g_PlayerConfigsArray[g_MpPlayerNum].base.mpheadnum, true);
		}
		return mpCharacterBodyMenuHandler(operation, item, &mappeddata,
				g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum,
				g_PlayerConfigsArray[g_MpPlayerNum].base.mpheadnum, true);
...
MenuDialogHandlerResult menudialog0017a174(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_OPEN:
		if (mpCharacterBodyIsRemoved(g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum)
				|| (g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum >= MP_CHARACTER_ORIGINAL_BODY_COUNT
					&& g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum != MPBODY_EVIL_CYBORG
					&& g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum != MPBODY_ROBOT_SKELINGTON)) {
			g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum = MPBODY_DARK_COMBAT;
			g_PlayerConfigsArray[g_MpPlayerNum].base.mpheadnum = MPHEAD_DARK_COMBAT;
		}
		break;
	case MENUOP_CLOSE:
		break;
...
MenuItemHandlerResult mpSelectRandomWeaponListHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_SET:
		{
			s32 numweapons = mpGetNumWeaponOptions();
			s32 optionindex = mpGetMpWeaponNumByOptionIndex(data->list.value);
			s32 i;
...
struct menuitem g_MpControlMenuItems[] = {
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)"Super Auto Aim",
		0,
		menuhandlerMpSuperAutoAim,
	},
	{
		MENUITEMTYPE_DROPDOWN,
		0,
...
MenuItemHandlerResult mpCharacterHeadMenuHandler(s32 operation, struct menuitem *item, union handlerdata *data, s32 mpheadnum, bool arg4)
{
	...
	case MENUOP_FOCUS:
		...
		if (mpheadnum < mpGetNumHeads2()) {
			...
			g_Menus[g_MpPlayerNum].menumodel.newparams = MENUMODELPARAMS_SET_FILENUM(g_HeadsAndBodies[headnum].filenum);
			g_Menus[g_MpPlayerNum].menumodel.isperfecthead = false;
			g_Menus[g_MpPlayerNum].menumodel.perfectheadnum = 0;
		} else {
			headnum = mpGetBeauHeadId(func0f14a9f8(mpheadnum - mpGetNumHeads2()));
			g_Menus[g_MpPlayerNum].menumodel.newparams = MENUMODELPARAMS_SET_FILENUM(g_HeadsAndBodies[headnum].filenum);
			g_Menus[g_MpPlayerNum].menumodel.isperfecthead = true;
...
MenuItemHandlerResult menuhandlerMpCharacterHead(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		...
		if (data->carousel.unk04 & MPCAROUSEL_SETFLAG_CONFIRM) {
			mpCharacterSelectionConfirmed(g_PlayerConfigsArray[g_MpPlayerNum].base.mpheadnum, true);
		}
	}
...
{
	u8 presets = g_Menus[g_MpPlayerNum].mpsetup.showpresets ? 1 : 0;
	s32 numpresets = mpGetNumUnlockedPresets()*presets;

	switch (operation) {
	case MENUOP_GETOPTIONCOUNT:
		mpsetupRefreshCurrentFile();
	...
	case MENUOP_GETOPTIONTEXT:
		...
		if (g_MpSetupFile.numsetups > 0) {
			s32 setupindex = data->list.value - numpresets;
			mpsetupRefreshCurrentFile();
			if (setupindex < 0 || setupindex >= g_MpSetupFile.numsetups) {
				return (uintptr_t)"";
			}
			struct setupblock *block = &g_MpSetupFile.setups[setupindex];
	...
	case MENUOP_LISTITEMFOCUS:
		if (presets && data->list.value < mpGetNumUnlockedPresets()) {
			...
			s32 setupindex = data->list.value - numpresets;
			g_Menus[g_MpPlayerNum].mpsetup.slotindex = (setupindex >= 0 && setupindex < g_MpSetupFile.numsetups) ? setupindex : 0xffff;
		}
		break;
	}
...
char *mpMenuTextMpconfigMarquee(struct menuitem *item)
{
	...
	if (g_Menus[g_MpPlayerNum].mpsetup.slotindex < 0xffff && g_MpSetupFile.numsetups > 0) {
		...
		mpsetupfileGetOverview(g_MpSetupFile.setups[g_Menus[g_MpPlayerNum].mpsetup.slotindex].bytes,
				g_MpSetupFile.version, filename, &numsims, &stagenum, &scenarionum);
...
#if VERSION >= VERSION_NTSC_1_0
			if (scenarionum < ARRAYCOUNT(g_MpScenarioOverviews) && arenanum != -1 && numsims >= 0 && filename[0] != '\0' && numsims <= MAX_BOTS_RUNTIME) {
			// "%s:  Scenario: %s   Arena: %s    Simulants: %d"
			sprintf(g_StringPointer, langGet(L_MPMENU_140),
					filename,
					scenarioGetNameByNum(scenarionum),
					mpGetArenaDisplayNameByStage(g_MpArenas[arenanum].stagenum, g_MpArenas[arenanum].name),
					numsims);
		} else {
			return "";
		}
...
MenuItemHandlerResult mpLoadPlayerMenuHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	switch (operation) {
	case MENUOP_GETOPTIONCOUNT:
		data->list.value = mpplayersGetCount();
		break;
	case MENUOP_GETOPTIONTEXT:
		mpplayersGetOverview(data->list.value, name, &playtime);
		for (i = 0; name[i] != '\0'; i++) {
			if (name[i] == '\n') {
				name[i] = '\0';
				break;
		...
		snprintf(g_StringPointer, 64, "%s %02d:%02d\n",
				name,
				playtime / 3600,
				(playtime / 60) % 60);
		return (uintptr_t)g_StringPointer;
	case MENUOP_SET:
		...
		for (i = 0; i < MAX_PLAYERS; i++) {
			if ((data->list.value + 1) == g_PlayerConfigsArray[i].fileguid.fileid
					&& g_PlayerConfigsArray[i].fileguid.deviceserial == 1) {
		...
		if (available) {
			menuPopDialog();
			mpplayersLoadPlayer(g_MpPlayerNum, data->list.value);
		} else {
...
MenuItemHandlerResult menuhandlerMpDeathLimitSlider(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GETSLIDER:
		data->slider.value = g_MpSetup.deathlimit;
		break;
	case MENUOP_SET:
		g_MpSetup.deathlimit = data->slider.value;
		break;
	case MENUOP_GETSLIDERLABEL:
		if (data->slider.value == 0) {
			sprintf(data->slider.label, langGet(L_MPMENU_112)); // "No Limit"
		} else {
			sprintf(data->slider.label, langGet(L_MPMENU_113), data->slider.value); // "%d"
		}
	}
...
MenuItemHandlerResult menuhandlerMpHandicapPlayer(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_CHECKHIDDEN:
					if ((g_MpSetup.chrslots & MPCHRSLOT_BIT(item->param)) == 0) {
			return 1;
		}
		break;
	case MENUOP_GETSLIDER:
...
{
		if (g_MpSetup.chrslots & MPCHRSLOT_BIT(item->param)) {
...
MenuItemHandlerResult menuhandlerMpStartAllPlayers(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_CHECKHIDDEN) {
		if (g_MpPlayerNum != 0 || g_MpNumJoined < 2) {
			return true;
		}
	}

	if (operation == MENUOP_SET) {
		func0f0f820c(&g_MpReadyMenuDialog, -5);
	}

	return 0;
}

MenuItemHandlerResult menuhandlerMpStartAllPlayersDirect(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_CHECKHIDDEN) {
		if (g_MpPlayerNum != 0 || g_MpNumJoined < 2) {
			return true;
	...
	if (operation == MENUOP_SET) {
		func0f0f820c(g_Menus[g_MpPlayerNum].curdialog->definition, -5);
	}
...
MenuDialogHandlerResult menudialogMpSimulant(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	if (operation == MENUOP_TICK) {
		...
		if (slotindex < 0
				|| slotindex >= MAX_BOTS_RUNTIME
				|| (u8)g_BotConfigsArray[slotindex].base.name[0] == '\0') {
			menuPopDialog();
		}
	}
...
{
	if (operation == MENUOP_OPEN) {
		g_Menus[g_MpPlayerNum].mpsetup.showpresets = g_Menus[g_MpPlayerNum].mpsetup.showpresets ? 1 : 0;
...
struct menuitem g_MpCharacterMenuItems[] = {
	...
	},
	{
		...
		0,
		MENUITEMFLAG_CAROUSEL_SELECTCONFIRM,
		0,
		0x00000022,
...
struct menuitem g_MpReadyMenuItems[] = {
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)"Start All Players",
		0,
		menuhandlerMpStartAllPlayers,
	},
	{ MENUITEMTYPE_END },
};
	...
	case MENUOP_GETOPTIONTEXT:
		...
		case MENUOP_SET:
			...
			if (botnum < 0) {
				...
				creating = 1;
			} else if ((g_MpSetup.chrslots & MPCHRSLOT_BIT(botnum + 4)) == 0) {
				creating = 1;
			}

			if (botnum < 0 || botnum >= MAX_BOTS_RUNTIME) {
				return 0;
			}
...
MenuItemHandlerResult menuhandlerMpSimulantHead(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	if (slotindex < 0 || slotindex >= MAX_BOTS_RUNTIME) {
		return 0;
	}
	...
	switch (operation) {
	case MENUOP_SET:
		g_BotConfigsArray[slotindex].base.mpheadnum = start + data->carousel.value;

		if (data->carousel.unk04 & MPCAROUSEL_SETFLAG_CONFIRM) {
			mpCharacterSelectionConfirmed(g_BotConfigsArray[slotindex].base.mpheadnum, false);
		}
	case MENUOP_FOCUS:
		if (operation == MENUOP_FOCUS
				&& item->param2 == 1
				&& g_BotConfigsArray[slotindex].base.mpheadnum < start) {
			g_BotConfigsArray[slotindex].base.mpheadnum = start;
		}
		break;
	}

	return mpCharacterHeadMenuHandler(operation, item, data, g_BotConfigsArray[slotindex].base.mpheadnum, 0);
}
...
MenuItemHandlerResult menuhandlerMpSimulantBody(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	if (operation == MENUOP_GETSELECTEDINDEX) {
		data->carousel.value = mpCharacterBodyMpbodynumToUi(g_BotConfigsArray[slotindex].base.mpbodynum);
		return 0;
	}
	...
	if (operation == MENUOP_21) {
		...
		mappeddata.carousel.value = mpCharacterBodyUiToMpbodynum(data->carousel.value);
		return mpCharacterBodyMenuHandler(operation, item, &mappeddata,
				g_BotConfigsArray[slotindex].base.mpbodynum,
				g_BotConfigsArray[slotindex].base.mpheadnum,
				false);
	}
	...
	if (operation == MENUOP_SET) {
		mappeddata = *data;
		mappeddata.carousel.value = mpCharacterBodyUiToMpbodynum(data->carousel.value);
		g_BotConfigsArray[slotindex].base.mpbodynum = mappeddata.carousel.value;
...
char *mpMenuTitleEditSimulant(struct menudialogdef *dialogdef)
{
	...
	sprintf(g_StringPointer, "%s", g_BotConfigsArray[slotindex].base.name);
	return g_StringPointer;
}
...
MenuItemHandlerResult menuhandlerMpAddSimulant(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	return 0;
			...
			{
				if (item->param < 0 || item->param >= MAX_BOTS_RUNTIME) {
					return operation == MENUOP_CHECKDISABLED;
				}
				...
				if ((g_MpSetup.chrslots & MPCHRSLOT_BIT(item->param + 4)) == 0) {
						menuPushDialog(&g_MpAddSimulantMenuDialog);
		} else if (IS4MB()) {
	...
	return 0;
			...
			char *mpMenuTextSimulantName(struct menuitem *item)
			{
				...
				if (index < 0 || index >= MAX_BOTS_RUNTIME) {
					return "";
				}

				if (g_BotConfigsArray[index].base.name[0] == '\0'
						|| (g_MpSetup.chrslots & MPCHRSLOT_BIT(index + 4)) == 0) {
					return "";
				}
...
struct menuitem g_MpSimulantCharacterMenuItems[] = {
	{
		MENUITEMTYPE_CAROUSEL,
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_CAROUSEL_SELECTCONFIRM,
		0,
		0x00000025,
...
struct menuitem g_MpEditSimulantMenuItems[] = {
	...
	},
	{
		...
		(uintptr_t)"Name...\n",
		0,
		menuhandlerMpSimulantName,
	},
	{
		MENUITEMTYPE_SELECTABLE,
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)"Random Name\n",
		0,
		menuhandlerMpRandomSimulantName,
	},
	{
		MENUITEMTYPE_SEPARATOR,
		0,
...
MenuItemHandlerResult menuhandlerMpMaximumTeams(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		...
		for (i = 0; i != MAX_MPCHRS_RUNTIME; i++) {
				if (g_MpSetup.chrslots & MPCHRSLOT_BIT(i)) {
...
char g_MpNoMusicText[] = "No Music\n";

/**
 * List handler for the select tune dialog.
 *
 * If multiple tracks are disabled, the listing contains the track listing plus
 * items for No Music and Randomize.
 *
 * If multiple tracks are enabled, the listing contains the track listing plus
 * items for No Music, Select All, Select None and Randomize.
 */
MenuItemHandlerResult mpSelectTuneListHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_GETOPTIONTEXT:
		{
			...
			if (data->list.value - numtracks == 0) {
				return (uintptr_t) g_MpNoMusicText;
			}
			...
			return (uintptr_t) langGet(var80085ce8[0]);
		}
	case MENUOP_SET:
		{
			s32 numtracks = mpGetNumUnlockedTracks();
	...
	case MENUOP_GETSELECTEDINDEX:
		if (mpGetUsingMultipleTunes()) {
			...
			if (slotnum == MPTRACK_NONE) {
				data->list.value = mpGetNumUnlockedTracks();
			} else if (slotnum < 0) {
				data->list.value = mpGetNumUnlockedTracks() + 1;
			} else {
	...
	case MENUOP_LISTITEMFOCUS:
		...
		} else if (!mpGetUsingMultipleTunes() && data->list.value == mpGetNumUnlockedTracks()) {
			musicStartTrackAsMenu(MUSIC_NONE);
		}
		break;
	case MENUOP_GETLISTITEMCHECKBOX:
		{
...
MenuItemHandlerResult mpSimulantNameMenuHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	char *name = data->keyboard.string;
	...
	case MENUOP_GETTEXT:
		...
		while (g_BotConfigsArray[botnum].base.name[i] != '\n'
				&& g_BotConfigsArray[botnum].base.name[i] != '\0'
				&& i < 13) {
			name[i] = g_BotConfigsArray[botnum].base.name[i];
			i++;
		}
		...
		break;
	case MENUOP_SETTEXT:
		mpSetBotCustomName(botnum, name);
		g_Vars.modifiedfiles |= MODFILE_MPSETUP;
		break;
	}
...
char *mpMenuTextSimulantNameDialogTitle(struct menudialogdef *dialogdef)
{
	return "Simulant Name\n";
}

MenuItemHandlerResult menuhandlerMpSimulantName(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		menuPushDialog(&g_MpChangeSimulantNameMenuDialog);
	}
...
{
	if (operation == MENUOP_SET) {
		mpRandomizeBotName(g_Menus[g_MpPlayerNum].mpsetup.slotindex);
		g_Vars.modifiedfiles |= MODFILE_MPSETUP;
	}
...
struct menuitem g_MpChangeSimulantNameMenuItems[] = {
	{
		MENUITEMTYPE_KEYBOARD,
		0,
		0,
...
struct menudialogdef g_MpChangeSimulantNameMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
	(uintptr_t)&mpMenuTextSimulantNameDialogTitle,
	g_MpChangeSimulantNameMenuItems,
	NULL,
	0,
	NULL,
};
...
MenuItemHandlerResult menuhandlerMpSavePlayer(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		...
		if (g_PlayerConfigsArray[g_MpPlayerNum].fileguid.fileid == 0) {
			s32 slotindex = mpplayersSavePlayer(g_MpPlayerNum, -1);
...
char *mpMenuTextArenaName(struct menuitem *item)
{
	...
	for (i = 0; i != ARRAYCOUNT(g_MpArenas); i++) {
		if (g_MpArenas[i].stagenum == g_MpSetup.stagenum) {
			return mpGetArenaDisplayNameByStage(g_MpArenas[i].stagenum, g_MpArenas[i].name);
		}
	}
...
MenuDialogHandlerResult menudialogMpGameSetup(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	if (operation == MENUOP_OPEN) {
		...
		if (dialogdef == &g_MpAdvancedSetupMenuDialog
				|| dialogdef == &g_MpAdvancedSetupViaAdvChallengeMenuDialog) {
			sndStart(var80095200, SFX_00F7, NULL, -1, -1, -1, -1, -1);
		}
	}
...
MenuItemHandlerResult menuhandlerMpNumberOfSimulants(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GETOPTIONCOUNT:
					data->dropdown.value = !challengeIsFeatureUnlocked(MPFEATURE_8BOTS) ? 4 : MAX_BOTS_RUNTIME;
		break;
	case MENUOP_GETOPTIONTEXT:
...
static const struct lmsstageentry g_LmsStageEntries[] = {
	...
	{ STAGE_MP_RAVINE, 0, "Ravine" },
	{ STAGE_MP_G5BUILDING, 0, "G5 Building" },
	{ STAGE_MP_SEWERS, 0, "Sewers" },
	{ STAGE_MP_WAREHOUSE, 0, "Warehouse" },
	{ STAGE_MP_GRID, 0, "Grid" },
	{ STAGE_MP_RUINS, 0, "Ruins" },
	{ STAGE_MP_AREA52, 0, "Area 52" },
	{ STAGE_MP_BASE, 0, "Base" },
	{ STAGE_MP_FORTRESS, 0, "Fortress" },
	{ STAGE_MP_VILLA, 0, "Villa" },
	{ STAGE_MP_CARPARK, 0, "Car Park" },
	{ STAGE_DEFECTION, 0, "dataDyne Central" },
	{ STAGE_INVESTIGATION, 0, "dataDyne Research" },
	{ STAGE_VILLA, 0, "Carrington Villa" },
	{ STAGE_CHICAGO, 0, "Chicago" },
	{ STAGE_G5BUILDING, 0, "G5 Building" },
	{ STAGE_INFILTRATION, 0, "Area 51" },
	{ STAGE_AIRBASE, 0, "Air Base" },
	{ STAGE_AIRFORCEONE, 0, "Air Force One" },
	{ STAGE_CRASHSITE, 0, "Crash Site" },
	{ STAGE_PELAGIC, 0, "Pelagic II" },
	{ STAGE_DEEPSEA, 0, "Deep Sea" },
	{ STAGE_DEFENSE, 0, "Carrington Institute" },
	{ STAGE_ATTACKSHIP, 0, "Attack Ship" },
	{ STAGE_SKEDARRUINS, 0, "Skedar Ruins" },
...
{
	if (entry->label != 0) {
		return langGet(entry->label);
	}

	return (char *)entry->literal;
}

static const struct lmspreset g_LmsPresets[] = {
	{
		5,
		MPOPTION_TEAMSENABLED | MPOPTION_SPAWNWITHWEAPON | MPOPTION_ONEHITHEADSHOTS,
		2,
		{ MPWEAPON_FALCON2, MPWEAPON_PP9I, MPWEAPON_KL01313, MPWEAPON_DMC, MPWEAPON_KF7SPECIAL, MPWEAPON_AR53 },
	},
	{
		10,
		MPOPTION_TEAMSENABLED | MPOPTION_SPAWNWITHWEAPON | MPOPTION_ONEHITHEADSHOTS,
...
static s32 g_LmsSelectedStageIndex = 0;

static void lmsConfigureSimulants(s32 botcount)
{
	s32 i;
	s32 profilenum = mpFindBotProfile(BOTTYPE_GENERAL, BOTDIFF_EASY);

	for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
		mpRemoveSimulant(i);
	}

	for (i = 0; i < botcount && i < MAX_BOTS_RUNTIME; i++) {
		mpCreateBotFromProfile(i, profilenum);
...
static void lmsApplyPreset(s32 stageindex, s32 diffindex)
{
	s32 i;
	const struct lmspreset *preset;

	if (stageindex < 0 || stageindex >= ARRAYCOUNT(g_LmsStageEntries)) {
		stageindex = 0;
	}

	if (diffindex < 0 || diffindex >= ARRAYCOUNT(g_LmsPresets)) {
		diffindex = 0;
	}

	preset = &g_LmsPresets[diffindex];

	mpSetLock(MPLOCKTYPE_NONE, 0);
	g_MpSetup.scenario = MPSCENARIO_LASTMANSTANDING;
	g_MpSetup.stagenum = g_LmsStageEntries[stageindex].stagenum;
	g_MpSetup.timelimit = 60;
	g_MpSetup.scorelimit = 100;
	g_MpSetup.teamscorelimit = 400;
	g_MpSetup.deathlimit = 0;
	g_MpSetup.options = preset->options;
	g_MpPickupRespawnSpeed = preset->pickuprespawn;
	g_MpSpHitReactions = true;
	...
	for (i = 0; i < NUM_MPWEAPONSLOTS; i++) {
		g_MpSetup.weapons[i] = preset->weapons[i];
	}
...
#if MP_LMS_DEBUG_QUICK_RESULTS
	lmsConfigureSimulants(1);
#else
	lmsConfigureSimulants(preset->botcount);
#endif
	scenarioInit();
	g_Vars.mpquickteam = MPQUICKTEAM_NONE;
	mpLmsCuratedSetPending(stageindex, diffindex);
}

static void lmsPromoteWaitingPlayersToReady(void)
{
	s32 prevmpplayernum = g_MpPlayerNum;
	s32 root = IS4MB() ? MENUROOT_4MBMAINMENU : MENUROOT_MPSETUP;
	s32 i;

	for (i = 0; i < MAX_PLAYERS; i++) {
		if (i != prevmpplayernum && g_Vars.waitingtojoin[i] && !g_Menus[i].curdialog) {
			g_Vars.waitingtojoin[i] = false;
			g_MpSetup.chrslots |= MPCHRSLOT_BIT(i);
			g_Menus[i].playernum = i;
			g_MpNumJoined++;

			g_MpPlayerNum = i;
			menuPushRootDialog(&g_MpReadyMenuDialog, root);
		}
	}

	g_MpPlayerNum = prevmpplayernum;
}

static u32 lmsGetStarColour(struct menuitemrenderdata *renderdata, s32 difficultyindex)
{
	switch (difficultyindex) {
	case 0:
		return (renderdata->colour & 0xff) * 0xff >> 8 | 0xcd7f3200;
	case 1:
		return (renderdata->colour & 0xff) * 0xff >> 8 | 0xc0c0c000;
	default:
		return (renderdata->colour & 0xff) * 0xff >> 8 | 0xffe56500;
	}
}

MenuItemHandlerResult menuhandlerMpLmsStageList(s32 operation, struct menuitem *item, union handlerdata *data)
{
	Gfx *gdl;
	struct menuitemrenderdata *renderdata;
	s32 x;
	...
	s32 starx;
	s32 stary;
	const struct lmsstageentry *entry;
	u8 completion;
	s32 i;

	switch (operation) {
	case MENUOP_GETOPTIONCOUNT:
		data->list.value = ARRAYCOUNT(g_LmsStageEntries);
		break;
	case MENUOP_SET:
		if (data->list.unk04 != 0) {
			data->list.unk04 = 2;
		...
		g_LmsSelectedStageIndex = data->list.value;
		menuPushDialog(&g_MpLastManStandingDifficultyMenuDialog);
		break;
	case MENUOP_GETSELECTEDINDEX:
		data->list.value = g_LmsSelectedStageIndex;
		break;
	case MENUOP_GETOPTGROUPCOUNT:
		data->list.value = 0;
		break;
	...
	case MENUOP_GETGROUPSTARTINDEX:
		...
		break;
	case MENUOP_RENDER:
		entry = &g_LmsStageEntries[data->type19.unk04];
		completion = gamefileGetLmsCompletion(data->type19.unk04);
		gdl = data->type19.gdl;
		renderdata = data->type19.renderdata2;
		x = renderdata->x + 10;
		y = renderdata->y + 1;
		...
		gdl = text0f153628(gdl);
			gdl = textRenderProjected(gdl, &x, &y, lmsStageEntryGetLabel(entry), g_CharsHandelGothicSm, g_FontHandelGothicSm,
		...
		starx = renderdata->x + renderdata->width - 50;
		stary = renderdata->y - 2;

		for (i = 0; i < 3; i++) {
			if (completion & (1 << i)) {
				gDPSetEnvColorViaWord(gdl++, lmsGetStarColour(renderdata, i));
			} else {
				gDPSetEnvColorViaWord(gdl++, (renderdata->colour & 0xff) * 0xff >> 8 | 0x43430000);
			}
			...
			gSPTextureRectangle(gdl++,
				((starx + i * 13) << 2) * g_ScaleX, stary << 2,
				((starx + i * 13 + 11) << 2) * g_ScaleX, (stary + 11) << 2,
				G_TX_RENDERTILE, 0, 0x0160, 1024 / g_ScaleX, -1024);
		}
...
MenuItemHandlerResult menuhandlerMpLmsOpenStage(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		s32 index = item->param;

		if (index < 0 || index >= ARRAYCOUNT(g_LmsStageEntries)) {
			index = 0;
		}

		g_LmsSelectedStageIndex = index;
		menuPushDialog(&g_MpLastManStandingDifficultyMenuDialog);
	}
...
MenuItemHandlerResult menuhandlerMpLmsStart(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		menuPushDialog(&g_MpLastManStandingDifficultyMenuDialog);
	}
...
MenuItemHandlerResult menuhandlerMpLmsStartDisabled(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_CHECKDISABLED) {
		return false;
	}

	return menuhandlerMpLmsStart(operation, item, data);
}

MenuItemHandlerResult menuhandlerMpLmsDifficulty(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		s32 stageindex = g_LmsSelectedStageIndex;
		s32 diffindex = item->param;
		...
		lmsApplyPreset(stageindex, diffindex);
		lmsPromoteWaitingPlayersToReady();
		menuPushDialog(&g_MpReadyMenuDialog);
	}
...
struct menuitem g_MpLastManStandingMenuItems[] = {
	{
		MENUITEMTYPE_SEPARATOR,
		0,
		0,
...
struct menudialogdef g_MpLastManStandingMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
	MD_MPMENU_TEXT_LAST_MAN_STANDING,
	g_MpLastManStandingMenuItems,
	NULL,
	0,
	NULL,
};

struct menuitem g_MpLastManStandingDifficultyMenuItems[] = {
	{
		MENUITEMTYPE_SELECTABLE,
		0,
		MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_DIFFICULTY_EASY,
		0,
		menuhandlerMpLmsDifficulty,
	},
	{
		MENUITEMTYPE_SELECTABLE,
		1,
		MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_DIFFICULTY_NORMAL,
		0,
		menuhandlerMpLmsDifficulty,
	},
	{
		...
		2,
		MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_DIFFICULTY_HARD,
		0,
		menuhandlerMpLmsDifficulty,
	},
	{
...
MenuDialogHandlerResult menudialogMpLastManStandingDifficulty(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	if (operation == MENUOP_OPEN) {
		if (g_LmsSelectedStageIndex < 0
				|| g_LmsSelectedStageIndex >= ARRAYCOUNT(g_LmsStageEntries)) {
			g_LmsSelectedStageIndex = 0;
		}
	}
...
struct menudialogdef g_MpLastManStandingDifficultyMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
	(uintptr_t)MD_MPMENU_LITERAL_SELECT_DIFFICULTY,
	g_MpLastManStandingDifficultyMenuItems,
	menudialogMpLastManStandingDifficulty,
	MENUDIALOGFLAG_LITERAL_TEXT,
	NULL,
};

#define TERMINATION_NORMAL_BOT_COUNT 6

static s32 g_TerminationSelectedStageIndex = 0;

static void terminationStartMenuMusic(void)
{
	musicStartTrackAsMenu(MUSIC_TERMINATION_MENU);
}

static void terminationStopMenuMusic(void)
{
	musicStartMenu();
}

static MenuItemHandlerResult menuhandlerMpTerminationCancel(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		terminationStopMenuMusic();
	}

	return 0;
}

static void terminationConfigureSimulants(void)
{
	s32 i;
	s32 terminationprofile = mpFindBotProfile(BOTTYPE_KAZE, BOTDIFF_NORMAL);
	s32 victimprofile = mpFindBotProfile(BOTTYPE_PEACE, BOTDIFF_NORMAL);

	if (terminationprofile < 0) {
		terminationprofile = mpFindBotProfile(BOTTYPE_GENERAL, BOTDIFF_NORMAL);
	}

	if (victimprofile < 0) {
		victimprofile = mpFindBotProfile(BOTTYPE_GENERAL, BOTDIFF_MEAT);
	}
	...
	for (i = 0; i < TERMINATION_NORMAL_BOT_COUNT && i < MAX_BOTS_RUNTIME; i++) {
		mpCreateBotFromProfile(i, i == 0 ? terminationprofile : victimprofile);
		g_BotConfigsArray[i].base.team = i == 0 ? MPTEAM_1 : MPTEAM_0;
		g_BotConfigsArray[i].base.mpbodynum = i == 0 ? MPBODY_EVIL_CYBORG : MPBODY_CASSANDRA;
		g_BotConfigsArray[i].base.mpheadnum = i == 0 ? MPHEAD_EVIL_CYBORG : mpGetMpheadnumByMpbodynum(g_BotConfigsArray[i].base.mpbodynum);
	}

	mpGenerateBotNames();
}

static void terminationApplyNormalPreset(s32 stageindex)
{
	s32 i;
	static const u8 weapons[] = {
	...
	mpSetLock(MPLOCKTYPE_NONE, 0);
	mpLmsCuratedReset();
	g_MpSetup.scenario = MPSCENARIO_TERMINATION;
	g_MpSetup.stagenum = g_LmsStageEntries[stageindex].stagenum;
	g_MpSetup.timelimit = 60;
	g_MpSetup.scorelimit = 100;
	g_MpSetup.teamscorelimit = 400;
	g_MpSetup.deathlimit = 0;
	g_MpSetup.options = MPOPTION_TEAMSENABLED | MPOPTION_SPAWNWITHWEAPON;
	g_MpPickupRespawnSpeed = 2;
	g_MpSpHitReactions = true;
	...
	for (i = 0; i < NUM_MPWEAPONSLOTS; i++) {
		g_MpSetup.weapons[i] = i < ARRAYCOUNT(weapons) ? weapons[i] : MPWEAPON_DISABLED;
	}
	...
	terminationConfigureSimulants();
	scenarioInit();
	g_Vars.mpquickteam = MPQUICKTEAM_NONE;
}

MenuItemHandlerResult menuhandlerMpTerminationStageList(s32 operation, struct menuitem *item, union handlerdata *data)
{
	Gfx *gdl;
	struct menuitemrenderdata *renderdata;
	s32 x;
	...
	case MENUOP_SET:
		...
		g_TerminationSelectedStageIndex = data->list.value;
		terminationApplyNormalPreset(g_TerminationSelectedStageIndex);
		lmsPromoteWaitingPlayersToReady();
		terminationStopMenuMusic();
		menuPushDialog(&g_MpReadyMenuDialog);
		break;
	case MENUOP_GETSELECTEDINDEX:
		data->list.value = g_TerminationSelectedStageIndex;
		break;
	case MENUOP_GETOPTGROUPCOUNT:
		data->list.value = 0;
		break;
...
MenuDialogHandlerResult menudialogMpTermination(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	if (operation == MENUOP_OPEN) {
		if (g_TerminationSelectedStageIndex < 0
				|| g_TerminationSelectedStageIndex >= ARRAYCOUNT(g_LmsStageEntries)) {
			g_TerminationSelectedStageIndex = 0;
		}

		terminationStartMenuMusic();
	}

	return 0;
}

struct menuitem g_MpTerminationMenuItems[] = {
	{
		MENUITEMTYPE_SEPARATOR,
		0,
		0,
	...
	},
	{
		...
		220,
		130,
		menuhandlerMpTerminationStageList,
	},
	{
		MENUITEMTYPE_SEPARATOR,
		0,
	...
	},
	{
		...
		L_MPMENU_055, // "Cancel"
		0,
		menuhandlerMpTerminationCancel,
	},
	{ MENUITEMTYPE_END },
};
...
struct menudialogdef g_MpTerminationMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
	MD_MPMENU_TEXT_TERMINATION,
	g_MpTerminationMenuItems,
	menudialogMpTermination,
	MENUDIALOGFLAG_LITERAL_TEXT,
	NULL,
};
...
struct menuitem g_CombatSimulatorMenuItems[] = {
	...
	},
	{
		...
		MD_MPMENU_TEXT_LAST_MAN_STANDING,
		0,
		(void *)&g_MpLastManStandingMenuDialog,
	},
	{
		MENUITEMTYPE_SELECTABLE,
		0,
		...
		MD_MPMENU_TEXT_TERMINATION,
		0,
		(void *)&g_MpTerminationMenuDialog,
	},
	{
		MENUITEMTYPE_SELECTABLE,
		0,
...
struct menuitem g_MpExtGameOptionsMenuItems[] = {
	{
		...
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_START_ARMED,
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_NO_DRUG_BLUR,
		MPOPTION_NODRUGBLUR,
		menuhandlerMpCheckboxOption,
	},
	{
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_NO_PLAYER_ON_RADAR,
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_NO_DOORS,
		MPOPTION_NODOORS,
		menuhandlerMpCheckboxOption,
	},
	{
		...
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_DESTRUCTIBLE_DOORS,
		MPOPTION_DESTRUCTIBLEDOORS,
		menuhandlerMpCheckboxOption,
	},
	{
		MENUITEMTYPE_DROPDOWN,
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_PICKUP_RESPAWN,
		0,
		menuhandlerMpPickupRespawnSpeed,
	},
	{
		MENUITEMTYPE_CHECKBOX,
		0,
		MENUITEMFLAG_LOCKABLEMINOR | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)MD_MPMENU_LITERAL_PICKUP_RESPAWN_SFX,
		0,
		menuhandlerMpPickupRespawnSfx,
	},
	{ MENUITEMTYPE_END },
};
...
struct menudialogdef g_ExtGameOptionsMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
	(uintptr_t)MD_MPMENU_LITERAL_MORE_OPTIONS,
