...
struct menudialogdef g_MpEndscreenLmsResultMenuDialog;

#define MP_LMS_RESULT_TEX_WIDTH 240
#define MP_LMS_RESULT_TEX_HEIGHT 135
#define MP_LMS_RESULT_WIN_TEX 0x02c8
#define MP_LMS_RESULT_LOSE_TEX 0x02c9
#define MP_TERMINATION_RESULT_WIN_TEX 0x0c42
#define MP_TERMINATION_RESULT_LOSE_TEX 0x0c43
#define MP_TERMINATION_RESULT_TEX_WIDTH 32
#define MP_TERMINATION_RESULT_TEX_HEIGHT 32

static bool g_MpLmsResultMusicStarted = false;
...
static s32 mpLmsResultGetKillsForChrSlot(s32 chrslot)
{
	struct mpchrconfig *mpchr = MPCHR(chrslot);
	s32 kills = 0;
	s32 i;

	for (i = 0; i < MAX_MPCHRS_RUNTIME; i++) {
		if (i != chrslot) {
			kills += mpchr->killcounts[i];
		}
	}
...
static void mpLmsResultCopyDisplayName(char *dst, s32 dstlen, const char *src)
{
	s32 i;
	...
	for (i = 0; i < dstlen - 1 && src[i] != '\0' && src[i] != '\n'; i++) {
		dst[i] = src[i];
	}
...
static Gfx *mpRenderLmsResultCenteredSmLine(Gfx *gdl, s32 y, char *line)
{
	s32 x;
	s32 textwidth;
	s32 textheight;

	textMeasure(&textheight, &textwidth, line, g_CharsHandelGothicSm, g_FontHandelGothicSm, 0);
	x = (viGetWidth() - textwidth) / 2;

	return textRenderProjected(gdl, &x, &y, line,
			g_CharsHandelGothicSm, g_FontHandelGothicSm,
			0xffffffff, viGetWidth(), viGetHeight(), 0, 0);
}

static Gfx *mpRenderLmsResultStats(Gfx *gdl)
{
	char line[96];
	char *prompt = "Return To Main Menu";
	s32 x;
	s32 y;
	s32 textwidth;
	s32 textheight;
	...
	if (isterminationresult) {
		y = mpLmsCuratedWasResultWin() ? viGetHeight() - 112 : viGetHeight() - 88;

		sprintf(line, "PLAYER MATCH STATS");
		gdl = mpRenderLmsResultCenteredSmLine(gdl, y, line);

		for (i = 0; i < PLAYERCOUNT(); i++) {
			struct mpchrconfig *mpchr = MPCHR(i);
			s32 remaininglives = mpGetRemainingLivesForChrSlot(i);
			char name[16];
			...
			mpLmsResultCopyDisplayName(name, sizeof(name), mpchr->name);

			y += 12;
			sprintf(line, "P%d %s", i + 1, name);
			gdl = mpRenderLmsResultCenteredSmLine(gdl, y, line);

			y += 12;
			sprintf(line, "KILLS %d  DEATHS %d  LIVES %d",
					mpLmsResultGetKillsForChrSlot(i),
					mpchr->numdeaths,
					remaininglives);
			gdl = mpRenderLmsResultCenteredSmLine(gdl, y, line);
		}
		...
		sprintf(line, "PLAYER MATCH STATS");
		gdl = textRenderProjected(gdl, &x, &y, line,
				g_CharsHandelGothicSm, g_FontHandelGothicSm,
				0xffffffff, viGetWidth(), viGetHeight(), 0, 0);
		...
		for (i = 0; i < PLAYERCOUNT(); i++) {
			...
			x = 14;
			y += 12;
			sprintf(line, "P%d %s  KILLS %d  DEATHS %d  LIVES %d",
					i + 1,
					mpchr->name,
					mpLmsResultGetKillsForChrSlot(i),
					mpchr->numdeaths,
	...
	textMeasure(&textheight, &textwidth, prompt, g_CharsHandelGothicMd, g_FontHandelGothicMd, 0);
	x = (viGetWidth() - textwidth) / 2;
	y = viGetHeight() - 20;
	gdl = textRenderProjected(gdl, &x, &y, prompt,
			g_CharsHandelGothicMd, g_FontHandelGothicMd,
			0xffffffff, viGetWidth(), viGetHeight(), 0, 0);
...
Gfx *mpRenderLmsResultBackground(Gfx *gdl)
{
	bool isterminationresult = g_MpSetup.scenario == MPSCENARIO_TERMINATION;
	s32 texwidth = isterminationresult ? MP_TERMINATION_RESULT_TEX_WIDTH : MP_LMS_RESULT_TEX_WIDTH;
	s32 texheight = isterminationresult ? MP_TERMINATION_RESULT_TEX_HEIGHT : MP_LMS_RESULT_TEX_HEIGHT;
	struct textureconfig texture = {
		mpLmsCuratedWasResultWin()
			? (isterminationresult ? MP_TERMINATION_RESULT_WIN_TEX : MP_LMS_RESULT_WIN_TEX)
			: (isterminationresult ? MP_TERMINATION_RESULT_LOSE_TEX : MP_LMS_RESULT_LOSE_TEX),
		texwidth,
		texheight,
		0,
		G_IM_FMT_RGBA,
	...
	texSelect(&gdl, &texture, 1, 0, 2, true, NULL);

	gDPSetCycleType(gdl++, G_CYC_1CYCLE);
	gDPSetCombineMode(gdl++, G_CC_DECALRGBA, G_CC_DECALRGBA);
	gDPSetTextureFilter(gdl++, G_TF_BILERP);

	dsdx = (texwidth << 10) / viGetWidth();
	dtdy = -((texheight << 10) / viGetHeight());
	...
	gdl = mpRenderLmsResultStats(gdl);
...
MenuItemHandlerResult mpStatsForPlayerDropdownHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_GETOPTIONCOUNT:
		...
		for (v0 = 0; v0 < MAX_MPCHRS_RUNTIME; v0++) {
			if (g_MpSetup.chrslots & MPCHRSLOT_BIT(v0)) {
				data->list.value++;
			}
		}
		break;
	case MENUOP_GETOPTIONTEXT:
		...
		for (a1 = 0; a1 < MAX_MPCHRS_RUNTIME; a1++) {
			if (g_MpSetup.chrslots & MPCHRSLOT_BIT(a1)) {
				mpchr = MPCHR(a1);
	...
	case MENUOP_GETSELECTEDINDEX:
		...
		for (v1 = 0; v1 < MAX_MPCHRS_RUNTIME; v1++) {
			if (g_MpSetup.chrslots & MPCHRSLOT_BIT(v1)) {
				if (v0);
...
void mpPushEndscreenDialog(u32 arg0, u32 playernum)
{
	...
	if (mpLmsCuratedHasResult()) {
		menuPushRootDialog(&g_MpEndscreenLmsResultMenuDialog, MENUROOT_MPENDSCREEN);
		g_MpPlayerNum = prevplayernum;
		return;
	}
...
MenuItemHandlerResult menuhandlerMpEndscreenExitAllHint(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_CHECKHIDDEN) {
		if (g_Vars.coopplayernum >= 0 || g_Vars.antiplayernum >= 0 || PLAYERCOUNT() < 2 || g_MpPlayerNum != 0) {
			return true;
		}
	}
...
MenuItemHandlerResult menuhandlerMpEndscreenExitAll(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_CHECKHIDDEN) {
		if (g_Vars.coopplayernum >= 0 || g_Vars.antiplayernum >= 0 || PLAYERCOUNT() < 2 || g_MpPlayerNum != 0) {
			return true;
...
MenuDialogHandlerResult menudialogMpEndscreenLmsResult(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	if (operation == MENUOP_OPEN) {
		g_MpLmsResultMusicStarted = true;
	}

	return 0;
}

MenuItemHandlerResult menuhandlerMpEndscreenLmsResultCard(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	if (operation == MENUOP_GETOPTIONHEIGHT) {
		data->list.value = viGetHeight() - 34;
		return 0;
	}

	if (operation == MENUOP_RENDER) {
		return (uintptr_t) data->type19.gdl;
	}

	return 0;
}

MenuItemHandlerResult menuhandlerMpEndscreenLmsExit(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		s32 prevmpplayernum = g_MpPlayerNum;
		s32 i;
		...
		g_MpLmsResultMusicStarted = false;
...
struct menuitem g_MpGameOverMenuItems[] = {
	...
	},
	{
		...
		(uintptr_t)"P1 START: Exit All",
		0,
		menuhandlerMpEndscreenExitAllHint,
	},
	{
		MENUITEMTYPE_SELECTABLE,
		0,
		...
		(uintptr_t)"Exit All",
		0,
		menuhandlerMpEndscreenExitAll,
	},
	{ MENUITEMTYPE_END },
};

struct menuitem g_MpLmsResultMenuItems[] = {
	{
		MENUITEMTYPE_SELECTABLE,
		0,
		MENUITEMFLAG_SELECTABLE_CENTRE | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)"Return To Main Menu",
		0,
		menuhandlerMpEndscreenLmsExit,
	},
	{ MENUITEMTYPE_END },
};
...
struct menudialogdef g_MpEndscreenLmsResultMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
	(uintptr_t)MD_MPMENU_LITERAL_LAST_MAN_STANDING,
	g_MpLmsResultMenuItems,
	menudialogMpEndscreenLmsResult,
	MENUDIALOGFLAG_LITERAL_TEXT | MENUDIALOGFLAG_DISABLETITLEBAR,
	NULL,
};
