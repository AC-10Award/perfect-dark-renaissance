...
// bss
struct chrdata *g_MpAllChrPtrs[MAX_MPCHRS_RUNTIME];
struct mpchrconfig *g_MpAllChrConfigPtrs[MAX_MPCHRS_RUNTIME];
s32 g_MpNumChrs;
u32 var800ac534;
struct mpbotconfig g_BotConfigsArray[MAX_BOTS_RUNTIME];
u8 g_MpSimulantDifficultiesPerNumPlayers[MAX_BOTS_RUNTIME][MAX_PLAYERS];
...
static s32 g_MpLmsCuratedPendingStageIndex = -1;
static s32 g_MpLmsCuratedPendingDifficultyIndex = -1;
static s32 g_MpLmsCuratedStageIndex = -1;
static s32 g_MpLmsCuratedDifficultyIndex = -1;
static bool g_MpLmsCuratedHasResult = false;
static bool g_MpLmsCuratedWasWin = false;

static bool mpDidAnyHumanAbortMatch(void)
{
...
static s32 mpBotSlotToChrSlot(s32 botnum)
{
	return MAX_PLAYERS + botnum;
}

static s32 mpGetConfigBotCount(void)
{
	return MAX_BOTS;
}
...
struct mpweapon g_MpWeapons[NUM_MPWEAPONS] = {
	...
	/*0x31*/ { MD_WEAPON_MATRIX85,      AMMOTYPE_PISTOL,      80,  0,                   0,  1, 0,                                MODEL_CHRLEEGUN1,       256 },
};

#ifndef PLATFORM_N64
static const u8 g_MpWeaponDisplayOrder[] = {
	MPWEAPON_PP9I,
	MPWEAPON_FALCON2,
...
void mpReset(void)
{
	...
	if (g_MpLmsCuratedPendingStageIndex >= 0 && g_MpLmsCuratedPendingDifficultyIndex >= 0) {
		mpLmsCuratedPromotePending();
	} else if (g_MpLmsCuratedStageIndex < 0) {
		mpLmsCuratedReset();
	}
	g_MpNumChrs = 0;
...
void func0f1881d4(s32 index)
{
	...
	g_BotConfigsArray[index].base.displayoptions = 0;
}
...
void mpInit(bool resetplayers)
{
	s32 i;
	s32 j;
	bool musicsettingsunset = !g_BossFile.usingmultipletunes && g_BossFile.tracknum == 0;

	for (i = 0; musicsettingsunset && i < ARRAYCOUNT(g_BossFile.multipletracknums); i++) {
		if (g_BossFile.multipletracknums[i] != 0) {
			musicsettingsunset = false;
		}
	}
...
bool mpIsDeathsLimitEnabled(void)
{
	return g_MpSetup.scenario == MPSCENARIO_LASTMANSTANDING
		|| g_MpSetup.scenario == MPSCENARIO_TERMINATION
		|| g_MpSetup.deathlimit > 0;
}

s32 mpGetDeathsLimitValue(void)
{
	if (g_MpSetup.scenario == MPSCENARIO_LASTMANSTANDING) {
		return 15;
	}

	if (g_MpSetup.scenario == MPSCENARIO_TERMINATION) {
		return 5;
	}

	return g_MpSetup.deathlimit;
}

s32 mpGetLifeLimitForChrSlot(s32 chrslot)
{
	if (chrslot < 0 || chrslot >= MAX_MPCHRS_RUNTIME) {
		return 0;
	}
...
s32 mpGetRemainingLivesForChrSlot(s32 chrslot)
{
	s32 lifelimit = mpGetLifeLimitForChrSlot(chrslot);

	if (lifelimit <= 0) {
		return -1;
	}

	return lifelimit - MPCHR(chrslot)->numdeaths;
}

bool mpHasRemainingLives(s32 chrslot)
{
...
static s32 mpCountRemainingHumanLives(void)
{
	s32 numremaining = 0;
	s32 i;

	for (i = 0; i < PLAYERCOUNT(); i++) {
		if (mpHasRemainingLives(i)) {
			numremaining++;
		}
	}
...
void mpMaybeEndMatchByDeathsLimit(void)
{
	s32 i;

	if (!g_Vars.normmplayerisrunning || !mpIsDeathsLimitEnabled() || g_NumReasonsToEndMpMatch > 0) {
		return;
	}

	// If no human players have lives left, there's nobody left to play.
	if (mpCountRemainingHumanLives() == 0) {
		g_NumReasonsToEndMpMatch++;
		return;
	}

	if (g_MpSetup.scenario == MPSCENARIO_LASTMANSTANDING) {
		s32 numbotsremaining = 0;

		for (i = MAX_PLAYERS; i < MAX_MPCHRS_RUNTIME; i++) {
			if (mpHasRemainingLives(i)) {
				numbotsremaining++;
			}
		}

		if (numbotsremaining == 0) {
			g_NumReasonsToEndMpMatch++;
		}
	...
	if (g_MpSetup.options & MPOPTION_TEAMSENABLED) {
		bool teamsremaining[4] = { false };
		s32 numteamsremaining = 0;
		...
		for (i = 0; i < MAX_MPCHRS_RUNTIME; i++) {
			...
			if (team < ARRAYCOUNT(teamsremaining) && !teamsremaining[team]) {
				teamsremaining[team] = true;
				numteamsremaining++;
			}
		}

		if (numteamsremaining <= 1) {
			g_NumReasonsToEndMpMatch++;
		}
	} else {
		s32 numremaining = 0;
...
{
	s32 i;
	s32 scores[MAX_MPCHRS_RUNTIME];
	u32 rankablescores[MAX_MPCHRS_RUNTIME];
	struct mpchrconfig *mpchrs[MAX_MPCHRS_RUNTIME];
	s32 chrnums[MAX_MPCHRS_RUNTIME];
	s32 count = 0;
	s32 numteams;
	struct ranking teamrankings[MAX_MPCHRS_RUNTIME];
	s32 winner;
	s32 loser;
	s32 score;
	s32 deaths;
...
static char *mpGetWeaponLabelByMpWeaponNum(s32 mpweaponnum)
{
	if (g_MpWeapons[mpweaponnum].weaponnum == WEAPON_NONE) {
		return langGet(L_MPWEAPONS_058); // "Nothing"
	}

	if (g_MpWeapons[mpweaponnum].weaponnum == WEAPON_MPSHIELD) {
	...
	return bgunGetName(g_MpWeapons[mpweaponnum].weaponnum);
}

s32 mpGetMpWeaponNumByOptionIndex(s32 optionindex)
{
	s32 i;

#ifndef PLATFORM_N64
	for (i = 0; i < ARRAYCOUNT(g_MpWeaponDisplayOrder); i++) {
		s32 mpweaponnum = g_MpWeaponDisplayOrder[i];

		if (challengeIsFeatureUnlocked(g_MpWeapons[mpweaponnum].unlockfeature)) {
			if (optionindex == 0) {
				return mpweaponnum;
			}
	...
	return MPWEAPON_NONE;
}

s32 mpGetWeaponOptionIndexByWeaponNum(s32 mpweaponnum)
{
	s32 count = 0;
	s32 i;
...
#ifndef PLATFORM_N64
	for (i = 0; i < ARRAYCOUNT(g_MpWeaponDisplayOrder); i++) {
		s32 candidate = g_MpWeaponDisplayOrder[i];

		if (challengeIsFeatureUnlocked(g_MpWeapons[candidate].unlockfeature)) {
			if (candidate == mpweaponnum) {
				return count;
			}
...
{
	s32 mpweaponnum = mpGetMpWeaponNumByOptionIndex(weaponnum);

	if (mpweaponnum >= 0 && mpweaponnum < ARRAYCOUNT(g_MpWeapons)) {
		return mpGetWeaponLabelByMpWeaponNum(mpweaponnum);
	}
...
{
#ifndef PLATFORM_N64
	g_MpSetup.weapons[slot] = mpGetMpWeaponNumByOptionIndex(mpweaponnum);
#else
...
{
#ifndef PLATFORM_N64
	return mpGetWeaponOptionIndexByWeaponNum(g_MpSetup.weapons[slot]);
#else
	s32 count = 0;
	s32 i;
...
void mpSetRandomWeapons(u8 weapons[])
{
	...
	for (i = 0; i < NUM_MPWEAPONS; i++) {
		if (challengeIsFeatureUnlocked(g_MpWeapons[i].unlockfeature)) {
			if (g_MpWeaponSetRandomFilters[i] == 1) {
				weapons[index] = mpGetWeaponOptionIndexByWeaponNum(i);
				index++;
			}
		}
	}
	...
	if (index == 0) {
		weapons[0] = mpGetWeaponOptionIndexByWeaponNum(MPWEAPON_NONE);
...
void mpApplyWeaponSet(void)
{
	...
	if (g_MpWeaponSetNum >= 0 && g_MpWeaponSetNum < ARRAYCOUNT(g_MpWeaponSets)) {
		...
		mpSetWeaponSlot(i, mpGetWeaponOptionIndexByWeaponNum(MPWEAPON_DISABLED));
	}
}
...
Gfx *mpRenderModalText(Gfx *gdl)
{
	...
	if (g_MpSetup.paused == MPPAUSEMODE_PAUSED) {
		...
		// Render "Press START" text, or a dead-state label if this player is eliminated.
		gdl = text0f153628(gdl);

		if (mpIsDeathsLimitEnabled() && !mpHasRemainingLives(g_Vars.currentplayerstats->mpindex)) {
			strcpy(text, "YOU ARE DEAD");
		} else {
		...
		if (g_Vars.currentplayer->deadtimer > 0
				&& !(mpIsDeathsLimitEnabled() && !mpHasRemainingLives(g_Vars.currentplayerstats->mpindex))) {
...
u16 g_AwardNames[] = {
	...
	L_MPMENU_006, // "Perfect Dark Renaissance"
...
struct mphead g_MpHeads[] = {
	...
	{ /*0x4e*/ HEAD_OURUMOV,      0                          },
	{ /*0x4f*/ HEAD_ODDJOB,       0                          },
	{ /*0x50*/ HEAD_TREVELYAN_Z,  0                          },
	{ /*0x51*/ HEAD_BROSNAN_CLASSIC, 0                       },
	{ /*0x52*/ HEAD_DALTON_CLASSIC, 0                        },
	{ /*0x53*/ HEAD_CONNERY_CLASSIC, 0                       },
	{ /*0x54*/ HEAD_MOORE_CLASSIC, 0                         },
	{ /*0x55*/ HEAD_LAZENBY_CLASSIC, 0                       },
	{ /*0x56*/ HEAD_JAWS_Z,       0                          },
	{ /*0x57*/ HEAD_SNOWGUARD_Z,  0                          },
	{ /*0x58*/ HEAD_BROSNAN_VARIANT, 0                       },
	{ /*0x59*/ HEAD_MISHKIN_Z,    0                          },
	{ /*0x5a*/ HEAD_BORIS_Z,      0                          },
	{ /*0x5b*/ HEAD_VALENTIN_Z,   0                          },
	{ /*0x5c*/ HEAD_MAYDAY_Z,     0                          },
	{ /*0x5d*/ HEAD_BARON_SAMEDI_Z, 0                        },
	{ /*0x5e*/ HEAD_BARON_SAMEDI_DEATH_Z, 0                  },
	{ /*0x5f*/ HEAD_SANTA_Z,      0                          },
	{ /*0x60*/ HEAD_ELF_Z,        0                          },
	{ /*0x61*/ HEAD_PILOT_Z,      0                          },
	{ /*0x62*/ HEAD_MARION_Z,     0                          },
	{ /*0x63*/ HEAD_GALORE_Z,     0                          },
	{ /*0x64*/ HEAD_SOGUN_Z,      0                          },
	{ /*0x65*/ HEAD_DOAK_Z1,      0                          },
	{ /*0x66*/ HEAD_DOAK_Z2,      0                          },
	{ /*0x67*/ HEAD_TERRORIST_Z,  0                          },
	{ /*0x68*/ HEAD_DWAYNE_Z,     0                          },
	{ /*0x69*/ HEAD_KARL_Z1,      0                          },
	{ /*0x6a*/ HEAD_KARL_Z2,      0                          },
	{ /*0x6b*/ HEAD_MARTIN_Z,     0                          },
	{ /*0x6c*/ HEAD_MARK_EDMONDS_Z, 0                        },
	{ /*0x6d*/ HEAD_DUNCAN_Z1,    0                          },
	{ /*0x6e*/ HEAD_DUNCAN_Z2,    0                          },
	{ /*0x6f*/ HEAD_B_JONES_Z,    0                          },
	{ /*0x70*/ HEAD_GRANT_Z,      0                          },
	{ /*0x71*/ HEAD_ROBIN_Z,      0                          },
	{ /*0x72*/ HEAD_GRAEME_Z1,    0                          },
	{ /*0x73*/ HEAD_GRAEME_Z2,    0                          },
	{ /*0x74*/ HEAD_STEVE_ELLIS_Z, 0                         },
	{ /*0x75*/ HEAD_KEN_LOBB_Z,   0                          },
	{ /*0x76*/ HEAD_EVIL_CYBORG,   0                          },
	{ /*0x77*/ HEAD_ROBOT_SKELINGTON, 0                     },
#endif
};
...
#ifndef PLATFORM_N64
	/*0x3f*/ { BODY_NATALYA,          L_OPTIONS_070,   HEAD_NATALYA,     0                          },
	/*0x40*/ { BODY_XENIA,            L_OPTIONS_070,   HEAD_XENIA,       0                          },
	/*0x41*/ { BODY_OURUMOV,          L_OPTIONS_070,   HEAD_OURUMOV,     0                          },
	/*0x42*/ { BODY_ODDJOB,           L_OPTIONS_070,   HEAD_ODDJOB,      0                          },
	/*0x43*/ { BODY_TREVELYAN_JANUS,  L_OPTIONS_070,   HEAD_TREVELYAN_Z, 0                          },
	/*0x44*/ { BODY_NATALYA_CUBA,     L_OPTIONS_070,   HEAD_NATALYA,     0                          },
	/*0x45*/ { BODY_BOND_BROSNAN_CLASSIC, L_OPTIONS_070, HEAD_BROSNAN_CLASSIC, 0                    },
	/*0x46*/ { BODY_BOND_DALTON_CLASSIC,  L_OPTIONS_070, HEAD_DALTON_CLASSIC,  0                    },
	/*0x47*/ { BODY_BOND_CONNERY_CLASSIC, L_OPTIONS_070, HEAD_CONNERY_CLASSIC, 0                    },
	/*0x48*/ { BODY_BOND_MOORE_CLASSIC,   L_OPTIONS_070, HEAD_MOORE_CLASSIC,   0                    },
	/*0x49*/ { BODY_BOND_LAZENBY_CLASSIC, L_OPTIONS_070, HEAD_LAZENBY_CLASSIC, 0                    },
	/*0x4a*/ { BODY_JAWS,             L_OPTIONS_070,   HEAD_JAWS_Z,      0                          },
	/*0x4b*/ { BODY_SNOWGUARD,        L_OPTIONS_070,   HEAD_SNOWGUARD_Z, 0                          },
	/*0x4c*/ { BODY_BOND_ARCTIC,      L_OPTIONS_070,   HEAD_BROSNAN_VARIANT, 0                      },
	/*0x4d*/ { BODY_MISHKIN_Z,        L_OPTIONS_070,   HEAD_MISHKIN_Z,   0                          },
	/*0x4e*/ { BODY_BOND_FORMAL,      L_OPTIONS_070,   HEAD_BROSNAN_VARIANT, 0                      },
	/*0x4f*/ { BODY_BOND_STEALTH,     L_OPTIONS_070,   HEAD_BROSNAN_VARIANT, 0                      },
	/*0x50*/ { BODY_TREVELYAN_006,    L_OPTIONS_070,   HEAD_TREVELYAN_Z, 0                          },
	/*0x51*/ { BODY_BOND_JUNGLE,      L_OPTIONS_070,   HEAD_BROSNAN_VARIANT, 0                      },
	/*0x52*/ { BODY_BORIS,            L_OPTIONS_070,   HEAD_BORIS_Z,     0                          },
	/*0x53*/ { BODY_VALENTIN,         L_OPTIONS_070,   HEAD_VALENTIN_Z,  0                          },
	/*0x54*/ { BODY_MAYDAY,           L_OPTIONS_070,   HEAD_MAYDAY_Z,    0                          },
	/*0x55*/ { BODY_BARON_SAMEDI,     L_OPTIONS_070,   HEAD_BARON_SAMEDI_Z, 0                       },
	/*0x56*/ { BODY_BARON_SAMEDI_DEATH, L_OPTIONS_070, HEAD_BARON_SAMEDI_DEATH_Z, 0                 },
	/*0x57*/ { BODY_SANTA,            L_OPTIONS_070,   HEAD_SANTA_Z,     0                          },
	/*0x58*/ { BODY_ELF,              L_OPTIONS_070,   HEAD_ELF_Z,       0                          },
	/*0x59*/ { BODY_PILOT_GE,         L_OPTIONS_070,   HEAD_PILOT_Z,     0                          },
	/*0x5a*/ { BODY_MARION,           L_OPTIONS_070,   HEAD_MARION_Z,    0                          },
	/*0x5b*/ { BODY_BOND_CONNERY_ALPS, L_OPTIONS_070,  HEAD_CONNERY_CLASSIC, 0                      },
	/*0x5c*/ { BODY_BOND_CONNERY_RANCH, L_OPTIONS_070, HEAD_CONNERY_CLASSIC, 0                      },
	/*0x5d*/ { BODY_GALORE_PLANE,     L_OPTIONS_070,   HEAD_GALORE_Z,    0                          },
	/*0x5e*/ { BODY_GALORE_RANCH,     L_OPTIONS_070,   HEAD_GALORE_Z,    0                          },
	/*0x5f*/ { BODY_SOGUN,            L_OPTIONS_070,   HEAD_SOGUN_Z,     0                          },
	/*0x60*/ { BODY_NATALYA_SANTA,    L_OPTIONS_070,   HEAD_NATALYA,     0                          },
	/*0x61*/ { BODY_NATALYA_ELF,      L_OPTIONS_070,   HEAD_NATALYA,     0                          },
	/*0x62*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_DOAK_Z1,     0                          },
	/*0x63*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_DOAK_Z2,     0                          },
	/*0x64*/ { BODY_DD_GUARD,         L_OPTIONS_070,   HEAD_TERRORIST_Z, 0                          },
	/*0x65*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_DWAYNE_Z,    0                          },
	/*0x66*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_KARL_Z2,     0                          },
	/*0x67*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_DUNCAN_Z1,   0                          },
	/*0x68*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_DUNCAN_Z2,   0                          },
	/*0x69*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_GRANT_Z,     0                          },
	/*0x6a*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_ROBIN_Z,     0                          },
	/*0x6b*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_GRAEME_Z1,   0                          },
	/*0x6c*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_GRAEME_Z2,   0                          },
	/*0x6d*/ { BODY_OFFICEWORKER,     L_OPTIONS_070,   HEAD_KEN_LOBB_Z,  0                          },
	/*0x6e*/ { BODY_EVIL_CYBORG,       L_OPTIONS_070,   HEAD_EVIL_CYBORG,  0                          },
	/*0x6f*/ { BODY_ROBOT_SKELINGTON, L_OPTIONS_070,  HEAD_ROBOT_SKELINGTON, 0                    },
#endif
};
...
void mpCalculateAwards(void)
{
	...
	// no effect on IDO.
#ifdef AVOID_UB
	struct ranking playerrankings[MAX_MPCHRS_RUNTIME];
#else
	...
	for (i = 0; i < playercount; i++) {
		...
		for (j = 0; j < MAX_MPCHRS_RUNTIME; j++) {
#if VERSION >= VERSION_NTSC_1_0
			if (chrnum == j)
#else
			if (i == j)
		...
		if (!g_CheatsActiveBank0 && !g_CheatsActiveBank1) {
...
#endif
			{
				if (mpplayer->base.placement == 0) {
					...
					for (j = 0; j < MAX_MPCHRS_RUNTIME; j++) {
#if VERSION >= VERSION_NTSC_1_0
						if (g_MpSetup.chrslots & MPCHRSLOT_BIT(j))
#endif
						{
							struct mpchrconfig *othermpchr = MPCHR(j);
	...
	for (i = 0; i < playercount; i++) {
		for (j = 0; j < 17; j++) {
			if (metrics[i].awards & MPCHRSLOT_BIT(j));
		}
	}
	...
	if (numchrs >= 2) {
		...
		for (k = 0; k < MAX_MPCHRS_RUNTIME; k++) {
			if (g_MpSetup.chrslots & MPCHRSLOT_BIT(k)) {
				s32 totalkills = 0;
...
static void mpStartCustomResultMusic(const char *key)
{
	...
	extMusicStopForTrack(TRACKTYPE_6);

	if (!extMusicStartForKey(key, TRACKTYPE_6, musicGetVolume())) {
		/* Force a fresh stock result event even if g_MenuTrack is stale. */
		musicStartTrackAsMenu(MUSIC_NONE);
		musicStartTrackAsMenu(MUSIC_COMBATSIM_COMPLETE);
	}
}
...
void mpEndMatch(void)
{
	...
	s32 prevplayernum;
	s32 i;
	bool wasabort = mpDidAnyHumanAbortMatch();
	bool isterminationresult = g_MpSetup.scenario == MPSCENARIO_TERMINATION
		&& (g_ScenarioData.term.matchending || wasabort);

	if ((g_MpLmsCuratedStageIndex < 0 || g_MpLmsCuratedDifficultyIndex < 0)
			&& !isterminationresult) {
		musicStartMenu();
	}
	...
	if (isterminationresult) {
		bool result = g_ScenarioData.term.waswin;

		if (wasabort || g_CheatsActiveBank0 || g_CheatsActiveBank1) {
			result = false;
		}

		g_MpLmsCuratedHasResult = true;
		g_MpLmsCuratedWasWin = result;

		mpStartCustomResultMusic(result ? MD_MUSIC_KEY_TERMINATION_WIN : MD_MUSIC_KEY_TERMINATION_LOSE);

		mpLmsCuratedReset();
	} else if (g_MpLmsCuratedStageIndex >= 0 && g_MpLmsCuratedDifficultyIndex >= 0) {
		bool result = false;

		if (!mpDidAnyHumanAbortMatch() && !g_CheatsActiveBank0 && !g_CheatsActiveBank1) {
			struct ranking rankings[MAX_MPCHRS_RUNTIME];

			mpGetTeamRankings(rankings);
			result = rankings[0].teamnum == MPTEAM_0;
		}

		g_MpLmsCuratedHasResult = true;
		g_MpLmsCuratedWasWin = result;

		mpStartCustomResultMusic(result ? MD_MUSIC_KEY_LMS_WIN : MD_MUSIC_KEY_LMS_LOSE);

		if (result) {
			bool wascomplete = gamefileIsLmsDifficultyComplete(
					g_MpLmsCuratedStageIndex, g_MpLmsCuratedDifficultyIndex);

			gamefileSetLmsDifficultyComplete(
					g_MpLmsCuratedStageIndex, g_MpLmsCuratedDifficultyIndex, true);

			if (!wascomplete
					&& g_GameFileGuid.fileid != 0
...
void mpLmsCuratedReset(void)
{
	g_MpLmsCuratedPendingStageIndex = -1;
	g_MpLmsCuratedPendingDifficultyIndex = -1;
	g_MpLmsCuratedStageIndex = -1;
	g_MpLmsCuratedDifficultyIndex = -1;
}

void mpLmsCuratedSetPending(s32 stageindex, s32 difficultyindex)
{
	mpLmsCuratedClearResult();
	g_MpLmsCuratedPendingStageIndex = stageindex;
	g_MpLmsCuratedPendingDifficultyIndex = difficultyindex;
}

void mpLmsCuratedPromotePending(void)
{
	if (g_MpLmsCuratedPendingStageIndex >= 0 && g_MpLmsCuratedPendingDifficultyIndex >= 0) {
		g_MpLmsCuratedStageIndex = g_MpLmsCuratedPendingStageIndex;
		g_MpLmsCuratedDifficultyIndex = g_MpLmsCuratedPendingDifficultyIndex;
		g_MpLmsCuratedPendingStageIndex = -1;
		g_MpLmsCuratedPendingDifficultyIndex = -1;
	}
}

bool mpLmsCuratedHasResult(void)
{
	return g_MpLmsCuratedHasResult;
}

bool mpLmsCuratedWasResultWin(void)
{
	return g_MpLmsCuratedWasWin;
}

void mpLmsCuratedClearResult(void)
{
	g_MpLmsCuratedHasResult = false;
	g_MpLmsCuratedWasWin = false;
}
...
{
	if (headnum >= ARRAYCOUNT(g_MpHeads)) {
		headnum = 0;
	}
...
#ifndef PLATFORM_N64
	switch (mpbodynum) {
	case MPBODY_NATALYA: return "Natalya\n";
	case MPBODY_XENIA: return "Xenia\n";
	case MPBODY_OURUMOV: return "Ourumov\n";
	case MPBODY_ODDJOB: return "Oddjob\n";
	case MPBODY_TREVELYAN_JANUS: return "Trevelyan (Janus)\n";
	case MPBODY_NATALYA_CUBA: return "Natalya (Cuba)\n";
	case MPBODY_BOND_BROSNAN_CLASSIC: return "Bond (Brosnan)\n";
	case MPBODY_BOND_DALTON_CLASSIC: return "Bond (Dalton)\n";
	case MPBODY_BOND_CONNERY_CLASSIC: return "Bond (Connery)\n";
	case MPBODY_BOND_MOORE_CLASSIC: return "Bond (Moore)\n";
	case MPBODY_BOND_LAZENBY_CLASSIC: return "Bond (Lazenby)\n";
	case MPBODY_JAWS: return "Jaws\n";
	case MPBODY_SNOWGUARD: return "Siberian Special Forces\n";
	case MPBODY_BOND_ARCTIC: return "Bond (Arctic)\n";
	case MPBODY_MISHKIN_Z: return "Mishkin\n";
	case MPBODY_BOND_FORMAL: return "Bond (Formal)\n";
	case MPBODY_BOND_STEALTH: return "Bond (Stealth)\n";
	case MPBODY_TREVELYAN_006: return "Trevelyan (006)\n";
	case MPBODY_BOND_JUNGLE: return "Bond (Jungle)\n";
	case MPBODY_BORIS: return "Boris\n";
	case MPBODY_VALENTIN: return "Valentin\n";
	case MPBODY_MAYDAY: return "May Day\n";
	case MPBODY_BARON_SAMEDI: return "Baron Samedi\n";
	case MPBODY_BARON_SAMEDI_DEATH: return "Baron Samedi (Death)\n";
	case MPBODY_SANTA: return "Santa Claus\n";
	case MPBODY_ELF: return "Elf\n";
	case MPBODY_PILOT_GE: return "Helicopter Pilot\n";
	case MPBODY_MARION: return "Rosika/Marion\n";
	case MPBODY_BOND_CONNERY_ALPS: return "Bond (Classic, Alps)\n";
	case MPBODY_BOND_CONNERY_RANCH: return "Bond (Classic, Ranch)\n";
	case MPBODY_GALORE_PLANE: return "Galore (Plane)\n";
	case MPBODY_GALORE_RANCH: return "Galore (Ranch)\n";
	case MPBODY_SOGUN: return "Sogun\n";
	case MPBODY_NATALYA_SANTA: return "Natalya (Santa)\n";
	case MPBODY_NATALYA_ELF: return "Natalya (Elf)\n";
	case MPBODY_DOAK_Z1: return "Dr. Doak 1\n";
	case MPBODY_DOAK_Z2: return "Dr. Doak 2\n";
	case MPBODY_TERRORIST_Z: return "Terrorist\n";
	case MPBODY_DWAYNE_Z: return "Dwayne\n";
	case MPBODY_KARL_Z2: return "Karl Hilton 2\n";
	case MPBODY_DUNCAN_Z1: return "Duncan Botwood 1\n";
	case MPBODY_DUNCAN_Z2: return "Duncan Botwood 2\n";
	case MPBODY_GRANT_Z: return "Grant Kirkhope\n";
	case MPBODY_ROBIN_Z: return "Robin Beanland\n";
	case MPBODY_GRAEME_Z1: return "Graeme Norgate 1\n";
	case MPBODY_GRAEME_Z2: return "Graeme Norgate 2\n";
	case MPBODY_KEN_LOBB_Z: return "Ken Lobb\n";
	case MPBODY_EVIL_CYBORG: return MD_MPMENU_LITERAL_TERMINATION_BODY;
	case MPBODY_ROBOT_SKELINGTON: return "Robot Skelington\n";
	}
#endif
...
s32 mpChooseRandomLockPlayer(void)
{
	...
	for (i = (start + 1) % 4;; i = (i + 1) % 4) {
		if ((g_MpSetup.chrslots & MPCHRSLOT_BIT(i)) || i == start) {
			break;
		}
	}
...
void mpCalculateLockIfLastWinnerOrLoser(void)
{
	...
	if (g_MpLockInfo.lockedplayernum >= 0
			...
			&& (g_MpSetup.chrslots & MPCHRSLOT_BIT(g_MpLockInfo.lockedplayernum)) == 0) {
...
struct mptrack g_MpTracks[] = {
	...
	/*0x00*/ { MUSIC_CI_OPERATIVE,    170, L_MISC_126, -1 }, // "CI Operative"
	/*0x01*/ { MUSIC_SKEDAR_MYSTERY,  170, L_MISC_125, -1 }, // "Skedar Mystery"
	/*0x02*/ { MUSIC_DARK_COMBAT,     160, L_MISC_124, -1 }, // "Dark Combat"
	...
	/*0x0a*/ { MUSIC_EXTRACTION,      120, L_MISC_135, SOLOSTAGEINDEX_EXTRACTION }, // "dD Extraction"
	/*0x0b*/ { MUSIC_INVESTIGATION_X, 120, L_MISC_134, SOLOSTAGEINDEX_INVESTIGATION }, // "dD Research X"
	/*0x0c*/ { MUSIC_VILLA,           120, L_MISC_137, SOLOSTAGEINDEX_VILLA }, // "Carrington Villa"
	/*0x0d*/ { MUSIC_VILLA_X,         120, L_MISC_138, SOLOSTAGEINDEX_VILLA }, // "Carrington Villa X"
	/*0x0e*/ { MUSIC_AIRFORCEONE_X,   120, L_MISC_152, SOLOSTAGEINDEX_AIRFORCEONE }, // "Air Force One X"
	/*0x0f*/ { MUSIC_EXTRACTION_X,    120, L_MISC_136, SOLOSTAGEINDEX_EXTRACTION }, // "dD Extraction X"
	...
	/*0x14*/ { MUSIC_RESCUE_X,        120, L_MISC_146, SOLOSTAGEINDEX_RESCUE }, // "A51 Rescue X"
	/*0x15*/ { MUSIC_AIRFORCEONE,     120, L_MISC_151, SOLOSTAGEINDEX_AIRFORCEONE }, // "Air Force One"
	/*0x16*/ { MUSIC_PELAGIC,         120, L_MISC_155, SOLOSTAGEINDEX_PELAGIC }, // "Pelagic II"
	/*0x17*/ { MUSIC_INFILTRATION_X,  120, L_MISC_144, SOLOSTAGEINDEX_INFILTRATION }, // "A51 Infiltration X"
	...
	/*0x1b*/ { MUSIC_CRASHSITE,       120, L_MISC_153, SOLOSTAGEINDEX_CRASHSITE }, // "Crash Site"
	/*0x1c*/ { MUSIC_CRASHSITE_X,     120, L_MISC_154, SOLOSTAGEINDEX_CRASHSITE }, // "Crash Site X"
	/*0x1d*/ { MUSIC_RESCUE,          120, L_MISC_145, SOLOSTAGEINDEX_RESCUE }, // "A51 Rescue"
	/*0x1e*/ { MUSIC_ESCAPE,          120, L_MISC_147, SOLOSTAGEINDEX_ESCAPE }, // "A51 Escape"
	/*0x1f*/ { MUSIC_CHICAGO,         120, L_MISC_139, SOLOSTAGEINDEX_CHICAGO }, // "Chicago"
...
static const s32 g_MpTerminationTrackNums[] = {
	0x12, // G5 Building X
	0x11, // G5 Building
	0x10, // Chicago X
	0x0f, // dD Extraction X
	0x01, // Skedar Mystery
	0x00, // CI Operative
	0x02, // Dark Combat
};
...
bool mpIsTrackUnlocked(s32 tracknum)
{
	...
	if (gamefileHasFlag(GAMEFILEFLAG_MP_ONLY_UNLOCKS)) {
		return true;
	}
...
{
	g_BossFile.tracknum = MPTRACK_RANDOM;
}

void mpSetTrackToNone(void)
{
	g_BossFile.tracknum = MPTRACK_NONE;
}
...
s32 mpChooseTrack(void)
{
	...
	if (!mpGetUsingMultipleTunes() && g_BossFile.tracknum == MPTRACK_NONE) {
		g_MpLockInfo.unk04 = -1;
		g_MusicLife60 = 0;
		return MUSIC_NONE;
	}
	...
	if (g_MpSetup.scenario == MPSCENARIO_TERMINATION) {
		do {
			tracknum = g_MpTerminationTrackNums[rngRandom() % ARRAYCOUNT(g_MpTerminationTrackNums)];
		} while (ARRAYCOUNT(g_MpTerminationTrackNums) > 1 && tracknum == g_MpLockInfo.unk04);
...
void mpCreateBotFromProfile(s32 botnum, u8 profilenum)
{
	...
	g_MpSetup.chrslots |= MPCHRSLOT_BIT(mpBotSlotToChrSlot(botnum));
	strcpy(g_BotConfigsArray[botnum].base.name, "Sim\n");
	g_BotConfigsArray[botnum].base.displayoptions &= ~MPDISPLAYOPTION_CUSTOMBOTNAME;
...
bool mpBotHasCustomName(s32 botnum)
{
	return (g_BotConfigsArray[botnum].base.displayoptions & MPDISPLAYOPTION_CUSTOMBOTNAME) != 0;
}

void mpRandomizeBotName(s32 botnum)
{
	static const char *g_RandomBotNames[] = {
		"ALEX",
		"SAM",
		"MAX",
		"JACK",
	...
	mpSetBotCustomName(botnum, g_RandomBotNames[rngRandom() % ARRAYCOUNT(g_RandomBotNames)]);
}

void mpSetBotCustomName(s32 botnum, const char *name)
{
	s32 i = 0;

	while (i < 13 && name[i] != '\0' && name[i] != '\n') {
		g_BotConfigsArray[botnum].base.name[i] = name[i];
		i++;
	}

	if (i == 0) {
		g_BotConfigsArray[botnum].base.name[0] = '\0';
		g_BotConfigsArray[botnum].base.displayoptions &= ~MPDISPLAYOPTION_CUSTOMBOTNAME;
		mpGenerateBotNames();
		return;
	}

	g_BotConfigsArray[botnum].base.name[i] = '\n';
	i++;
...
/**
 * Return the first unused bot slot, or the last slot if all are full.
 *
...
s32 mpGetSlotForNewBot(void)
{
	...
	while (i < MAX_BOTS_RUNTIME - 1 && g_MpSetup.chrslots & MPCHRSLOT_BIT(mpBotSlotToChrSlot(i))) {
		i++;
	}
...
void mpRemoveSimulant(s32 index)
{
	...
	g_MpSetup.chrslots &= ~MPCHRSLOT_BIT(mpBotSlotToChrSlot(index));
	g_BotConfigsArray[index].base.name[0] = '\0';
	g_BotConfigsArray[index].base.displayoptions &= ~MPDISPLAYOPTION_CUSTOMBOTNAME;
	func0f1881d4(index);
	mpGenerateBotNames();

	if ((g_MpSetup.chrslots & MP_SIM_SLOTS_MASK) == 0
...
void mpCopySimulant(s32 index)
{
	...
	g_MpSetup.chrslots |= MPCHRSLOT_BIT(mpBotSlotToChrSlot(dest));
	strcpy(g_BotConfigsArray[dest].base.name, g_BotConfigsArray[index].base.name);
	...
	g_BotConfigsArray[dest].base.team = g_BotConfigsArray[index].base.team;
	g_BotConfigsArray[dest].base.displayoptions = g_BotConfigsArray[index].base.displayoptions;
	...
	for (i = 0; i < MAX_PLAYERS; i++) {
		g_MpSimulantDifficultiesPerNumPlayers[dest][i] = g_MpSimulantDifficultiesPerNumPlayers[index][i];
	}
...
{
		s32 numvacant = challengeIsFeatureUnlocked(MPFEATURE_8BOTS) ? MAX_BOTS_RUNTIME : 4;
	s32 i;

		for (i = 4; i < MAX_MPCHRS_RUNTIME; i++) {
		if (g_MpSetup.chrslots & MPCHRSLOT_BIT(i)) {
			numvacant--;
		}
	}
...
{
	s32 numfree = MAX_BOTS_RUNTIME;
	s32 i;

	if ((g_MpSetup.chrslots & MPCHRSLOT_BIT(mpBotSlotToChrSlot(slot))) == 0) {
		for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
			if (g_MpSetup.chrslots & MPCHRSLOT_BIT(mpBotSlotToChrSlot(i))) {
				numfree--;
			}
		}
...
void mpplayerfileGetOverview(char *arg0, char *name, u32 *playtime)
{
	...
	memcpy(buffer.bytes, arg0, sizeof(buffer.bytes));
...
{
#ifndef PLATFORM_N64
	s32 slotindex = fileid > 0 ? fileid - 1 : -1;

	slotindex = mpplayersSavePlayer(playernum, slotindex);

	if (slotindex >= 0) {
		g_PlayerConfigsArray[playernum].fileguid.fileid = slotindex + 1;
		g_PlayerConfigsArray[playernum].fileguid.deviceserial = 1;
		return 0;
	}
...
#ifndef PLATFORM_N64
	if (fileid > 0) {
		return mpplayersLoadPlayer(playernum, fileid - 1);
	}
...
void mpApplyConfig(struct mpconfigfull *config)
{
	...
	s32 j;
	u32 chrslots;
	s32 configbotcount = mpGetConfigBotCount();
...
#if VERSION >= VERSION_JPN_FINAL
		g_MpSetup.chrslots &= MP_PLAYER_SLOTS_MASK;

		for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
		if (g_BotConfigsArray[i].difficulty != BOTDIFF_DISABLED) {
				g_MpSetup.chrslots |= MPCHRSLOT_BIT(i + 4);
		}
	}
#endif
}
...
static s32 mpsetupGetSerializedBotCount(u8 version)
{
	return version >= 5 ? MAX_BOTS_RUNTIME : version >= 3 ? 16 : version >= 2 ? 13 : MAX_BOTS;
}

static s32 mpsetupGetSerializedNumSimsBitCount(u8 version)
{
	return version >= 3 ? 5 : 4;
}

static s32 mpsetupGetSerializedScenarioBitCount(u8 version)
{
	return version >= 7 ? 4 : 3;
}
...
void mpsetupfileLoadWad(struct savebuffer *buffer, u8 version)
{
	s32 i;
	s32 j;
	s32 serializedbotcount = mpsetupGetSerializedBotCount(version);
	...
	savebufferReadBits(buffer, mpsetupGetSerializedNumSimsBitCount(version));

	g_MpSetup.stagenum = savebufferReadBits(buffer, 7);
	g_MpSetup.scenario = savebufferReadBits(buffer, mpsetupGetSerializedScenarioBitCount(version));
	...
	for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
		...
		g_BotConfigsArray[i].base.mpbodynum = 0;
		g_BotConfigsArray[i].base.team = MPTEAM_1;
		g_BotConfigsArray[i].base.displayoptions &= ~MPDISPLAYOPTION_CUSTOMBOTNAME;

		if (i < serializedbotcount) {
	...
	if (version >= 6) {
		for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
			...
			if (i < serializedbotcount) {
				hascustomname = savebufferReadBits(buffer, 1);
			}

			if (hascustomname) {
				savebufferReadString_ext(buffer, g_BotConfigsArray[i].base.name, true, sizeof(g_BotConfigsArray[i].base.name));
				g_BotConfigsArray[i].base.displayoptions |= MPDISPLAYOPTION_CUSTOMBOTNAME;
			}
		}
	}
...
void mpsetupfileSaveWad(struct savebuffer *buffer)
{
	...
	savebufferWriteString_ext(buffer, g_MpSetup.name, MPSETUP_MAXNAME + 1);
		...
		for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
		if (g_MpSetup.chrslots & MPCHRSLOT_BIT(i + 4)) {
			numsims++;
		}
	}

	savebufferOr(buffer, numsims, mpsetupGetSerializedNumSimsBitCount(7));
	savebufferOr(buffer, g_MpSetup.stagenum, 7);
	savebufferOr(buffer, g_MpSetup.scenario, mpsetupGetSerializedScenarioBitCount(7));
	...
	for (i = 0; i < MAX_BOTS_RUNTIME; i++) {
		savebufferOr(buffer, mpBotHasCustomName(i) ? 1 : 0, 1);

		if (mpBotHasCustomName(i)) {
			savebufferWriteString_ext(buffer, g_BotConfigsArray[i].base.name, sizeof(g_BotConfigsArray[i].base.name));
		}
	}
}

void mpsetupfileGetOverview(char *arg0, u8 version, char *filename, u16 *numsims, u16 *stagenum, u16 *scenarionum)
{
	struct savebuffer buffer;
	...
	*numsims = savebufferReadBits(&buffer, mpsetupGetSerializedNumSimsBitCount(version));
	*stagenum = savebufferReadBits(&buffer, 7);
	*scenarionum = savebufferReadBits(&buffer, mpsetupGetSerializedScenarioBitCount(version));
}
