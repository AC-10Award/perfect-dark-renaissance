...
#include "data.h"
#include "types.h"
#include "md/scenario_aliases.h"

/**
 * There are several multiplayer scenarios:
 *
 * - Combat
...
MenuItemHandlerResult menuhandlerMpOneHitHeadshots(s32 operation, struct menuitem *item, union handlerdata *data)
{
	return menuhandlerMpCheckboxOption(operation, item, data);
}
...
#include "scenarios/rescuethehostages.inc"
#include "scenarios/lastmanstanding.inc"
#include "scenarios/termination.inc"
...
struct mpscenariooverview g_MpScenarioOverviews[] = {
	...
	{ MD_MPSCENARIO_TEXT_COMBAT_NAME,           MD_MPSCENARIO_TEXT_COMBAT_SHORT,           0,                      false },
	{ MD_MPSCENARIO_TEXT_BRIEFCASE_NAME,        MD_MPSCENARIO_TEXT_BRIEFCASE_SHORT,        MPFEATURE_SCENARIO_HTB, false },
	{ MD_MPSCENARIO_TEXT_HACKER_CENTRAL_NAME,   MD_MPSCENARIO_TEXT_HACKER_CENTRAL_SHORT,   MPFEATURE_SCENARIO_HTM, false },
	{ MD_MPSCENARIO_TEXT_POP_A_CAP_NAME,        MD_MPSCENARIO_TEXT_POP_A_CAP_SHORT,        MPFEATURE_SCENARIO_PAC, false },
	{ MD_MPSCENARIO_TEXT_KING_OF_THE_HILL_NAME, MD_MPSCENARIO_TEXT_KING_OF_THE_HILL_SHORT, MPFEATURE_SCENARIO_KOH, true  },
	{ MD_MPSCENARIO_TEXT_CAPTURE_THE_CASE_NAME, MD_MPSCENARIO_TEXT_CAPTURE_THE_CASE_SHORT, MPFEATURE_SCENARIO_CTC, true  },
	{ 0,            0,            0,                       false },
	{ 0,            0,            0,                       false },
	{ 0,            0,            0,                       false },
};

char *scenarioGetNameByNum(s32 scenarionum)
{
	if (scenarionum == MD_MPSCENARIO_RESCUE_HOSTAGES) {
		return langGet(MD_MPSCENARIO_TEXT_RESCUE_HOSTAGES_NAME);
	}

	if (scenarionum == MD_MPSCENARIO_LAST_MAN_STANDING) {
		return langGet(MD_MPSCENARIO_TEXT_LAST_MAN_STANDING_NAME);
	}

	if (scenarionum == MD_MPSCENARIO_TERMINATION) {
		return langGet(MD_MPSCENARIO_TEXT_TERMINATION_NAME);
	}

	return langGet(g_MpScenarioOverviews[scenarionum].name);
}

char *scenarioGetShortNameByNum(s32 scenarionum)
{
	if (scenarionum == MD_MPSCENARIO_RESCUE_HOSTAGES) {
		return langGet(MD_MPSCENARIO_TEXT_RESCUE_HOSTAGES_SHORT);
	}

	if (scenarionum == MD_MPSCENARIO_LAST_MAN_STANDING) {
		return langGet(MD_MPSCENARIO_TEXT_LAST_MAN_STANDING_SHORT);
	}

	if (scenarionum == MD_MPSCENARIO_TERMINATION) {
		return langGet(MD_MPSCENARIO_TEXT_TERMINATION_SHORT);
	}

	return langGet(g_MpScenarioOverviews[scenarionum].shortname);
}
...
{
	sprintf(g_StringPointer, "%s\n", scenarioGetShortNameByNum(g_MpSetup.scenario));
	return g_StringPointer;
}

char *mpMenuTextScenarioName(struct menuitem *item)
{
	sprintf(g_StringPointer, "%s\n", scenarioGetNameByNum(g_MpSetup.scenario));
	return g_StringPointer;
}
...
static bool scenarioIsVisibleInMenu(s32 scenarionum, bool teamgame)
{
	return challengeIsFeatureUnlocked(g_MpScenarioOverviews[scenarionum].requirefeature)
		&& (teamgame || g_MpScenarioOverviews[scenarionum].teamonly == false);
}

static s32 scenarioGetMenuGroup(s32 scenarionum)
{
	if (scenarionum == MD_MPSCENARIO_LAST_MAN_STANDING || scenarionum == MD_MPSCENARIO_TERMINATION) {
		return 2;
	}

	return g_MpScenarioOverviews[scenarionum].teamonly ? 1 : 0;
}
...
MenuItemHandlerResult scenarioScenarioMenuHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	struct scenariogroup groups[] = {
		{ 0, MD_MPSCENARIO_GROUP_FREE_FOR_ALL },
		{ 0, MD_MPSCENARIO_GROUP_TEAMWORK     },
		{ 0, MD_MPSCENARIO_GROUP_PVE_MODES    },
	};
	s32 visiblegroups[ARRAYCOUNT(groups)];
	s32 numvisiblegroups = 0;
	...
	for (i = 0; i < ARRAYCOUNT(g_MpScenarioOverviews); i++) {
		if (scenarioIsVisibleInMenu(i, teamgame)) {
			s32 group = scenarioGetMenuGroup(i);
			s32 j;
			bool found = false;

			for (j = 0; j < numvisiblegroups; j++) {
				if (visiblegroups[j] == group) {
					found = true;
					break;
				}
			}

			if (!found && numvisiblegroups < ARRAYCOUNT(visiblegroups)) {
				visiblegroups[numvisiblegroups] = group;
				numvisiblegroups++;
			}
		}
	}
		...
		for (i = 0; i < ARRAYCOUNT(g_MpScenarioOverviews); i++) {
			if (scenarioIsVisibleInMenu(i, teamgame)) {
				if (count == data->list.value) {
					return (uintptr_t)scenarioGetNameByNum(i);
				}
		...
		for (i = 0; i < ARRAYCOUNT(g_MpScenarioOverviews); i++) {
			if (scenarioIsVisibleInMenu(i, teamgame)) {
				if (count == data->list.value) {
					...
					if (g_MpSetup.scenario == MD_MPSCENARIO_LAST_MAN_STANDING) {
						g_MpSetup.stagenum = STAGE_EXTRA9;
					}
					break;
				}
	...
	case MENUOP_GETOPTGROUPCOUNT:
		data->list.value = numvisiblegroups;
		break;
	case MENUOP_GETOPTGROUPTEXT:
		if (data->list.value >= 0 && data->list.value < numvisiblegroups) {
			s32 groupindex = visiblegroups[data->list.value];

			if (groupindex == 2) {
				return (uintptr_t)langGet(MD_MPSCENARIO_GROUP_PVE_MODES);
			}

			return (uintptr_t)langGet(groups[groupindex].textid);
		}
		break;
	case MENUOP_GETGROUPSTARTINDEX:
		if (data->list.value >= 0 && data->list.value < numvisiblegroups) {
			...
			for (i = 0; i < ARRAYCOUNT(g_MpScenarioOverviews); i++) {
				if (scenarioIsVisibleInMenu(i, teamgame) && scenarioGetMenuGroup(i) < groupindex) {
					count++;
				}
			}
		}
...
bool scenarioIsPlayerInputLocked(s32 playernum)
{
	if (!g_Vars.normmplayerisrunning) {
		return false;
	}

	if (g_MpSetup.scenario == MPSCENARIO_RESCUEHOSTAGES
			&& playernum >= 0
			&& playernum < PLAYERCOUNT()) {
		return g_ScenarioData.rth.activedefusehostage[playernum] >= 0;
	}

	return false;
}

s32 scenarioGetReservedSimulantSlots(void)
{
	if (!g_Vars.normmplayerisrunning) {
		return 0;
	}
	...
	if (g_MpSetup.scenario == MPSCENARIO_RESCUEHOSTAGES) {
		return g_ScenarioData.rth.numhostages;
	}

	return 0;
}

s32 scenarioGetExtraChrSlots(void)
{
	if (!g_Vars.normmplayerisrunning) {
		return 0;
	}
	...
	if (g_MpSetup.scenario == MPSCENARIO_RESCUEHOSTAGES) {
		/*
		 * Rescue hostages use real chr props, and a rescued hostage briefly
		 * persists as an escaping chr while its replacement is spawned. Reserve
		 * headroom for both the seated hostage set and the temporary escape set.
		 */
		return g_ScenarioData.rth.numhostages * 2;
	}
...
void scenarioReset(void)
{
	...
	case MPSCENARIO_HOLDTHEBRIEFCASE:
		htbReset();
		break;
	case MPSCENARIO_RESCUEHOSTAGES:
		rthReset();
		break;
	case MPSCENARIO_TERMINATION:
	case MPSCENARIO_LASTMANSTANDING:
...
void scenarioHandleActivatedProp(struct chrdata *chr, struct prop *prop)
{
	...
	} else if (g_MpSetup.scenario == MPSCENARIO_RESCUEHOSTAGES) {
		struct defaultobj *obj = prop->obj;

		if (obj->flags3 & OBJFLAG3_RESCUEHOSTAGE) {
			s32 i;
			u32 mpindex = mpPlayerGetIndex(chr);

			for (i = 0; i < ARRAYCOUNT(g_ScenarioData.rth.hostages); i++) {
				if (g_ScenarioData.rth.hostages[i] == prop) {
					if (g_ScenarioData.rth.activehostage[mpindex] != i) {
						g_ScenarioData.rth.activehostage[mpindex] = i;
						g_ScenarioData.rth.rescuetime240[mpindex] = 0;
					}

#if VERSION >= VERSION_JPN_FINAL
					hudmsgCreateWithFlags("Rescuing hostage.\n", HUDMSGTYPE_MPSCENARIO, HUDMSGFLAG_ONLYIFALIVE | HUDMSGFLAG_NOWRAP);
#else
