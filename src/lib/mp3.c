...
#ifndef PLATFORM_N64
	static s32 logged_mp3_load = 0;
	static s32 logged_mp3_env = 0;
	static s32 logged_mp3_load_done = 0;
	static s32 logged_mp3_return = 0;
#endif
...
#else
					// On PC this is a direct function call, so pass the host pointer.
					acmd07((*cmd)++, g_Mp3Vars.romaddr, g_Mp3Vars.filesize, sp58, g_Mp3Vars.reset);
					g_Mp3Vars.reset = 0;
#endif
...
#ifndef PLATFORM_N64
			if (!logged_mp3_load) {
				sysLogPrintf(LOG_NOTE, "ext_music: mp3 loadbuffer pre cached=%p bytes=%d destL=%d stereo=%d", sp54, sp50 + sp50, sp4c, g_Mp3Vars.var8009c3f1);
				logged_mp3_load = 1;
			}
#endif
#ifdef PLATFORM_N64
...
#else
			n_aLoadBuffer((*cmd)++, sp50 + sp50, sp4c, sp54);
#endif
...
#else
				n_aLoadBuffer((*cmd)++, sp50 + sp50, sp48, sp54);
#endif
			}

#ifndef PLATFORM_N64
			if (!logged_mp3_load_done) {
				sysLogPrintf(LOG_NOTE, "ext_music: mp3 loadbuffer done cached bytes=%d destL=%d destR=%d stereo=%d", sp50 + sp50, sp4c, sp48, g_Mp3Vars.var8009c3f1);
				logged_mp3_load_done = 1;
			}
#endif
...
#ifndef PLATFORM_N64
			if (!logged_mp3_load) {
				sysLogPrintf(LOG_NOTE, "ext_music: mp3 loadbuffer pre streaming=%p bytes=%d destL=%d stereo=%d", sp54, arg0 + arg0, sp4c, g_Mp3Vars.var8009c3f1);
				logged_mp3_load = 1;
			}
#endif
#ifdef PLATFORM_N64
...
#else
			n_aLoadBuffer((*cmd)++, arg0 + arg0, sp4c, sp54);
#endif
...
#else
				n_aLoadBuffer((*cmd)++, arg0 + arg0, sp48, sp54);
#endif
			}

#ifndef PLATFORM_N64
			if (!logged_mp3_load_done) {
				sysLogPrintf(LOG_NOTE, "ext_music: mp3 loadbuffer done streaming bytes=%d destL=%d destR=%d stereo=%d", arg0 + arg0, sp4c, sp48, g_Mp3Vars.var8009c3f1);
				logged_mp3_load_done = 1;
			}
#endif
...
#ifndef PLATFORM_N64
				if (!logged_mp3_env) {
					sysLogPrintf(LOG_NOTE, "ext_music: mp3 envmixer pre state=%p vol=%d", g_Mp3Vars.var8009c398, g_Mp3Vars.ivol2);
					logged_mp3_env = 1;
				}
#endif
#ifdef PLATFORM_N64
...
#else
				n_aEnvMixer((*cmd)++, 1, g_Mp3Vars.ivol2, g_Mp3Vars.var8009c398);
#endif
			} else {
#ifndef PLATFORM_N64
				if (!logged_mp3_env) {
					sysLogPrintf(LOG_NOTE, "ext_music: mp3 envmixer pre continue state=%p", g_Mp3Vars.var8009c398);
					logged_mp3_env = 1;
				}
#endif
#ifdef PLATFORM_N64
...
#else
				n_aEnvMixer((*cmd)++, 0, 0, g_Mp3Vars.var8009c398);
#endif
			}
...
#ifndef PLATFORM_N64
	if (!logged_mp3_return) {
		sysLogPrintf(LOG_NOTE, "ext_music: func00037fc0 returning ok stereo=%d d0=%d cc=%d", g_Mp3Vars.var8009c3f1, g_Mp3Vars.var8009c3d0, g_Mp3Vars.var8009c3cc);
		logged_mp3_return = 1;
	}
#endif
...
#else
	bcopy((u8 *)(g_Mp3Vars.romaddr + g_Mp3Vars.var8009c3c4), arg1, arg2);
#endif
...
#else
	// On PC the mixer decodes directly from g_Mp3Vars.romaddr as a host pointer.
	// ROM-style audio DMA prefetch is unnecessary here and breaks external
	// heap-loaded MP3s such as ext_music/menu.mp3.
#endif
}
