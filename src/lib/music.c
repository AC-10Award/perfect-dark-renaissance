...
s32 musicHandlePlayEvent(struct musicevent *event, s32 result)
{
	...
	if (result == RESULT_FAIL) {
		...
		/*
		 * Future external music hook belongs here, before seqPlay(). This is the
		 * single low-level convergence point for queued PRIMARY / AMBIENT / NRG /
		 * MENU / DEATH play events, so an ext_music MP3 layer can intercept them
		 * without patching many separate gameplay call sites.
		 */
		exthandled = extMusicStartForEvent(event->tracktype, event->tracknum, event->volume);

		if (extMusicIsAnyTrackActive() && !exthandled) {
			extMusicStopAll();
		}
		...
		/*
		 * MUSIC_TERMINATION_MENU is a synthetic external-music slot and has no
		 * corresponding sequence in the ROM. If the user has not supplied a
		 * termination_menu MP3, fall back before the stock sequencer sees it.
		 */
		if (event->tracknum == MUSIC_TERMINATION_MENU) {
			event->tracknum = MUSIC_COMBATSIM_MENU;
		}
...
s32 musicHandleStopEvent(struct musicevent *event, s32 result)
{
	...
	extMusicStopForTrack(event->tracktype);
...
s32 musicHandleFadeEvent(struct musicevent *event, s32 result)
{
	...
	// Prototype behavior: external menu music stops immediately on fade/stop.
	extMusicStopForTrack(event->tracktype);
