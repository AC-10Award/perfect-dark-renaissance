...
#ifndef PLATFORM_N64
static bool sndIsExternalMusicActive(void)
{
	return extMusicIsTrackActive(TRACKTYPE_PRIMARY)
		|| extMusicIsTrackActive(TRACKTYPE_MENU);
}
#endif
...
void sndLoadSfxCtl(void)
{
	...
	// The final one might be a null termination?
...
bool sndStartMp3FromMemory(uintptr_t romaddr, u32 romsize, s32 volume, s32 pan)
{
	if (g_SndDisabled || !g_SndMp3Enabled || romaddr == 0 || romsize == 0) {
		return false;
	}
