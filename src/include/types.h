...
struct aibot {
	...
	/*0x130*/ s8 chrnumsbydistanceasc[MAX_MPCHRS_RUNTIME];
	/*0x13c*/ f32 chrdistances[MAX_MPCHRS_RUNTIME];
	/*0x16c*/ u8 chrsinsight[MAX_MPCHRS_RUNTIME];
	/*0x178*/ s32 chrslastseen60[MAX_MPCHRS_RUNTIME];
	/*0x1a8*/ RoomNum chrrooms[MAX_MPCHRS_RUNTIME];
...
struct player {
	...
	/*0x1be7*/ u8 aibuddynums[MAX_BOTS_RUNTIME];
...
struct gamefile {
	...
	/*0xbb*/ u8 lmscompletions[LMS_STAGE_COUNT];
};
...
struct mpchrconfig {
	...
	/*0x24*/ s16 killcounts[MAX_MPCHRS_RUNTIME]; // per player - each index is a chrslot
	/*0x3c*/ s16 numdeaths;
	/*0x3e*/ s16 numpoints;
	/*0x40*/ s16 unk40;
};
...
struct mpsetup {
	...
	/*0x800acb9e*/ u8 deathlimit;

		/**
		 * Each bit signifies that a player or sim is participating.
		 *
		 * Bits 0-3 are players.
		 * Bits 4+ are simulants.
		 */
	/*0x800acba0*/ u32 chrslots;
	/*0x800acba4*/ u8 weapons[NUM_MPWEAPONSLOTS];
...
struct bossfile {
	...
	/*0x63*/ u8 unk8b;
	/*0x64*/ s8 tracknum; // -1 = random, -2 = no music
...
struct scenariodata_htm {
	...
	/*0x800ac1e8*/ s32 numpoints[MAX_MPCHRS_RUNTIME];
	/*0x800ac218*/ s32 dltime240[MAX_MPCHRS_RUNTIME];
...
struct scenariodata_pac {
	...
	u16 age240;
	s32 victimindex;
	s16 victims[MAX_MPCHRS_RUNTIME]; // shuffled list of player numbers
	s16 killcounts[MAX_MPCHRS_RUNTIME]; // indexed by player num
	s16 survivalcounts[MAX_MPCHRS_RUNTIME]; // indexed by player num
};
...
struct scenariodata_rth {
	s16 numpads;
	s16 numhostages;
	s16 padnums[16];
	s16 hostagepadnums[10];
	struct prop *hostages[10];
	struct prop *hostagebombprops[10][3];
	struct prop *hostagechrprops[10];
	s32 executiontimers240[10];
	s32 vaporizewaittimers240[10];
	s32 deadhostagetimers240[10];
	struct prop *escapinghostagechrprops[10];
	s32 escapinghostagetimers240[10];
	u8 escapinghostagestates[10];
	struct coord escapinghostagedstpos[10];
	RoomNum escapinghostagedstrooms[10][8];
	s16 activehostage[MAX_PLAYERS];
	s32 rescuetime240[MAX_PLAYERS];
	s32 rescuedcount[MAX_PLAYERS];
	s32 lostpenaltycount[MAX_PLAYERS];
	s16 activedefusehostage[MAX_PLAYERS];
	u8 defuseselectedwire[MAX_PLAYERS];
	u8 defusetargetwire[MAX_PLAYERS];
	s32 defusestickcooldown240[MAX_PLAYERS];
	s16 hostageslost;
	u8 matchending;
	struct sndstate *rescueaudiohandles[MAX_PLAYERS];
	struct sndstate *rescueaudiohandles2[MAX_PLAYERS];
	struct sndstate *executionbeepaudiohandles[10];
};

struct scenariodata_lms {
	u8 lastbotdifficulties[MAX_BOTS_RUNTIME];
};

struct scenariodata_termination {
	u8 matchending;
	u8 waswin;
	u8 bossdefeated;
	u8 transformed;
...
struct scenariodata {
	union {
		...
		struct scenariodata_rth rth;
		struct scenariodata_lms lms;
		struct scenariodata_termination term;
	};
};
