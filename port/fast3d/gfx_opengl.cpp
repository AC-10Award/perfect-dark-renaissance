...
static bool gl_anisotropic_filtering_supported = false;
static GLfloat gl_max_texture_anisotropy = 1.0f;

extern "C" uint32_t gfx_texture_anisotropy_level;
...
static void gfx_opengl_set_sampler_parameters(int tile, bool linear_filter, uint32_t cms, uint32_t cmt) {
    ...
    if (gl_anisotropic_filtering_supported) {
        GLfloat anisotropy = 1.0f;

        if (filter == GL_LINEAR && gfx_texture_anisotropy_level > 1) {
            anisotropy = (GLfloat)gfx_texture_anisotropy_level;

            if (anisotropy > gl_max_texture_anisotropy) {
                anisotropy = gl_max_texture_anisotropy;
            }
        }
...
static void gfx_opengl_log_info(void) {
    ...
    sysLogPrintf(LOG_NOTE, "GL: texture_filter_anisotropic: %s (max %.1fx)",
        gl_anisotropic_filtering_supported ? "yes" : "no",
        gl_max_texture_anisotropy);
}
...
static void gfx_opengl_init_extensions(void) {
    ...
    gl_anisotropic_filtering_supported =
        SDL_GL_ExtensionSupported("GL_EXT_texture_filter_anisotropic") ||
        SDL_GL_ExtensionSupported("GL_ARB_texture_filter_anisotropic");

    if (gl_anisotropic_filtering_supported) {
        glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &gl_max_texture_anisotropy);

        if (gl_max_texture_anisotropy < 1.0f) {
            gl_max_texture_anisotropy = 1.0f;
        }
    } else {
        gl_max_texture_anisotropy = 1.0f;
    }
