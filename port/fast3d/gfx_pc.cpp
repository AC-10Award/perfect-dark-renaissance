...
static std::unordered_set<uint64_t> g_logged_ext_texture_misses;
static std::unordered_set<uint64_t> g_logged_ext_texture_setinfo;
static std::unordered_set<uint64_t> g_logged_ext_texture_loads;
static std::unordered_set<uint64_t> g_logged_ext_texture_binds;
static std::unordered_set<uint64_t> g_logged_head_draw_tiles;
static s32 g_ext_texture_trace = 0;

static bool gfx_is_tracked_head_tex(uint8_t type, uint32_t texnum);
static bool gfx_is_tracked_general_probe_tex(uint8_t type, uint32_t texnum);
static bool gfx_is_tracked_classic_temple_tex(uint8_t type, uint32_t texnum);
static bool gfx_is_fullscreen_intro_tex(uint8_t type, uint32_t texnum);
static bool gfx_should_use_external_tile_dimensions(uint8_t type, uint32_t texnum);
static void gfx_log_tracked_bind(uint8_t texture_unit, uint8_t tile, uint16_t tmem_slot,
        const LoadedTexture &loaded_texture);
static void gfx_log_tracked_draw_tile(uint8_t texture_unit, uint8_t tile,
        uint32_t tex_width, uint32_t tex_height, uint32_t tex_width2, uint32_t tex_height2);

static bool gfx_should_trace_ext_texture(void) {
    return g_ext_texture_trace != 0;
}

PD_CONSTRUCTOR static void gfxConfigInit(void) {
    configRegisterInt("Video.ExternalTextureTrace", &g_ext_texture_trace, 0, 1);
}
...
uint32_t gfx_texture_anisotropy_level = 1;
...
void gfx_texture_cache_clear() {
    ...
    g_logged_ext_texture_misses.clear();
    g_logged_ext_texture_setinfo.clear();
    g_logged_ext_texture_loads.clear();
    g_logged_ext_texture_binds.clear();
    extTexFree();
}
...
void gfx_texture_cache_delete(const uint8_t* orig_addr) {
    ...
    while (gfx_texture_cache.map.bucket_count() > 0) {
        TextureCacheKey key = { orig_addr, { 0 }, 0, 0, 0, 0, 0 }; // bucket index only depends on the address
...
static void import_texture(int i, int tile, bool importReplacement) {
    ...
    if ((rdp.tex_lod && tile >= rdp.first_tile_index + rdp.tex_detail) || !loaded_texture.addr) {
        ...
        const bool preserve_external = (loaded_texture.ext_key >> 56) != 0;
    ...
    gfx_log_tracked_bind(i, tile, rdp.texture_tile[tile].tmem, loaded_texture);

    TextureCacheKey key;
    uint8_t external = loaded_texture.ext_key >> 56;
    ...
    if (gfx_texture_cache_lookup(i, key)) {
        if (external) {
            ...
            if (gfx_should_trace_ext_texture() &&
                    (gfx_is_tracked_head_tex(type, texnum) || gfx_is_tracked_general_probe_tex(type, texnum))) {
                sysLogPrintf(LOG_NOTE,
                        "ext_tex: cache_hit tile=%d type=%u id=%04x tex=%04x ext_key=%016llx",
                        i, type, id, texnum, (unsigned long long)loaded_texture.ext_key);
            }
        }
    ...
    if (external) {
        ...
        if (gfx_should_trace_ext_texture() &&
                (gfx_is_tracked_head_tex(type, texnum) || gfx_is_tracked_general_probe_tex(type, texnum))) {
            sysLogPrintf(LOG_NOTE,
                    "ext_tex: upload_external tile=%d type=%u id=%04x tex=%04x ext_key=%016llx",
                    i, type, id, texnum, (unsigned long long)loaded_texture.ext_key);
        }
        ...
        if (gfx_should_trace_ext_texture() &&
                (gfx_is_tracked_head_tex(type, texnum) || gfx_is_tracked_general_probe_tex(type, texnum))) {
            sysLogPrintf(LOG_NOTE,
                    "ext_tex: upload_external_result tile=%d type=%u id=%04x tex=%04x size=%ux%u addr=%p",
                    i, type, id, texnum, width, height, addr);
        }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (comb->used_textures[i]) {
...

            {
                ...
                if ((loaded_texture.ext_key >> 56) != 0 &&
                        (gfx_is_fullscreen_intro_tex(loaded_texture.type, loaded_texture.texnum) ||
                         gfx_should_use_external_tile_dimensions(loaded_texture.type, loaded_texture.texnum))) {
            ...
            gfx_log_tracked_draw_tile(i, tile, tex_width[i], tex_height[i], tex_width2[i], tex_height2[i]);
...
static bool gfx_is_tracked_head_tex(uint8_t type, uint32_t texnum);
static bool gfx_try_infer_general_texture_info(const void *addr, uint32_t *texnum, int *matched_offset);
static inline uint64_t make_key(bool external, uint64_t type, uint64_t id, uint32_t texnum);

static bool gfx_is_fullscreen_intro_tex(uint8_t type, uint32_t texnum) {
    if (type != G_TEXTYPE_GENERAL) {
        return false;
    }
...
static void gfx_dp_set_texture_image(uint32_t format, uint32_t size, uint32_t width, uint32_t tex_flags, const void* addr) {
    ...
    uint32_t inferred_texnum = 0;
    int inferred_offset = 0;
    bool inferred_general = gfx_try_infer_general_texture_info(addr, &inferred_texnum, &inferred_offset);

    if (!rdp.texture_to_load.pending_info) {
    ...
    if (inferred_general && (!rdp.texture_to_load.pending_info ||
            rdp.texture_to_load.type == G_TEXTYPE_NONE)) {
        rdp.texture_to_load.type = G_TEXTYPE_GENERAL;
        rdp.texture_to_load.id = 0;
        rdp.texture_to_load.id_mask = 0;
        rdp.texture_to_load.texnum = inferred_texnum;

        if (gfx_should_trace_ext_texture() && gfx_is_tracked_head_tex(G_TEXTYPE_GENERAL, inferred_texnum)) {
            sysLogPrintf(LOG_NOTE,
                    "headtexhook: infer_texinfo type=%u id=%04x effective=%04x tex=%04x offset=%d",
                    G_TEXTYPE_GENERAL, 0, 0, inferred_texnum, inferred_offset);
        }
    }

    rdp.texture_to_load.pending_info = false;
}

static void gfx_dp_set_texture_info(uint8_t type, uint8_t id_mask, uint16_t id, uint32_t texnum) {
    rdp.texture_to_load.pending_info = true;
    ...
    if (gfx_should_trace_ext_texture() && type == G_TEXTYPE_GENERAL) {
        uint16_t effective_id = id | (id_mask << 8);
        uint64_t key = make_key(false, type, effective_id, texnum);

        if (g_logged_ext_texture_setinfo.insert(key).second) {
            sysLogPrintf(LOG_NOTE,
                    "ext_tex: set_texinfo type=%u id=%04x effective=%04x tex=%04x mask=%04x",
                    type, id, effective_id, texnum, id_mask << 8);
        }
    }

    if (gfx_should_trace_ext_texture() && gfx_is_tracked_head_tex(type, texnum)) {
        sysLogPrintf(LOG_NOTE,
                "headtexhook: set_texinfo type=%u id=%04x effective=%04x tex=%04x mask=%04x",
                type, id, id | (id_mask << 8), texnum, id_mask << 8);
    }
}
...
static inline uint64_t make_key(bool external, uint64_t type, uint64_t id, uint32_t texnum) {
    return (uint64_t)external << 56 | type << 48 | id << 32 | texnum;
}
...
static void gfx_log_tracked_draw_tile(uint8_t texture_unit, uint8_t tile,
    ...
    const auto &texture_tile = rdp.texture_tile[tile];
    const LoadedTexture &loaded_texture = rdp.loaded_texture[texture_tile.tmem];
    uint8_t type = loaded_texture.type;
    uint16_t id = loaded_texture.id | loaded_texture.id_mask;
    uint32_t texnum = loaded_texture.texnum;
    uint8_t external = loaded_texture.ext_key >> 56;

    if (!gfx_is_tracked_head_tex(type, texnum)) {
        return;
    }

    uint64_t key = ((uint64_t)texture_unit << 60)
            | ((uint64_t)tile << 56)
            | ((uint64_t)(texture_tile.tmem & 0x1ff) << 47)
            | ((uint64_t)(texnum & 0xffff) << 31)
            | ((uint64_t)(tex_width & 0x7ff) << 20)
            | ((uint64_t)(tex_height & 0x7ff) << 9)
            | ((uint64_t)(texture_tile.shifts & 0xf) << 5)
            | (uint64_t)(texture_tile.shiftt & 0xf);

    if (!g_logged_head_draw_tiles.insert(key).second) {
        return;
    }

    sysLogPrintf(LOG_NOTE,
            "headtexhook: draw_tile unit=%u first_tile=%u tile=%u tmem=%u type=%u id=%04x tex=%04x external=%u ext_key=%016llx tex_width=%u tex_height=%u tile_width=%u tile_height=%u tile_size=%ux%u line=%u fmt=%u siz=%u cms=%u cmt=%u shifts=%u shiftt=%u uls=%u ult=%u lrs=%u lrt=%u tex_lod=%u tex_detail=%u tex_max_lod=%u orig_size=%u full_size=%u",
            texture_unit, rdp.first_tile_index, tile, texture_tile.tmem,
            type, id, texnum, external, (unsigned long long)loaded_texture.ext_key,
            tex_width, tex_height, tex_width2, tex_height2,
            texture_tile.width, texture_tile.height, texture_tile.line_size_bytes,
            texture_tile.fmt, texture_tile.siz, texture_tile.cms, texture_tile.cmt,
            texture_tile.shifts, texture_tile.shiftt,
            texture_tile.uls, texture_tile.ult, texture_tile.lrs, texture_tile.lrt,
            rdp.tex_lod, rdp.tex_detail, rdp.tex_max_lod,
            loaded_texture.orig_size_bytes, loaded_texture.full_size_bytes);
}
...
static void gfx_log_tracked_external_overwrite(const char *op, uint8_t tile, uint16_t tmem_slot,
        const LoadedTexture &loaded_texture, uint8_t new_type, uint16_t new_id, uint32_t new_texnum,
        bool new_has_ext) {
    if (!gfx_should_trace_ext_texture()) {
        return;
    }

    uint8_t old_external = loaded_texture.ext_key >> 56;
    uint8_t old_type = loaded_texture.type;
    uint16_t old_id = loaded_texture.id | loaded_texture.id_mask;
    uint32_t old_texnum = loaded_texture.texnum;

    if (!old_external) {
        return;
    }

    if (!(gfx_is_tracked_head_tex(old_type, old_texnum) ||
          gfx_is_tracked_general_probe_tex(old_type, old_texnum))) {
        return;
    }

    sysLogPrintf(LOG_NOTE,
            "ext_tex: overwrite op=%s tile=%u tmem=%u old_type=%u old_id=%04x old_tex=%04x old_ext_key=%016llx new_type=%u new_id=%04x new_tex=%04x new_has_ext=%u",
            op, tile, tmem_slot, old_type, old_id, old_texnum,
            (unsigned long long)loaded_texture.ext_key,
            new_type, new_id, new_texnum, new_has_ext);
}
...
static void gfx_log_tracked_bind(uint8_t texture_unit, uint8_t tile, uint16_t tmem_slot,
    ...
    if (!gfx_is_tracked_classic_temple_tex(type, texnum)) {
        return;
    }

    uint64_t key = ((uint64_t)texture_unit << 56)
            | ((uint64_t)tile << 48)
            | ((uint64_t)tmem_slot << 32)
            | texnum;

    if (!g_logged_ext_texture_binds.insert(key).second) {
        return;
    }

    sysLogPrintf(LOG_NOTE,
            "ext_tex: bind_general unit=%u tile=%u tmem=%u type=%u id=%04x tex=%04x external=%u ext_key=%016llx addr=%p",
            texture_unit, tile, tmem_slot, type, id, texnum, external,
            (unsigned long long)loaded_texture.ext_key, loaded_texture.addr);
}

static bool gfx_try_infer_general_texture_info(const void *addr, uint32_t *texnum, int *matched_offset) {
    if (addr == nullptr || texnum == nullptr) {
        return false;
    }

    if (extTexLookupGeneralPtr(addr, texnum)) {
        if (matched_offset != nullptr) {
            *matched_offset = 0;
        }
    ...
    static const int kProbeOffsets[] = { -8, -4, -6, -2 };
    const uint8_t *bytes = static_cast<const uint8_t *>(addr);
    const uint8_t *general_start = romdataSegGetData("texturesdata");
    const uint8_t *general_end = romdataSegGetDataEnd("texturesdata");

    if (general_start == nullptr || general_end == nullptr || bytes < general_start || bytes >= general_end) {
        return false;
    }

    for (int offset : kProbeOffsets) {
        if (bytes + offset < general_start || bytes + offset + sizeof(uint16_t) > general_end) {
            continue;
        }

        uint16_t candidate16 = 0;
        uint32_t candidate;

        std::memcpy(&candidate16, bytes + offset, sizeof(candidate16));
        ...
        if (!gfx_external_textures_enabled || !extTexExists(G_TEXTYPE_GENERAL, 0, candidate)) {
            continue;
        }
...
static void gfx_log_ext_texture_missing_once(uint8_t type, uint16_t id, uint16_t id_mask, uint32_t texnum) {
    if (!gfx_should_trace_ext_texture()) {
        return;
    }

    uint16_t effective_id = id | id_mask;
    uint64_t key = make_key(false, type, effective_id, texnum);

    if (g_logged_ext_texture_misses.insert(key).second) {
        const char *name = NULL;

        if (type == G_TEXTYPE_MODEL) {
            name = romdataFileGetName(effective_id);
        }

        if (name && name[0]) {
            sysLogPrintf(LOG_NOTE,
                    "ext_tex: no replacement indexed for type=%u id=%04x name=%s tex=%04x (mask=%04x)",
                    type, effective_id, name, texnum, id_mask);
        } else {
            sysLogPrintf(LOG_NOTE,
                    "ext_tex: no replacement indexed for type=%u id=%04x tex=%04x (mask=%04x)",
                    type, effective_id, texnum, id_mask);
        }
    }
}
...
static void gfx_dp_load_block(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t dxt) {
    ...
    uint16_t effective_id = id | id_mask;
    bool has_ext_tex = false;

    if (gfx_external_textures_enabled) {
        has_ext_tex = extTexExists(type, effective_id, texnum);

        if (!has_ext_tex && id_mask == 0) {
            has_ext_tex = extTexExists(type, id, texnum);
        }
    }

    gfx_log_tracked_external_overwrite("load_block", tile, rdp.texture_tile[tile].tmem,
            loaded_texture, type, effective_id, texnum, has_ext_tex);

    if (gfx_should_trace_ext_texture() && type == G_TEXTYPE_GENERAL) {
        uint64_t key = make_key(false, type, effective_id, texnum);

        if (g_logged_ext_texture_loads.insert(key).second) {
            sysLogPrintf(LOG_NOTE,
                    "ext_tex: load_block_general id=%04x effective=%04x tex=%04x has_ext=%u addr=%p size=%u",
                    id, effective_id, texnum, has_ext_tex, rdp.texture_to_load.addr, size_bytes);
        }
    }

    if (gfx_should_trace_ext_texture() && gfx_is_tracked_head_tex(type, texnum)) {
        sysLogPrintf(LOG_NOTE,
                "headtexhook: load_block type=%u id=%04x effective=%04x tex=%04x has_ext=%u",
                type, id, effective_id, texnum, has_ext_tex);
    }

    if (gfx_external_textures_enabled && has_ext_tex) {
        loaded_texture.type = type;
        loaded_texture.id = effective_id;
        loaded_texture.texnum = texnum;
        loaded_texture.id_mask = 0;
        loaded_texture.ext_key = make_key(1, type, effective_id, texnum);
    } else {
        if (gfx_external_textures_enabled && type != G_TEXTYPE_NONE) {
            gfx_log_ext_texture_missing_once(type, id, id_mask, texnum);
        }
...
static void gfx_dp_load_tile(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t lrt) {
    ...
    gfx_log_tracked_external_overwrite("load_tile", tile, rdp.texture_tile[tile].tmem,
            loaded_texture, type, effective_id, texnum, has_ext_tex);
    ...
    if (gfx_should_trace_ext_texture() && type == G_TEXTYPE_GENERAL) {
        ...
        if (g_logged_ext_texture_loads.insert(key).second) {
            sysLogPrintf(LOG_NOTE,
                    "ext_tex: load_tile_general id=%04x effective=%04x tex=%04x has_ext=%u addr=%p size=%u offset=%u",
                    id, effective_id, texnum, has_ext_tex, rdp.texture_to_load.addr, size_bytes, start_offset_bytes);
        }
    }

    if (gfx_should_trace_ext_texture() && gfx_is_tracked_head_tex(type, texnum)) {
        sysLogPrintf(LOG_NOTE,
                "headtexhook: load_tile type=%u id=%04x effective=%04x tex=%04x has_ext=%u offset=%u",
                type, id, effective_id, texnum, has_ext_tex, start_offset_bytes);
    }

    if (gfx_external_textures_enabled && has_ext_tex) {
        // Preserve the external texture identity for tiles loaded via LoadTile too.
        loaded_texture.addr = rdp.texture_to_load.addr;
        loaded_texture.type = type;
        loaded_texture.id = effective_id;
        loaded_texture.texnum = texnum;
