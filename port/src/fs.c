...
const char *fsFullPath(const char *relPath)
{
	...
	if (kakarikoModDir[0] && g_ModNum == MOD_KAKARIKO) {
	...
	if (darknoonModDir[0] && g_ModNum == MOD_DARKNOON) {
	...
	if (goldfinger64ModDir[0] && g_ModNum == MOD_GOLDFINGER_64) {
...
static const char *fsFullPathForWrite(const char *relPath)
{
	static char pathBuf[FS_MAXPATH + 1];

	if (relPath[0] == '$' || !baseDir[0] || fsPathIsAbsolute(relPath) || fsPathIsCwdRelative(relPath)) {
		return fsFullPath(relPath);
	}
...
{
	return fopen(fsFullPathForWrite(name), "wb");
}
...
{
#ifdef PLATFORM_WIN32
	return _mkdir(fsFullPathForWrite(path));
#else
	return mkdir(fsFullPathForWrite(path), 0777);
#endif
}
