...
void aLoadBufferImpl(const void *source_addr, uint16_t dest_addr, uint16_t nbytes) {
    static int logged_loadbuf = 0;
    if (!logged_loadbuf) {
        sysLogPrintf(LOG_NOTE, "ext_music: aLoadBufferImpl src=%p dest=%u nbytes=%u", source_addr, dest_addr, nbytes);
        logged_loadbuf = 1;
    }
...
void aSaveBufferImpl(uint16_t source_addr, int16_t *dest_addr, uint16_t nbytes) {
    static int logged_savebuf = 0;
    if (!logged_savebuf) {
        sysLogPrintf(LOG_NOTE, "ext_music: aSaveBufferImpl src=%u dest=%p nbytes=%u", source_addr, dest_addr, nbytes);
        logged_savebuf = 1;
    }
...
void aInterleaveImpl(void) {
    static int logged_interleave = 0;
    if (!logged_interleave) {
        sysLogPrintf(LOG_NOTE, "ext_music: aInterleaveImpl");
        logged_interleave = 1;
    }
...
void aEnvMixerImpl(uint8_t flags, ENVMIX_STATE state, int16_t rvol) {
    static int logged_envmix = 0;
    if (!logged_envmix) {
        sysLogPrintf(LOG_NOTE, "ext_music: aEnvMixerImpl flags=%u state=%p vol=%d", flags, state, rvol);
        logged_envmix = 1;
    }
...
void aMixImpl(uint8_t flags, int16_t gain, uint16_t in_addr, uint16_t out_addr) {
    static int logged_mix = 0;
    if (!logged_mix) {
        sysLogPrintf(LOG_NOTE, "ext_music: aMixImpl flags=%u gain=%d in=%u out=%u", flags, gain, in_addr, out_addr);
        logged_mix = 1;
    }
...
void aSetVolumeImpl(uint8_t flags, int16_t v, int16_t t, int16_t r) {
    static int logged_setvol = 0;
    if (!logged_setvol) {
        sysLogPrintf(LOG_NOTE, "ext_music: aSetVolumeImpl flags=%u v=%d t=%d r=%d", flags, v, t, r);
        logged_setvol = 1;
    }
...
void aPlayMP3Impl(const void *mp3file, u32 mp3size, void *out, int reset) {
    ...
    static s32 source_hz = 44100;
    static s32 mono_count = 0;
    static u32 phase_fp = 0;
    static int logged_start = 0;
    static int decode_count = 0;
    static int16_t mono_buf[MINIMP3_MAX_SAMPLES_PER_FRAME * 8];
    mp3d_sample_t decoded[MINIMP3_MAX_SAMPLES_PER_FRAME];
    s16 *dst = (s16 *)out;

    enum {
        MP3_OUTPUT_SAMPLES = 580,
        MP3_TARGET_HZ = 22020,
        MP3_PHASE_BITS = 16,
        MP3_PHASE_ONE = 1 << MP3_PHASE_BITS,
    };
    ...
    if (!logged_start) {
        sysLogPrintf(LOG_NOTE, "ext_music: aPlayMP3Impl start mp3=%p size=%u out=%p reset=%d", mp3file, mp3size, out, reset);
        logged_start = 1;
    }

    memset(out, 0, MP3_OUTPUT_SAMPLES * sizeof(s16));

    for (s32 out_index = 0; out_index < MP3_OUTPUT_SAMPLES; out_index++) {
        while (((phase_fp >> MP3_PHASE_BITS) + 1) >= (u32)mono_count) {
            mp3dec_frame_info_t info;
            const s32 old_dataptr = dataptr;
            const s32 samples = dataptr < mp3size
                ? mp3dec_decode_frame(&mp3d, curdata + dataptr, mp3size - dataptr, decoded, &info)
                : 0;

            if (decode_count < 8) {
                sysLogPrintf(LOG_NOTE, "ext_music: aPlayMP3Impl decode[%d] samples=%d frame_bytes=%d channels=%d hz=%d layer=%d bitrate=%d dataptr=%d remaining=%u", decode_count, samples, info.frame_bytes, info.channels, info.hz, info.layer, info.bitrate_kbps, old_dataptr, mp3size - old_dataptr);
            }

            if (samples > 0 && info.channels > 0) {
                const s32 append_limit = (s32)(sizeof(mono_buf) / sizeof(mono_buf[0])) - mono_count;
                const s32 append_count = samples < append_limit ? samples : append_limit;

                if (info.hz > 0) {
                    source_hz = info.hz;
                }

                for (s32 i = 0; i < append_count; i++) {
                    if (info.channels >= 2) {
                        const s32 base = i * info.channels;
                        mono_buf[mono_count + i] = (int16_t)(((s32)decoded[base] + (s32)decoded[base + 1]) / 2);
                    } else {
                        mono_buf[mono_count + i] = decoded[i];
                    }
                }

                mono_count += append_count;
            }
            ...
            if (decode_count < 8) {
                sysLogPrintf(LOG_NOTE, "ext_music: aPlayMP3Impl advance[%d] old=%d new=%d", decode_count, old_dataptr, dataptr);
            }
            decode_count++;

            if (samples <= 0 && dataptr >= mp3size) {
                break;
            }
        }
        ...
        {
            const u32 idx = phase_fp >> MP3_PHASE_BITS;
            const u32 frac = phase_fp & (MP3_PHASE_ONE - 1);
            const s32 s0 = mono_buf[idx];
            const s32 s1 = (idx + 1 < (u32)mono_count) ? mono_buf[idx + 1] : s0;
            dst[out_index] = (int16_t)(s0 + (((s1 - s0) * (s32)frac) >> MP3_PHASE_BITS));
        }

        phase_fp += (u32)(((uint64_t)source_hz << MP3_PHASE_BITS) / MP3_TARGET_HZ);

        if ((phase_fp >> MP3_PHASE_BITS) > MINIMP3_MAX_SAMPLES_PER_FRAME) {
            const s32 consumed = (s32)(phase_fp >> MP3_PHASE_BITS);

            if (consumed > 0) {
                const s32 remaining = mono_count - consumed;

                if (remaining > 0) {
                    memmove(mono_buf, mono_buf + consumed, remaining * sizeof(mono_buf[0]));
                }

                mono_count = remaining > 0 ? remaining : 0;
                phase_fp -= (u32)(consumed << MP3_PHASE_BITS);
            }
        }
    }
    ...
    if (decode_count < 8) {
        sysLogPrintf(LOG_NOTE, "ext_music: aPlayMP3Impl wrote[%d] samples=%d,%d,%d,%d out=%p hz=%d buffered=%d", decode_count, dst[0], dst[1], dst[2], dst[3], out, source_hz, mono_count);
    }
}

void aPoleFilterImpl(uint8_t flags, int16_t gain, uint32_t t, uint32_t addr) {
    static int logged_pole = 0;
    if (!logged_pole) {
        sysLogPrintf(LOG_NOTE, "ext_music: aPoleFilterImpl flags=%u gain=%d t=%u addr=0x%x", flags, gain, t, addr);
        logged_pole = 1;
    }
}
