...
static s32 g_SfxBrowserIndex = 0;
static struct sndstate *g_SfxBrowserPreviewHandle = NULL;

static MenuItemHandlerResult menuhandlerSelectPlayer(s32 operation, struct menuitem *item, union handlerdata *data);

static void sfxbrowserStopPreview(void)
{
	if (g_SfxBrowserPreviewHandle) {
		audioStop(g_SfxBrowserPreviewHandle);
		g_SfxBrowserPreviewHandle = NULL;
	}
}
...
static MenuItemHandlerResult menuhandlerAimStickSpeed(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GETSLIDER:
		if (g_PlayerExtCfg[g_ExtMenuPlayer].aimstickspeed < 0.f) {
			data->slider.value = 0;
		} else if (g_PlayerExtCfg[g_ExtMenuPlayer].aimstickspeed > 4.f) {
			data->slider.value = 400;
		} else {
			data->slider.value = g_PlayerExtCfg[g_ExtMenuPlayer].aimstickspeed * 100.f + 0.5f;
		}
		break;
	case MENUOP_SET:
		g_PlayerExtCfg[g_ExtMenuPlayer].aimstickspeed = (f32)data->slider.value / 100.f;
		break;
	case MENUOP_GETSLIDERLABEL:
		sprintf(data->slider.label, "%.2f", (f32)data->slider.value / 100.f);
		break;
...
static MenuItemHandlerResult menuhandlerAimStickBounds(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GETSLIDER:
		if (g_PlayerExtCfg[g_ExtMenuPlayer].aimstickbounds < 0.f) {
			data->slider.value = 0;
		} else if (g_PlayerExtCfg[g_ExtMenuPlayer].aimstickbounds > 1.f) {
			data->slider.value = 100;
		} else {
			data->slider.value = g_PlayerExtCfg[g_ExtMenuPlayer].aimstickbounds * 100.f + 0.5f;
		}
		break;
	case MENUOP_SET:
		g_PlayerExtCfg[g_ExtMenuPlayer].aimstickbounds = (f32)data->slider.value / 100.f;
		break;
	case MENUOP_GETSLIDERLABEL:
		sprintf(data->slider.label, "%.2f", (f32)data->slider.value / 100.f);
		break;
...
static MenuItemHandlerResult menuhandlerStickDeadzone(s32 operation, struct menuitem *item, union handlerdata *data)
{
	/* Deadzone sliders start immediately after the separator at index 4. */
...
struct menuitem g_ExtendedControllerMenuItems[] = {
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LITERAL_TEXT | MENUITEMFLAG_SLIDER_WIDE,
		(uintptr_t)"Aim Mode Sensitivity",
		400,
		menuhandlerAimStickSpeed,
	},
	{
		MENUITEMTYPE_SLIDER,
		0,
		MENUITEMFLAG_LITERAL_TEXT | MENUITEMFLAG_SLIDER_WIDE,
		(uintptr_t)"Aim Mode Bounds",
		100,
		menuhandlerAimStickBounds,
	},
	{
		MENUITEMTYPE_SEPARATOR,
		0,
...
static MenuItemHandlerResult menuhandlerTexAnisotropy(s32 operation, struct menuitem *item, union handlerdata *data)
{
	static const char *opts[] = {
		"Off",
		"2x",
	...
	case MENUOP_GETOPTIONTEXT:
		return (intptr_t)opts[data->dropdown.value];
	case MENUOP_SET:
		videoSetTextureAnisotropy(data->dropdown.value == 0 ? 1 : (1 << data->dropdown.value));
		break;
	case MENUOP_GETSELECTEDINDEX:
		anisotropy = videoGetTextureAnisotropy();
		if (anisotropy < 2) {
			data->dropdown.value = 0;
...
MenuItemHandlerResult menuhandlerSfxBrowserSound(s32 operation, struct menuitem *item, union handlerdata *data)
{
	static char namebuf[96];
	s32 numoptions = ARRAYCOUNT(g_SfxNames);

	if (g_SfxBrowserIndex < 0 || g_SfxBrowserIndex >= numoptions) {
		g_SfxBrowserIndex = 0;
	}

	switch (operation) {
	case MENUOP_GETOPTIONCOUNT:
		data->dropdown.value = numoptions > 0 ? numoptions : 1;
		break;
	case MENUOP_GETOPTIONTEXT:
		if (numoptions <= 0) {
			return (uintptr_t)"No matching sounds";
		}

		snprintf(namebuf, sizeof(namebuf), "%04X %s", data->dropdown.value, g_SfxNames[data->dropdown.value]);
		return (uintptr_t)namebuf;
	case MENUOP_SET:
		if (numoptions > 0) {
			sfxbrowserStopPreview();
			g_SfxBrowserIndex = data->dropdown.value;
			snd00010718(&g_SfxBrowserPreviewHandle, 0, AL_VOL_FULL, AL_PAN_CENTER, g_SfxBrowserIndex, 1, 1, -1, 1);
		} else {
			sfxbrowserStopPreview();
		}
		break;
	case MENUOP_GETSELECTEDINDEX:
		if (numoptions <= 0) {
			...
			data->dropdown.value = g_SfxBrowserIndex;
		}
		break;
	}
...
MenuDialogHandlerResult menudialogExtendedAudio(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	if (operation == MENUOP_OPEN || operation == MENUOP_CLOSE) {
		sfxbrowserStopPreview();
	}
...
struct menuitem g_ExtendedAudioMenuItems[] = {
	...
	},
	{
		MENUITEMTYPE_DROPDOWN,
		0,
		MENUITEMFLAG_LITERAL_TEXT | MENUITEMFLAG_SILENT_PREVIEW | MENUITEMFLAG_00010000,
		(uintptr_t)"SOUND TEST",
		0,
		menuhandlerSfxBrowserSound,
	},
	{
		MENUITEMTYPE_SEPARATOR,
		0,
...
static MenuItemHandlerResult menuhandlerSuperAutoAim(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GET:
		return g_PlayerExtCfg[g_ExtMenuPlayer].superautoaim;
	case MENUOP_SET:
		g_PlayerExtCfg[g_ExtMenuPlayer].superautoaim = data->checkbox.value;
		break;
	}
...
struct menuitem g_ExtendedGameMenuItems[] = {
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)"Super Auto Aim (SP)",
		0,
		menuhandlerSuperAutoAim,
	},
	{
		MENUITEMTYPE_SLIDER,
		0,
