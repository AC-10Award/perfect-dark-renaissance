...
s32 g_OneHitBots = false;
s32 g_MpSpHitReactions = false;
s32 g_MpPickupRespawnSpeed = 0;
s32 g_MpPickupRespawnSfx = true;
...
int main(int argc, const char **argv)
{
	...
	mainProc();
	sysLogPrintf(LOG_ERROR, "exit path: mainProc returned");
...
PD_CONSTRUCTOR static void gameConfigInit(void)
{
	...
	configRegisterInt("Game.OneHitBots", &g_OneHitBots, 0, 1);
	configRegisterInt("Game.MpSpHitReactions", &g_MpSpHitReactions, 0, 1);
	configRegisterInt("Game.MpPickupRespawnSpeed", &g_MpPickupRespawnSpeed, 0, 3);
	configRegisterInt("Game.MpPickupRespawnSfx", &g_MpPickupRespawnSfx, 0, 1);
	...
	for (s32 j = 0; j < MAX_PLAYERS; ++j) {
		...
		configRegisterFloat(strFmt("Game.Player%d.AimStickSpeed", i), &g_PlayerExtCfg[j].aimstickspeed, 0.f, 4.f);
		configRegisterFloat(strFmt("Game.Player%d.AimStickBounds", i), &g_PlayerExtCfg[j].aimstickbounds, 0.f, 1.f);
		configRegisterInt(strFmt("Game.Player%d.SuperAutoAim", i), &g_PlayerExtCfg[j].superautoaim, 0, 1);
		...
		configRegisterInt(strFmt("Game.Player%d.UseKeyReloads", i), &g_PlayerExtCfg[j].usereloads, 0, 1);
	}
}
