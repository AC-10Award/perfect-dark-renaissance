...
MP Setup File Format
	...
	[numsetups{1}]
	# setups
	[setup_1{versioned block size}]
	...
	[setup_n{versioned block size}]
 */

#define MPSETUP_VERSION 7
#define MPSETUP_BLOCKSIZE_V1 80
#define MPSETUP_BLOCKSIZE_V2 128
...
static size_t mpsetupGetBlockSize(u8 version)
{
	if (version >= MPSETUP_VERSION) {
		return MPSETUP_BLOCKSIZE;
	}

	if (version >= 2) {
		return MPSETUP_BLOCKSIZE_V2;
	}

	return MPSETUP_BLOCKSIZE_V1;
}

static char *mpsetupGetDisplayName(struct setupblock *block)
{
	func0f0d564c_ext(block->bytes, g_StringPointer, false, MPSETUP_MAXNAME + 1);

	if (g_StringPointer[0] == '\0') {
		strcpy(g_StringPointer, "<unnamed>\n");
	}

	return g_StringPointer;
}

static bool mpsetupIsKnownVersion(u8 version)
{
	return version >= 1 && version <= MPSETUP_VERSION;
}

static bool mpsetupIsBlockValid(struct setupblock *block, u8 version)
{
	...
	mpsetupfileGetOverview(block->bytes, version, filename, &numsims, &stagenum, &scenarionum);

	if (filename[0] == '\0') {
		return false;
	}

	if (numsims > MAX_BOTS_RUNTIME) {
		return false;
	}

	if (scenarionum > MPSCENARIO_TERMINATION) {
		return false;
	}
...
static void mpsetupSanitizeFile(struct mpsetupfile *setupfile)
{
	s32 writeindex = 0;
	s32 i;

	if (!mpsetupIsKnownVersion(setupfile->version)) {
		memset(setupfile, 0, sizeof(*setupfile));
		setupfile->version = MPSETUP_VERSION;
		return;
	}

	for (i = 0; i < setupfile->numsetups; i++) {
		if (mpsetupIsBlockValid(&setupfile->setups[i], setupfile->version)) {
			if (writeindex != i) {
				memcpy(setupfile->setups[writeindex].bytes, setupfile->setups[i].bytes, MPSETUP_BLOCKSIZE);
			}
			writeindex++;
		}
	}

	for (i = writeindex; i < MPSETUP_MAXSETUPS; i++) {
		memset(setupfile->setups[i].bytes, 0, MPSETUP_BLOCKSIZE);
	}

	setupfile->numsetups = writeindex;

	if (setupfile->defaultsetup == 0 || setupfile->defaultsetup > setupfile->numsetups) {
...
static MenuDialogHandlerResult menudialogManageSettings(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data);
...
static s32 mpsetupDeserialize(FILE *f, struct mpsetupfile *setupfile)
{
	...
	blocksize = mpsetupGetBlockSize(setupfile->version);
...
static s32 mpsetupSerialize(FILE *f, struct mpsetupfile *setupfile)
{
	...
	for (int i = 0; i < setupfile->numsetups; ++i) {
		wx += fwrite(setupfile->setups[i].bytes, mpsetupGetBlockSize(setupfile->version), 1, f);
	}
...
static s32 mpsetupLoadFile(struct mpsetupfile *setupfile, u8 op, bool loaddefault)
{
	...
	mpsetupSanitizeFile(setupfile);

	if (loaddefault && op == MPSETUP_OP_DEFAULT && setupfile->defaultsetup > 0) {
...
static MenuItemHandlerResult menuhandlerImportOrExportSettings(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_GETOPTIONTEXT:
		if (data->list.value < numsetups) {
			return (uintptr_t)mpsetupGetDisplayName(&setupfile->setups[data->list.value]);
		} else {
...
static MenuItemHandlerResult menuhandlerOpenImportExportDialog(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_SET:
		if (item->param == MPSETUP_OP_IMPORT) {
			...
			mpsetupLoadFile(&g_ImportMpSetupFile, MPSETUP_OP_IMPORT, false);
		} else {
...
static MenuItemHandlerResult menuhandlerSelectSetupHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_GETOPTIONTEXT:
		mpsetupRefreshCurrentFile();
		if (data->list.value < 0 || data->list.value >= g_MpSetupFile.numsetups) {
			return (uintptr_t)"";
		}
		return (uintptr_t)mpsetupGetDisplayName(&g_MpSetupFile.setups[data->list.value]);
	case MENUOP_SET:
		if (data->list.value < 0 || data->list.value >= g_MpSetupFile.numsetups) {
			return 0;
		}
...
{
	return mpsetupLoadFile(&g_MpSetupFile, MPSETUP_OP_DEFAULT, true);
}

s32 mpsetupRefreshCurrentFile(void)
{
	return mpsetupLoadFile(&g_MpSetupFile, MPSETUP_OP_DEFAULT, false);
}
...
s32 mpsetupSaveSetup(s32 slotindex, u8 savefile)
{
	...
	memset(g_MpSetupFile.setups[slotindex].bytes, 0, MPSETUP_BLOCKSIZE);
...
void mpsetupLoadSetup(s32 index)
{
	...
	memcpy(&buffer.bytes, block->bytes, mpsetupGetBlockSize(g_MpSetupFile.version));
